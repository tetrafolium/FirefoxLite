...
public class TelemetryAnnotationProcessor extends AbstractProcessor {
  ...
  // TODO: TelemetryEvent's fields are private, I'll create a PR to make them
  // public so I can test the ping format in compile time.
  ...
  private void genDoc(Collection<? extends Element> annotatedElements,
                      String header, String path, char separator)
      ...
      if (type.getKind() == ElementKind.METHOD) {
        ...
        if (result != null) {
          throw new IllegalArgumentException(
              "Duplicate event combination:" + annotation + "\n" + result);
        }
        ...
        sb.append(start)
            ...
            .append(separator)
            .append(annotation.category())
            .append(separator)
            .append(annotation.method())
            .append(separator)
            .append(annotation.object())
            .append(separator)
            .append('"')
        ...
        processingEnv.getMessager().printMessage(
            Diagnostic.Kind.ERROR, "This should not happen:" + type);
      }
    }
  ...
  String verifyEventDuplication(TelemetryDoc annotation,
                                HashMap<String, Boolean> lookup) {
    String key = annotation.category() + annotation.method() +
                 annotation.object() + annotation.value();
  ...
  private void verifyEventFormat(TelemetryDoc annotation) {
    ...
    if (action.length() > TelemetryEventConstant.MAX_LENGTH_CATEGORY) {
      throw new IllegalArgumentException("The length of category is too long:" +
                                         action);
    }
    final String method = annotation.method();
    if (method.length() > TelemetryEventConstant.MAX_LENGTH_METHOD) {
      throw new IllegalArgumentException("The length of method is too long:" +
                                         method);
    }
    final String object = annotation.object();
    if (object.length() > TelemetryEventConstant.MAX_LENGTH_OBJECT) {
      throw new IllegalArgumentException("The length of object is too long:" +
                                         object);
    }
    final String value = annotation.value();
    if (value.length() > TelemetryEventConstant.MAX_LENGTH_VALUE) {
      throw new IllegalArgumentException("The length of value is too long:" +
                                         value);
    }
    ...
    for (TelemetryExtra extra : extras) {
      ...
      if (eName.length() > TelemetryEventConstant.MAX_LENGTH_EXTRA_KEY) {
        throw new IllegalArgumentException(
            "The length of extra key is too long:" + eName);
      }
      if (eVal.length() > TelemetryEventConstant.MAX_LENGTH_VALUE) {
        throw new IllegalArgumentException(
            "The length of extra value is too long:" + eVal);
      }
    }
  }
}

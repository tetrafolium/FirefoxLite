...
public class RepositoryTest {
  ...
  private static final String FAKE_PATH =
      "/atp?channel=*&section=top-news&lang=english&curpg=%d&pp=%d&v=v1";
  ...
  public void testParsing() throws InterruptedException {
    try {
      ...
      webServer.start();
      Repository repository = new Repository<NewsPointItem>(
          InstrumentationRegistry.getContext(), null, SOCKET_TAG,
          itemPojoList
          -> {
            Assert.assertEquals(LOAD_SIZE, itemPojoList.size());
            countDownLatch.countDown();
          },
          null, "FAKE", DEFAULT_SUBSCRIPTION_URL,
          RepositoryNewsPoint.FIRST_PAGE, RepositoryNewsPoint.PARSER, false) {
        @Override

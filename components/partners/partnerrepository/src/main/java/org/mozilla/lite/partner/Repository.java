...
public abstract class Repository<T extends NewsItem> {
  ...
  @SuppressFBWarnings(value = "URF_UNREAD_PUBLIC_OR_PROTECTED_FIELD",
                      justification =
                          "Variable would be read in the child classes")
  ...
  public Repository(Context context, String userAgent, int socketTag,
                    OnDataChangedListener onDataChangedListener,
                    OnCacheInvalidateListener onCacheInvalidateListener,
                    String subscriptionKeyName, String subscriptionUrl,
                    int firstPage, Parser<T> parser, boolean dedup) {
  ...
  public void loadMore() { nextSubscription(); }
  ...
  public void
  setOnDataChangedListener(@Nullable OnDataChangedListener listener) {
  ...
  private BackgroundCachedRequestLoader createLoader(int page) {
    return new BackgroundCachedRequestLoader(
        context, getSubscriptionKey(page), getSubscriptionUrl(page), userAgent,
        socketTag, cacheIsDirty);
  }

  private Observer<Pair<Integer, String>>
  createObserver(LiveData<Pair<Integer, String>> liveData, int page) {
    return new Observer<Pair<Integer, String>>() {
       ...
       */
      private boolean updateCacheStatus(List<T> oldValuesFromCache,
                                        List<T> newValuesFromNetwork) {
        ...
        if (!TextUtils.isEmpty(integerStringPair.second)) {
          try {
            ...
            if (!cacheIsDirty) {
              ...
              if (cacheIsDirty &&
                  integerStringPair.first == ResponseData.SOURCE_NETWORK) {
          ...
          if ((integerStringPair.first == ResponseData.SOURCE_NETWORK &&
              "".equals(integerStringPair.second)) && (onDataChangedListener != null)) {
            onDataChangedListener.onDataChanged(cloneData());
          }
        }
        // Removes the subscription and mark as done once network returns, no
        // matter if there is a Network error or not.
        if (integerStringPair.first == ResponseData.SOURCE_NETWORK) {
          removeObserver();
        }
      }
  ...
  public interface OnCacheInvalidateListener { void onCacheInvalidate(); }

...
public class ShrinkSizeTransformation extends BitmapTransformation {
  ...
  protected Bitmap transform(@NonNull BitmapPool pool,
                             ...
                             int outHeight) {
    Bitmap bitmap = pool.get((int)(toTransform.getWidth() / scale),
                             (int)(toTransform.getHeight() / scale),
                             toTransform.getConfig());
    Canvas canvas = new Canvas(bitmap);
    canvas.drawBitmap(toTransform, margin(toTransform.getWidth(), scale),
                      margin(toTransform.getHeight(), scale), paint);
    return bitmap;
  }

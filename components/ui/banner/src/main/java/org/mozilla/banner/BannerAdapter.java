...
public class BannerAdapter extends RecyclerView.Adapter<BannerViewHolder> {
  ...
  public BannerAdapter(@NonNull String[] configs,
    ...
    for (int i = 0; i < configs.length; i++) {
      ...
      // We always write full item into cache, but discard a page if it is not
      // known in this apk version. With this once the user upgrades, the cache
      // is still usable. We still inflate an item with SingleButtonViewHolder
      // if it for unknown reason managed to pass this check. (See:
      // onCreateViewHolder)
      if (getItemViewType(thisDAO.type) == UNKNOWN_VIEW_TYPE) {
        Logger.throwOrWarn(LOG_CONDITION, LOG_TAG,
                           String.format(Locale.US,
                                         "Unknown view type: %s in page %d",
                                         thisDAO.type, i),
                           null);
        continue;
      }

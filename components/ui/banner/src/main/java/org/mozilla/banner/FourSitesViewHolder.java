...
class FourSitesViewHolder extends BannerViewHolder {
  ...
  public void onBindViewHolder(Context context, BannerDAO bannerDAO) {
    ...
    try {
      Glide.with(context)
          .load(bannerDAO.values.getString(0))
      ...
      for (int i = 0; i < icons.length; i++) {
        ...
        PorterDuffColorFilter alphaToWhitePorterDuff =
            new PorterDuffColorFilter(Color.WHITE, PorterDuff.Mode.DST_OVER);
        Glide.with(context)
            .load(bannerDAO.values.getString(1 + i))
            .apply(new RequestOptions().transforms(
                new ShrinkSizeTransformation(0.62f),
                new PorterDuffTransformation(alphaToWhitePorterDuff),
                new CircleCrop()))
            .into(new SimpleTarget<Drawable>() {
              @Override
              public void onResourceReady(
    ...
    background.setOnClickListener(v -> { sendClickBackgroundTelemetry(); });
  }
}

...
public class UrlUtils {
  ...
  private final static String[] HTTP_SCHEME_PREFIX_ARRAY = {
      HTTP_SCHEME_PREFIX, HTTPS_SCHEME_PREFIX};
  ...
  private final static String[] COMMON_SUBDOMAINS_PREFIX_ARRAY = {
      WWW_PREFIX, MOBILE_PREFIX, M_PREFIX};
  ...
  public static String stripUserInfo(@Nullable String url) {
    ...
    try {
      ...
      // Strip the userInfo to minimise spoofing ability. This only affects
      // what's shown during browsing, this information isn't used when we start
      // editing the URL:
      ...
      // We might be trying to display a user-entered URL (which could plausibly
      // contain errors), in this case its safe to just return the raw input.
      // There are also some special cases that URI can't handle, such as
      // "http:" by itself.
      return url;
    }
  }
  ...
  isPermittedResourceProtocol(@NonNull final String scheme) {
    if (TextUtils.isEmpty(scheme)) {
      return false;
    }

    return scheme.startsWith("http") || scheme.startsWith("https") ||
        scheme.startsWith("file") || scheme.startsWith("data");
  }
  ...
  public static boolean isSupportedProtocol(@NonNull final String scheme) {
    ...
    return isPermittedResourceProtocol(scheme) || scheme.startsWith("error");
  }
  ...
  public static boolean urlsMatchExceptForTrailingSlash(final
                                                        @NonNull String url1,
                                                        final
                                                        @NonNull String url2) {
  ...
  public static String stripPrefix(@Nullable String host,
                                   @NonNull String[] prefixArray) {
    if (host == null) {
      return null;
    }

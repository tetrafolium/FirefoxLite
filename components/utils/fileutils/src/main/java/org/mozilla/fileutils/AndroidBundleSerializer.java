...
public class AndroidBundleSerializer {
  ...
  public interface DataTypeHandler {
    ...
    void restore(Bundle bundle, SerializedItem serializedItem)
        throws IOException;
  }
  ...
  public void serializeBundle(ObjectOutputStream outputStream, Bundle bundle)
      throws IOException {
  ...
  public Bundle deserializeBundle(ObjectInputStream inputStream)
      throws IOException {
    ...
    for (SerializedItem serializedItem : serializedItemList) {
      if (serializedItem != null) {
        final DataTypeHandler dataTypeHandler =
            dataTypeHandlers.get(serializedItem.getClassName());
        if (dataTypeHandler != null) {
  ...
  private static class NullHandler implements DataTypeHandler {
    ...
    public void restore(final Bundle bundle,
                        final SerializedItem serializedItem)
        throws IOException {
  ...
  private static class StringHandler implements DataTypeHandler {
    ...
    public void restore(final Bundle bundle,
        ...
        throws IOException {
      bundle.putString(
          serializedItem.getKey(),
          new String(serializedItem.getValue(), Charset.forName("UTF-8")));
    }
  }
}

...
public class RequestLoaderDelegation {
  ...
  @VisibleForTesting static final int TEST_DELAY = 300;

  RequestLoaderDelegation(Context context, String subscriptionKey,
                          String subscriptionUrl, String userAgent,
                          int socketTag, RequestLoader requestLoader) {
  ...
  ResponseData getStringLiveData() {
    if (stringLiveData == null) {
      ...
      requestLoader.loadFromRemote(context, stringLiveData, subscriptionUrl,
                                   userAgent, socketTag);
    }
  ...
  void deleteCache() { requestLoader.deleteCache(context, subscriptionKey); }

  interface RequestLoader {
    void loadFromCache(Context context, String subscriptionKey,
                       ResponseData stringLiveData);

    void loadFromRemote(Context context, ResponseData stringLiveData,
                        String subscriptionUrl, String userAgent,
                        int socketTag);

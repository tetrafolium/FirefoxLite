...
public class CachedRequestLoaderTest {
  ...
  public void testLoadAndCacheLoadFaster() throws InterruptedException {
    ...
    CachedRequestLoader cachedRequestLoader = new CachedRequestLoader(
        context, KEY, targetUrl, null, SOCKET_TAG, false, true);
    cachedRequestLoader.deleteCache(context, KEY);
    LiveData<Pair<Integer, String>> liveData =
        cachedRequestLoader.getStringLiveData();
    liveData.observeForever(observer);
    latch1.await();
    // TODO: 11/5/18 This can be a source of intermittent.
    // Wait one second so the write to cache finishes.
    Thread.sleep(1000);
    CachedRequestLoader urlSubscription2 =
        new CachedRequestLoader(InstrumentationRegistry.getContext(), KEY,
                                targetUrl, null, SOCKET_TAG, false, true);
    LiveData<Pair<Integer, String>> liveData2 =
        urlSubscription2.getStringLiveData();
  ...
  public void testLoadAndCacheCacheFaster() throws InterruptedException {
    ...
    CachedRequestLoader cachedRequestLoader =
        new CachedRequestLoader(InstrumentationRegistry.getContext(), KEY2,
                                targetUrl, null, SOCKET_TAG, true, false);
    LiveData<Pair<Integer, String>> liveData =
        cachedRequestLoader.getStringLiveData();
    liveData.observeForever(observer);
    latch1.await();

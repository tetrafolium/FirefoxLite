...
public interface TabView {
  class HitTarget {
    ...
    public HitTarget(@NonNull final TabView source, final boolean isLink,
                     final String linkURL, final boolean isImage,
  ...
  /**
   * Enable/Disable content blocking for this session (Only the blockers that
   * are enabled in the app's settings will be turned on/off).
   */
  void setContentBlockingEnabled(boolean enabled);

  /**
   * Be invoked by TabChromeClient.onCreateWindow to transport this
   * new-created-window to its parent window.
   *

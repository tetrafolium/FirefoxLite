...
public class EntityListTest {
  ...
  // TODO: we might want to clean up the mess of revhost vs normal host vs
  // inserting a whitelist item vs inserting a whitelist trie. And that
  // isWhiteListed relies on domains, the rest doesn't
  @Test
  public void testWhitelist() {
    ...
    // Thus mozilla.org can only use foo.com, but foo.mozilla.org can use
    // foo.com and bar.com
    ...
    entityList.putWhiteList(FocusString.create(mozillaOrg).reverse(),
                            fooComTrie);
    entityList.putWhiteList(FocusString.create(fooMozillaOrg).reverse(),
                            barComTrie);

    assertTrue(entityList.isWhiteListed(Uri.parse("http://" + mozillaOrg),
                                        Uri.parse("http://" + fooCom)));
    assertFalse(entityList.isWhiteListed(Uri.parse("http://" + mozillaOrg),
                                         Uri.parse("http://" + barCom)));

    assertTrue(entityList.isWhiteListed(Uri.parse("http://" + fooMozillaOrg),
                                        Uri.parse("http://" + fooCom)));
    assertTrue(entityList.isWhiteListed(Uri.parse("http://" + fooMozillaOrg),
                                        Uri.parse("http://" + barCom)));

    // Test some junk inputs to make sure we haven't messed up
    assertFalse(entityList.isWhiteListed(Uri.parse("http://" + barCom),
                                         Uri.parse("http://" + barCom)));
    assertFalse(entityList.isWhiteListed(Uri.parse("http://" + barCom),
                                         Uri.parse("http://" + mozillaOrg)));

    // Test some made up subdomains to ensure they still match *.foo.mozilla.org
    assertTrue(entityList.isWhiteListed(Uri.parse("http://"
                                                  + "hello." + fooMozillaOrg),
                                        Uri.parse("http://" + fooCom)));
    assertTrue(entityList.isWhiteListed(Uri.parse("http://"
                                                  + "hello." + fooMozillaOrg),
    ...
    assertTrue(entityList.isWhiteListed(Uri.parse("http://"
                                                  + "hello." + mozillaOrg),
                                        Uri.parse("http://" + fooCom)));
    assertFalse(entityList.isWhiteListed(Uri.parse("http://"
                                                   + "hello." + mozillaOrg),
                                         Uri.parse("http://" + barCom)));

    // And random subpages don't fail:
    assertTrue(entityList.isWhiteListed(
        Uri.parse("http://" + mozillaOrg + "/somewhere"),
        Uri.parse("http://" + fooCom + "/somewhereElse/bla/bla")));
    assertFalse(entityList.isWhiteListed(

...
public class TrieTest {
  ...
  public void findNode() throws Exception {
    ...
    // Substring matching: doesn't happen (except for subdomains, we test those
    // later)
  ...
  public void testWhiteListTrie() {
    ...
    // In practice EntityList uses it's own search in order to cover all
    // possible matching notes (e.g. in case we have separate whitelists for
    // mozilla.org and foo.mozilla.org), however we don't need to test that here
    // yet.
    final WhiteListTrie foundWhitelist =
        (WhiteListTrie)trie.findNode(FocusString.create("def"));

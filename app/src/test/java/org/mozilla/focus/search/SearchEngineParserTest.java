...
public class SearchEngineParserTest {
  ...
  public void testParser() throws Exception {
    ...
    final SearchEngine searchEngine =
        SearchEngineParser.load(searchEngineIdentifier, stream);
  ...
  private static void collectSearchPlugins(Collection<Object[]> searchPlugins,
                                           File path) {
    if (!path.isDirectory()) {
    ...
    if (entries == null) {
      throw new AssertionError("No files in directory " +
    ...
    for (String entry : entries) {
      ...
      if (file.isDirectory()) {
        ...
        searchPlugins.add(new Object[] {
            file.getAbsolutePath(), entry.substring(0, entry.length() - 4)});
      }
    }
  }
  ...
  private static File getBasePath() {
    final ClassLoader classLoader =
        SearchEngineParserTest.class.getClassLoader();

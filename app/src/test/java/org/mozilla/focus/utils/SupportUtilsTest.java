...
public class SupportUtilsTest {
  ...
  @Test
  public void cleanup() {
    // Other tests might get confused by our locale fiddling, so lets go back to
    // the default:
  ...
  public void testNormalize() {
    assertEquals("http://www.mozilla.org",
                 SupportUtils.normalize("http://www.mozilla.org"));
    assertEquals("https://www.mozilla.org",
                 SupportUtils.normalize("https://www.mozilla.org"));
    assertEquals("https://www.mozilla.org/en-US/internet-health/",
                 SupportUtils.normalize(
                     "https://www.mozilla.org/en-US/internet-health/"));
    assertEquals("file:///mnt/sdcard/",
                 SupportUtils.normalize("file:///mnt/sdcard/"));

    assertEquals("http://mozilla.org", SupportUtils.normalize("mozilla.org"));
    assertEquals("http://mozilla.org",
                 SupportUtils.normalize("http://mozilla.org "));
    assertEquals("http://mozilla.org",
                 SupportUtils.normalize(" http://mozilla.org "));
    assertEquals("http://mozilla.org",
                 SupportUtils.normalize(" http://mozilla.org"));
  ...
  public void testIsUrl() {
    ...
    assertTrue(SupportUtils.isUrl(" https://www.mozilla.org "));
    assertTrue(
        SupportUtils.isUrl("https://www.mozilla.org/en-US/internet-health/"));
  ...
  /*
   * Super simple sumo URL test - it exists primarily to verify that we're
   * setting the language and page tags correctly. appVersion is null in tests,
   * so we just test that there's a null there, which doesn't seem too useful...
   */
  @Test
  public void getSumoURLForTopic() throws Exception {
    ...
    assertEquals("https://support.mozilla.org/1/mobile/" + appVersion +
                     "/Android/de-DE/foobar",
                 SupportUtils.getSumoURLForTopic(
                     ApplicationProvider.getApplicationContext(), "foobar"));

    Locale.setDefault(Locale.CANADA_FRENCH);
    assertEquals("https://support.mozilla.org/1/mobile/" + appVersion +
                     "/Android/fr-CA/foobar",
                 SupportUtils.getSumoURLForTopic(
                     ApplicationProvider.getApplicationContext(), "foobar"));
  }

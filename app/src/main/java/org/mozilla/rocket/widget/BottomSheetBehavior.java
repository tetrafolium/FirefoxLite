...
/**
 * An interaction behavior plugin for a child view of {@link CoordinatorLayout}
 * to make it work as a bottom sheet.
 *
 * Modification:
 * We need to extend BottomSheetBehavior to support ViewPager. To prevent future
 * changes in BottomSheetBehavior class, we need to "snapshot" its
 * implementation. This file is for that purpose.
 *
...
public class BottomSheetBehavior<V extends View>
  ...
  public abstract static class BottomSheetCallback {
     ...
     * @param newState The new state. This will be one of {@link
     *     #STATE_DRAGGING}, {@link #STATE_SETTLING}, {@link #STATE_EXPANDED},
     *     {@link #STATE_COLLAPSED}, {@link #STATE_HIDDEN}, or {@link
     *     #STATE_HALF_EXPANDED}.
     */
     ...
     *
     * @param bottomSheet The bottom sheet view.
     * @param slideOffset The new offset of this bottom sheet within [-1,1]
     *     range. Offset increases as this bottom sheet is moving upward. From 0
     *     to 1 the sheet is between collapsed and expanded states and from -1
     *     to 0 it is between hidden and collapsed states.
     */
  ...
  @Override
  public void onRestoreInstanceState(CoordinatorLayout parent, V child,
                                     Parcelable state) {
  ...
  public boolean onLayoutChild(CoordinatorLayout parent, V child,
    ...
    if (peekHeightAuto) {
      ...
      lastPeekHeight =
          Math.max(peekHeightMin, parentHeight - parent.getWidth() * 9 / 16);
    } else {
  ...
  public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child,
    ...
    case MotionEvent.ACTION_DOWN:
      ...
      View scroll = nestedScrollingChildRef != null
                        ...
                        : null;
      if (scroll != null &&
          parent.isPointInChildBounds(scroll, initialX, initialY)) {
    ...
    if (!ignoreEvents && viewDragHelper != null &&
        viewDragHelper.shouldInterceptTouchEvent(event)) {
      return true;
    }
    // We have to handle cases that the ViewDragHelper does not capture the
    // bottom sheet because it is not the top most view of its parent. This is
    // not necessary when the touch event is happening over the scrolling
    // content as nested scrolling logic handles that case.
    View scroll =
        nestedScrollingChildRef != null ? nestedScrollingChildRef.get() : null;
    return action == MotionEvent.ACTION_MOVE && scroll != null &&
        !ignoreEvents && state != STATE_DRAGGING &&
        !parent.isPointInChildBounds(scroll, (int)event.getX(),
                                     (int)event.getY()) &&
        viewDragHelper != null &&
  ...
  @Override
  public boolean
  onStartNestedScroll(@NonNull CoordinatorLayout coordinatorLayout,
  ...
  public void onNestedPreScroll(@NonNull CoordinatorLayout coordinatorLayout,
                                @NonNull V child, @NonNull View target, int dx,
    ...
    if (nestedScrollingChildRef != null) {
      return target == nestedScrollingChildRef.get() &&
          (state != STATE_EXPANDED ||
           super.onNestedPreFling(coordinatorLayout, child, target, velocityX,
                                  velocityY));
    } else {
      return false;
    }
  ...
  /**
   * @return whether the height of the expanded sheet is determined by the
   ...
   */
  public boolean isFitToContents() { return fitToContents; }

  /**
   * Sets whether the height of the expanded sheet is determined by the height
   * of its contents, or if it is expanded in two stages (half the height of the
   * parent container, full height of parent container). Default value is true.
   *
   * @param fitToContents whether or not to fit the expanded sheet to its
   *     contents.
   */
  public void setFitToContents(boolean fitToContents) {
    ...
    // If sheet is already laid out, recalculate the collapsed offset based on
    // new setting. Otherwise, let onLayoutChild handle this later.
    if (viewRef != null) {
      calculateCollapsedOffset();
    }
    // Fix incorrect expanded settings depending on whether or not we are
    // fitting sheet to contents.
    setStateInternal((this.fitToContents && state == STATE_HALF_EXPANDED)
                         ? STATE_EXPANDED
                         : state);
  }
  ...
  /**
   * Sets the height of the bottom sheet when it is collapsed while optionally
   * animating between the old height and the new height.
   *
   * @param peekHeight The height of the collapsed bottom sheet in pixels, or
   *     {@link #PEEK_HEIGHT_AUTO} to configure the sheet to peek automatically
   *     at 16:9 ratio keyline.
   * @param animate Whether to animate between the old height and the new
   *     height.
   * @attr ref
   * com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_peekHeight
   */
   ...
   *     #PEEK_HEIGHT_AUTO} if the sheet is configured to peek automatically at
   *     16:9 ratio keyline
   * @attr ref
   * com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_peekHeight
   */
   ...
   */
  public void setHideable(boolean hideable) { this.hideable = hideable; }
   ...
   * being hidden after it is expanded once. Setting this to true has no effect
   * unless the sheet is hideable.
   *
   * @param skipCollapsed True if the bottom sheet should skip the collapsed
   *     state.
   * @attr ref
  ...
  /**
   * Sets the state of the bottom sheet. The bottom sheet will transition to
   * that state with animation.
   *
   * @param state One of {@link #STATE_COLLAPSED}, {@link #STATE_EXPANDED},
   *     {@link #STATE_HIDDEN}, or {@link #STATE_HALF_EXPANDED}.
   */
  public final void setState(@State int state) {
    ...
    if (viewRef == null) {
      // The view is not laid out yet; modify mState and let onLayoutChild
      // handle it later
      if (state == STATE_COLLAPSED || state == STATE_EXPANDED ||
          state == STATE_HALF_EXPANDED || (hideable && state == STATE_HIDDEN)) {
        this.state = state;
      }
      return;
    }
   ...
   *
   * @return One of {@link #STATE_EXPANDED}, {@link #STATE_HALF_EXPANDED},
   *     {@link #STATE_COLLAPSED},
   *     {@link #STATE_DRAGGING}, {@link #STATE_SETTLING}, or {@link
   * #STATE_HALF_EXPANDED}.
   */
  @State
  ...
  void setStateInternal(@State int state) {
    ...
    bottomSheet.sendAccessibilityEvent(
  ...
  boolean shouldHide(View child, float yvel) {
    ...
    return Math.abs(newTop - collapsedOffset) / (float)peekHeight >
        HIDE_THRESHOLD;
  }
  ...
  void startSettlingAnimation(View child, int state) {
    ...
    if (state == STATE_COLLAPSED) {
      ...
      if (fitToContents && top <= fitToContentsOffset) {
        // Skip to the expanded state if we would scroll past the height of the
        // contents.
        state = STATE_EXPANDED;
        top = fitToContentsOffset;
      }
  ...
  private final ViewDragHelper.Callback dragCallback =
      new ViewDragHelper.Callback() {
        ...
        @Override
        public void onViewPositionChanged(@NonNull View changedView, int left,
        ...
        @Override
        public void onViewReleased(@NonNull View releasedChild, float xvel,
                                   float yvel) {
          int top;
            ...
            if (fitToContents) {
              ...
              if (currentTop > halfExpandedOffset) {
                ...
                targetState = STATE_EXPANDED;
                     ...
                     (releasedChild.getTop() > collapsedOffset ||
                      Math.abs(xvel) < Math.abs(yvel))) {
            // Hide if we shouldn't collapse and the view was either released
            // low or it was a vertical swipe.
            top = parentHeight;
            targetState = STATE_HIDDEN;
          } else if (yvel == 0.f || Math.abs(xvel) > Math.abs(yvel)) {
            // If the Y velocity is 0 or the swipe was mostly horizontal
            // indicated by the X velocity being greater than the Y velocity,
          ...
          if (viewDragHelper.settleCapturedViewAt(releasedChild.getLeft(),
                                                  top)) {
            setStateInternal(STATE_SETTLING);
        ...
        public int clampViewPositionVertical(@NonNull View child, int top,
                                             int dy) {
          return MathUtils.clamp(top, getExpandedOffset(),
                                 hideable ? parentHeight : collapsedOffset);
        }
  ...
  void dispatchOnSlide(int top) {
    ...
    if (bottomSheet != null && callback != null) {
      if (top > collapsedOffset) {
        callback.onSlide(bottomSheet, (float)(collapsedOffset - top) /
                                          (parentHeight - collapsedOffset));
      } else {
        callback.onSlide(bottomSheet,
                         (float)(collapsedOffset - top) /
  ...
  protected static class SavedState extends AbsSavedState {
    ...
    public SavedState(Parcel source) { this(source, null); }
  ...
  /**
   * A utility function to get the {@link BottomSheetBehavior} associated with
   * the {@code view}.
   *
  ...
  private void updateImportantForAccessibility(boolean expanded) {
    ...
    if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) && expanded) {
      if (importantForAccessibilityMap == null) {
        ...
        // The important for accessibility values of the child views have been
        // saved already.
        return;
      }
    }
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (!expanded) {
        if (importantForAccessibilityMap != null &&
          ...
          // Restores the original important for accessibility value of the
          // child view.
          ViewCompat.setImportantForAccessibility(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
          importantForAccessibilityMap.put(
              child, child.getImportantForAccessibility());
        }

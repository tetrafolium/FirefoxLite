...
public class NewsSourceManager {
  ...
  public static NewsSourceManager getInstance() { return instance; }
  ...
  public void init(Context context) {
    ...
    ThreadUtils.postToBackgroundThread(() -> {
      ...
      // "DainikBhaskar.com" doesn't provide their feed anymore. Switch to
      // "Newspoint" by default
      if (TextUtils.isEmpty(source) || NEWS_DB.equals(source)) {
        newsSource = NEWS_NP;
        Log.d(NewsSourceManager.TAG,
              "NewsSourceManager sets default:" + newsSource);
        ...
        Log.d(NewsSourceManager.TAG,
              "NewsSourceManager already set:" + newsSource);
      }
      // previously we only set the source url after remote config is fetched.
      // But when there's no internet connection, the remote config's callback
      // will never be called, thus newsSourceUrl will always be null. We try to
      // set the url here if previously we already got the value from remote
      // config.
      final String url =
  ...
  public String getNewsSourceUrl() { return newsSourceUrl; }

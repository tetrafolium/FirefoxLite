...
public class PartnerActivator {
  ...
  private static final String PARTNER_ACTIVATION_SOURCE =
      "https://firefox.settings.services.mozilla.com/v1/buckets/main/collections/rocket-prefs/records";

  private static Executor executor = Executors.newCachedThreadPool();

  public enum Status { Disabled, Default, Done, Snooze }
  ...
  public void launch() { postWorker(new QueryActivationStatus(this)); }

  private void postWorker(Runnable runnable) { executor.execute(runnable); }
  ...
  private Status getStatus() {
    ...
    final String statusString = getPreferences(context).getString(
        KEY_STRING_STATUS, Status.Default.toString());
  ...
  private void setStatus(Status status) {
    getPreferences(context)
        .edit()
        .putString(KEY_STRING_STATUS, status.toString())
        .apply();
  }
  ...
  private void setSnoozeDuration(long duration) {
    getPreferences(context)
        .edit()
        .putLong(KEY_LONG_SNOOZE_DURATION, duration)
        .apply();
  }
  ...
  private void setLastCheckedTimestamp(long timestamp) {
    getPreferences(context)
        .edit()
        .putLong(KEY_LONG_FETCH_TIMESTAMP, timestamp)
        .apply();
  }
  ...
  private static final class QueryActivationStatus
      extends ActivationJobs implements Runnable {
    ...
    public void run() {
      ...
      String partnerActivateKey =
          PartnerUtil.getProperty(PARTNER_ACTIVATION_KEY);
      ...
      if (partnerActivator.partnerActivateKeys == null ||
          partnerActivator.partnerActivateKeys.length != 3) {
        PartnerUtil.log("partner key format invalid");
        return;
      }

      partnerActivator.postWorker(
          new FetchActivation(partnerActivator, PARTNER_ACTIVATION_SOURCE));
    }
  }

  private static final class FetchActivation
      extends ActivationJobs implements Runnable {
    ...
    public void run() {
      try {
        ...
        String json =
            HttpRequest.get(request, HTTP_REQUEST_TIMEOUT, UserAgentString);
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
          try {
            ...
            activationJsonArray =
                object.getJSONObject("partner").getJSONArray("activation");
        ...
        for (int i = 0, size = activationJsonArray.length(); i < size; i++) {
          JSONObject activationJsonObject =
              (JSONObject)activationJsonArray.get(i);
  ...
  private static class PingActivation
      extends ActivationJobs implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        fields.put("device_id",
                   new JSONObject().put(
                       TYPE_STRING,
                       PartnerUtil.getDeviceIdentifier(
                           partnerActivator.context.getContentResolver())));
        fields.put("owner",
                   new JSONObject().put(TYPE_STRING, activation.owner));
        fields.put("version",
                   new JSONObject().put(TYPE_INTEGER, activation.version));
        fields.put("manufacture",
                   new JSONObject().put(
                       TYPE_STRING,
                       PartnerUtil.getProperty("ro.product.manufacturer")));
        fields.put("model",
                   new JSONObject().put(TYPE_STRING, PartnerUtil.getProperty(
                                                         "ro.product.model")));
        fields.put("name",
                   new JSONObject().put(TYPE_STRING, PartnerUtil.getProperty(
                                                         "ro.product.name")));
        ...
        fields.put("build_id",
                   new JSONObject().put(
                       TYPE_STRING, PartnerUtil.getProperty("ro.build.id")));
        ...
        if (responseCode != HttpURLConnection.HTTP_OK) {
          ...
          PartnerUtil.log("PingActivation server response not OK: " +
                          responseCode);
        } else {

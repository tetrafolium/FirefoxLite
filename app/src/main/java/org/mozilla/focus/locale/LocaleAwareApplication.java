...
public class LocaleAwareApplication extends Application {
  ...
  public void onConfigurationChanged(Configuration config) {
    ...
    // Otherwise, correct the locale. This catches some cases that the current
    // Activity doesn't get a chance to.
    try {
      LocaleManager.getInstance().correctLocale(this, getResources(), config);
    } catch (IllegalStateException ex) {
      // Activity hasn't started yey, so we have no ContextGetter in
      // LocaleManager.
    }
  ...
  public void onActivityPause() { mInBackground = true; }

  public void onActivityResume() { mInBackground = true; }
}

...
public class BannerHelper {
  ...
  private static class LoadRootConfigTask extends SimpleLoadUrlTask {
    ...
    protected void onPostExecute(String line) {
      ...
      try {
        ...
        LoadConfigTask.OnConfigLoadedListener onConfigLoadedListener =
            (config, index) -> {
        ...
        for (int i = 0; i < length; i++) {
          new LoadConfigTask(new WeakReference<>(onConfigLoadedListener), i)
              .execute(jsonArray.getString(i), userAgent,
                       Integer.toString(SocketTags.BANNER));
        }
  ...
  private static class LoadConfigTask extends SimpleLoadUrlTask {
    ...
    protected void onPostExecute(String line) {
      ...
      line = line.replace("\n", "").replace("\r", "");
      OnConfigLoadedListener onConfigLoadedListener =
          onConfigLoadedListenerRef.get();
  ...
  void initHomeBanner(Context context,
                      MutableLiveData<String[]> configLiveData) {
    if (listener != null) {
      initBanner(context, AppConfigWrapper.getBannerRootConfig(),
                 CURRENT_HOME_BANNER_CONFIG, configLiveData,
                 listener::hideHomeBannerProcedure);
    }
  }

  public void initCouponBanner(Context context,
                               MutableLiveData<String[]> configLiveData) {
    initBanner(context, AppConfigWrapper.getCouponBannerRootConfig(),
               CURRENT_COUPON_BANNER_CONFIG, configLiveData, null);
  }

  // TODO: 10/3/18 Now we have cachedrequestloader, should consider migrate to
  // use it.
  private void initBanner(Context context, String manifest, String cacheName,
                          MutableLiveData<String[]> configLiveData,
                          Consumer<Void> hideBannerProcedure) {
    // Setup from Cache
    try {
      new FileUtils
          .ReadStringFromFileTask<>(
              new FileUtils.GetCache(new WeakReference<>(context)).get(),
              cacheName, configLiveData, BannerHelper::stringToStringArray)
          .execute();
    ...
    if (TextUtils.isEmpty(manifest)) {
      ...
      new LoadRootConfigTask(callback).execute(
          manifest, WebViewProvider.getUserAgentString(context),
          Integer.toString(SocketTags.BANNER));
    }
  }

  private static class Callback
      implements LoadRootConfigTask.OnRootConfigLoadedListener {

    private WeakReference<Context> contextRef;
    private String cacheName;
    private MutableLiveData<String[]> configLiveData;

    Callback(Context context, String cacheName,
             MutableLiveData<String[]> configLiveData) {
  ...
  void setUpHomeBannerFromConfig(String[] configArray) {
    TelemetryListener bannerInnerTelemetryListener = new TelemetryListener() {
      ...
      public void sendClickItemTelemetry(String jsonString, int itemPosition) {
        try {
          TelemetryWrapper.clickBannerItem(
              new JSONObject(jsonString).getString("id"), itemPosition);
        } catch (JSONException e) {
          e.printStackTrace();
        }
      }
      ...
      public void sendClickBackgroundTelemetry(String jsonString) {
        try {
          TelemetryWrapper.clickBannerBackground(
              new JSONObject(jsonString).getString("id"));
        } catch (JSONException e) {
          e.printStackTrace();
        }
      }
    ...
    if (listener != null) {
      setUpBannerFromConfig(configArray, this::updateHomeConfig,
                            homeBannerconfigArray, bannerInnerTelemetryListener,
                            listener::hideHomeBannerProcedure,
                            listener::showHomeBannerProcedure);
    }
  }
  ...
  private void
  setUpBannerFromConfig(String[] configArray, Consumer<String[]> configUpdater,
                        String[] oldConfigArray,
                        TelemetryListener telemetryListener,
                        Consumer<Void> hideBannerConsumer,
                        Consumer<BannerAdapter> showBannerConsumer) {
    ...
    try {
      ...
      BannerAdapter bannerAdapter = new BannerAdapter(
          configArray, listener.onBannerClickListener(), telemetryListener);
  ...
  private static void writeToCache(Context context, String[] configArray,
                                   String cacheName) {
    try {
      final Runnable runnable = new FileUtils.WriteStringToFileRunnable(
          new File(new FileUtils.GetCache(new WeakReference<>(context)).get(),
                   cacheName),
          stringArrayToString(configArray));
      ThreadUtils.postToBackgroundThread(runnable);
    } catch (ExecutionException | InterruptedException e) {
      e.printStackTrace();
      LoggerWrapper.throwOrWarn(
          TAG,
          "Failed to open cache directory when writing banner config to cache");
    }
  }

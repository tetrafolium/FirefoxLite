...
class TopSiteAdapter extends RecyclerView.Adapter<SiteViewHolder> {
  ...
  @NotNull
  @Override
  public SiteViewHolder onCreateViewHolder(@NotNull ViewGroup parent,
                                           int viewType) {
  ...
  private int addWhiteToColorCode(int colorCode,
  ...
  public void onBindViewHolder(@NotNull SiteViewHolder holder, int position) {
    ...
    // Tried AsyncTask and other simple offloading, the performance drops
    // significantly.
    // FIXME: 9/21/18 by saving bitmap color, cause
    // FaviconUtils.getDominantColor runs slow. Favicon
    Bitmap favicon = StrictModeViolation.tempGrant(
    ...
    ViewCompat.setBackgroundTintList(holder.img,
                                     ColorStateList.valueOf(backgroundColor));

    // Pin
    holder.pinView.setVisibility(pinSiteManager.isPinned(site) ? View.VISIBLE
                                                               : View.GONE);
  ...
  private Bitmap getBestFavicon(Resources res, String url,
  ...
  private Bitmap createFavicon(Resources resources, String url,
                               int backgroundColor) {
  ...
  private int calculateBackgroundColor(Bitmap favicon) {
    ...
    int red = addWhiteToColorCode((dominantColor & 0x00FF0000) >> 16, 0.25f)
              << 16;
    int green = addWhiteToColorCode((dominantColor & 0x0000FF00) >> 8, 0.25f)
                << 8;

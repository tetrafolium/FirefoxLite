...
public class HomeFragment extends LocaleAwareFragment
  ...
  private ContentFeature contentFeature =
      new ContentFeature(Settings.getInstance(getContext()));
  ...
  final Observer<String[]> homeBannerObserver =
      bannerHelper::setUpHomeBannerFromConfig;
  ...
  private void observeChromeAction() {
    chromeViewModel.getClearBrowsingHistory().observe(
        this, unit -> { updateTopSitesData(); });
  }
  ...
  private void showCurrentBannerTelemetry() {
    if (homeBanner.getVisibility() != View.VISIBLE ||
    ...
    String id = ((BannerViewHolder)homeBanner.getChildViewHolder(displayedView))
                    .getId();
    if (id == null) {
      return;
    }
  ...
  public OnClickListener onBannerClickListener() {
    return arg
        -> chromeViewModel.getOpenUrl().setValue(
            new OpenUrlAction(arg, true, false));
  }
  ...
  public View onCreateView(@NonNull LayoutInflater inflater,
    ...
    if (contentFeature.hasContentPortal()) {
      view = inflater.inflate(R.layout.fragment_homescreen_content, container,
                              false);
    ...
    SnapHelper snapHelper = new PagerSnapHelper() {
      ...
      // This is kinda deprecated by sendSwipeToIdTelemetry so consider removing
      // it in the future.
      private void sendSwipeTelemetry(int superRet, int velocityX) {
        ...
        TelemetryWrapper.swipeBannerItem(velocityX / Math.abs(velocityX),
                                         boundedTarget);
      }
      ...
      private void sendSwipeToIdTelemetry(int superRet) {
        ...
        String id =
            ((BannerViewHolder)homeBanner.getChildViewHolder(nextDisplayed))
                .getId();
        if (id == null) {
          return;
        }
      ...
      public int findTargetSnapPosition(
          RecyclerView.LayoutManager layoutManager, int velocityX,
          int velocityY) {
        final int superRet =
    ...
    if (LifeFeedOnboarding.shouldShow(getContext())) {
      ...
      LayoutInflater.from(view.getContext())
          .inflate(R.layout.fragment_homescreen_life_feed_onboarding,
                   home_container);
      final TextView contentTextView =
          home_container.findViewById(R.id.life_feed_onboarding_content);
      contentTextView.setText(LifeFeedOnboarding.getContentText(getContext()));
      contentTextView.setMovementMethod(LinkMovementMethod.getInstance());

      lifeFeedOnboardingLayer =
          home_container.findViewById(R.id.life_feed_onboarding_root);
      lifeFeedOnboardingLayer.setOnClickListener(
          v -> { dismissLifeFeedOnboarding(); });
    }
  ...
  private void dismissLifeFeedOnboarding() {
    if (lifeFeedOnboardingLayer != null) {
      ((ViewGroup)lifeFeedOnboardingLayer.getParent())
          .removeView(lifeFeedOnboardingLayer);
  ...
  public void onResume() {
    ...
    this.receiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        bannerHelper.initHomeBanner(context,
                                    bannerConfigViewModel.getHomeConfig());
      }
    };
    Context context = getContext();
    if (context != null) {
      LocalBroadcastManager.getInstance(context).registerReceiver(this.receiver,
                                                                  intentFilter);
    }
    updateTopSitesData();
    setupBannerTimer();
  ...
  private void playContentPortalAnimation() {
    final Animation fadeout =
        AnimationUtils.loadAnimation(getActivity(), R.anim.arrow_fade_out);
    final Animation fadein =
        AnimationUtils.loadAnimation(getActivity(), R.anim.arrow_fade_in);
  ...
  private void setupBannerTimer() {
    ...
    timer.scheduleAtFixedRate(new TimerTask() {
      ...
      public void run() {
        ...
        int nextPage =
            (bannerLayoutManager.findFirstVisibleItemPosition() + 1) %
  ...
  public void onActivityCreated(@Nullable Bundle savedInstanceState) {
    ...
    doWithActivity(getActivity(),
                   themeManager
                   -> themeManager.subscribeThemeChange(homeScreenBackground));

    Context context = getContext();

    bannerConfigViewModel =
        ViewModelProviders.of(getActivity()).get(BannerConfigViewModel.class);
  ...
  public void onDestroyView() {
    doWithActivity(
        getActivity(),
        themeManager
        -> themeManager.unsubscribeThemeChange(homeScreenBackground));
  ...
  public void showSites(@NonNull List<Site> sites) {
    ...
    if (this.topSiteAdapter == null) {
      this.topSiteAdapter = new TopSiteAdapter(sites, clickListener,
                                               clickListener, pinSiteManager);
  ...
  public void appendSite(@NonNull Site site) {
    final TopSiteAdapter adapter =
        (TopSiteAdapter)this.recyclerView.getAdapter();
    if (adapter != null) {
  ...
  private void setupBottomBar(View rootView) {
    ...
    bottomBarItemAdapter = new BottomBarItemAdapter(
        bottomBar, BottomBarItemAdapter.Theme.Dark.INSTANCE);
    BottomBarViewModel bottomBarViewModel =
        Inject.obtainBottomBarViewModel(getActivity());
  ...
  private class SiteItemClickListener
    ...
    public void onClick(View v) {
      ...
      if (site != null) {
        ScreenNavigator.get(v.getContext())
            .showBrowserScreen(site.getUrl(), true, false);
        ViewParent viewParent = v.getParent();
        if (viewParent instanceof ViewGroup) {
          int index = ((ViewGroup)v.getParent()).indexOfChild(v);
          TelemetryWrapper.clickTopSiteOn(
              index, site.isDefault() ? site.getTitle() : "");
        }
      }
    }
    ...
    public boolean onLongClick(View v) {
      ...
      final PopupMenu popupMenu =
          new PopupMenu(v.getContext(), v, Gravity.CLIP_HORIZONTAL);
      popupMenu.getMenuInflater().inflate(R.menu.menu_top_site_item,
                                          popupMenu.getMenu());

      MenuItem pinItem = popupMenu.getMenu().findItem(R.id.pin);
      if (pinItem != null) {
        pinItem.setVisible(pinSiteManager.isEnabled() &&
                           !pinSiteManager.isPinned(site));
      }
      ...
      popupMenu.setOnMenuItemClickListener(item -> {
        ...
        case R.id.remove:
          if (site.getId() < 0) {
            ...
            TopSitesUtils.saveDefaultSites(
                getContext(), HomeFragment.this.orginalDefaultSites);
            refreshTopSites();
            TelemetryWrapper.removeTopSite(true);
          } else {
            site.setViewCount(1);
            BrowsingHistoryManager.getInstance().updateLastEntry(
                site, mTopSiteUpdateListener);
  ...
  private void constructTopSiteList(List<Site> historySites) {
    ...
    if (topSites.size() > TOP_SITES_QUERY_LIMIT) {
      List<Site> removeSites =
          topSites.subList(TOP_SITES_QUERY_LIMIT, topSites.size());
  ...
  private void mergeHistorySiteToTopSites(List<Site> historySites,
                                          List<Site> topSites) {
    for (Site topSite : topSites) {
      removeDuplicatedSites(historySites, topSite,
                            site
                            -> topSite.setViewCount(topSite.getViewCount() +
                                                    site.getViewCount()));
    }
    topSites.addAll(historySites);
  }

  private void mergePinSiteToTopSites(List<Site> pinSites,
                                      List<Site> topSites) {
  ...
  private void initDefaultSitesFromJSONArray(JSONArray jsonDefault) {
    List<Site> defaultSites =
        TopSitesUtils.paresJsonToList(getContext(), jsonDefault);
  ...
  private void initFeatureSurveyViewIfNecessary(final View view) {
    final RemoteConfigConstants.SURVEY featureSurvey =
        RemoteConfigConstants.SURVEY.Companion.parseLong(
            AppConfigWrapper.getFeatureSurvey());
    final ImageView imgSurvey = view.findViewById(R.id.home_wifi_vpn_survey);
    final Settings.EventHistory eventHistory =
        Settings.getInstance(getContext()).getEventHistory();
    if (featureSurvey == RemoteConfigConstants.SURVEY.WIFI_FINDING &&
        !eventHistory.contains(Settings.Event.FeatureSurveyWifiFinding)) {
      imgSurvey.setImageResource(R.drawable.find_wifi);
      imgSurvey.setVisibility(View.VISIBLE);
      if (getContext() != null) {
        imgSurvey.setOnClickListener(
            new FeatureSurveyViewHelper(getContext(), featureSurvey));
      }
    } else if (featureSurvey == RemoteConfigConstants.SURVEY.VPN &&
               !eventHistory.contains(Settings.Event.FeatureSurveyVpn)) {
      ...
      if (getContext() != null) {
        imgSurvey.setOnClickListener(
            new FeatureSurveyViewHelper(getContext(), featureSurvey));
               ...
               !eventHistory.contains(Settings.Event.VpnRecommenderIgnore)) {
  ...
  // TODO: Reduce complexity here, a possible solution is to use the HashMap for
  // site list
  private void removeDuplicatedSites(List<Site> sites, Site site,
                                     OnRemovedListener listener) {
    final Iterator<Site> siteIterator = sites.iterator();
    while (siteIterator.hasNext()) {
      Site nextSite = siteIterator.next();
      if (UrlUtils.urlsMatchExceptForTrailingSlash(nextSite.getUrl(),
                                                   site.getUrl())) {
        siteIterator.remove();
  ...
  private interface OnRemovedListener { void onRemoved(Site site); }

  private static void parseCursorToSite(Cursor cursor, List<String> urls,
                                        List<byte[]> icons) {
    String url = cursor.getString(
        cursor.getColumnIndex(HistoryContract.BrowsingHistory.URL));
    byte[] icon = cursor.getBlob(
        cursor.getColumnIndex(HistoryContract.BrowsingHistory.FAV_ICON));
    urls.add(url);
    icons.add(icon);
  }
  ...
  private static class MigrateHistoryRunnable implements Runnable {
    ...
    public void run() {
      ...
      final SupportSQLiteOpenHelper helper =
          HistoryDatabase.getInstance(context).getOpenHelper();
      final SupportSQLiteDatabase db = helper.getWritableDatabase();
      // We can't differentiate if this is a new install or upgrade given the db
      // version will already become the latest version here. We create a temp
      // table if no migration is needed and later delete it to prevent
      // crashing.
      db.execSQL(HistoryDatabase.CREATE_LEGACY_IF_NOT_EXIST);
      final SupportSQLiteQueryBuilder builder =
          SupportSQLiteQueryBuilder.builder(
              HistoryDatabaseHelper.Tables.BROWSING_HISTORY_LEGACY);
      final String[] columns = {HistoryContract.BrowsingHistory._ID,
                                HistoryContract.BrowsingHistory.URL,
                                HistoryContract.BrowsingHistory.FAV_ICON};
      ...
      if (icons.size() == 0) {
        ...
        new FavIconUtils
            .SaveBitmapsTask(
                faviconFolder, urls, icons,
                new UpdateHistoryWrapper(urls, handlerWeakReference),
      ...
      PreferenceManager.getDefaultSharedPreferences(context)
          .edit()
          .putBoolean(TOP_SITES_V2_PREF, true)
          .apply();
    }
  }
  ...
  private static class UpdateHistoryWrapper
      implements FavIconUtils.Consumer<List<String>> {

    private List<String> urls;
    private WeakReference<Handler> handlerWeakReference;

    private UpdateHistoryWrapper(List<String> urls,
    ...
    public void accept(List<String> fileUris) {
      ...
      for (int i = 0; i < fileUris.size(); i++) {
        if (i == fileUris.size() - 1) {
          BrowsingHistoryManager.updateHistory(null, urls.get(i),
                                               fileUris.get(i), listener);
        } else {
          BrowsingHistoryManager.updateHistory(null, urls.get(i),
                                               fileUris.get(i));
        }
  ...
  public void setNightModeEnabled(boolean enable) {
    ...
    for (int i = 0; i < recyclerView.getChildCount(); i++) {
      final ThemedTextView item =
          recyclerView.getChildAt(i).findViewById(R.id.text);
  ...
  private void setupContentPortalView(View view) {
    ...
    if (arrowContainer != null) {
      arrowContainer.setOnTouchListener(
          new SwipeMotionDetector(getContext(), new OnSwipeListener() {
            @Override

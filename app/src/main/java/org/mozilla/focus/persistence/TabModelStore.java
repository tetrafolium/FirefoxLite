...
public class TabModelStore {
  ...
  public interface AsyncQueryListener {
    void onQueryComplete(List<SessionManager.SessionWithState> states,
                         String focusTabId);
  }

  public interface AsyncSaveListener { void onSaveComplete(); }
  ...
  public void getSavedTabs(@NonNull final Context context,
                           @Nullable final AsyncQueryListener listener) {
    new QueryTabsTask(context, tabsDatabase, listener)
  ...
  public void saveTabs(@NonNull final Context context,
    ...
    PreferenceManager.getDefaultSharedPreferences(context)
        .edit()
        .putString(
            context.getResources().getString(R.string.pref_key_focus_tab_id),
            focusTabId)
        .apply();

    new SaveTabsTask(context, tabsDatabase, listener)
        .executeOnExecutor(SERIAL_EXECUTOR,
                           sessionList.toArray(new Session[0]));
  }

  private static class QueryTabsTask
      extends AsyncTask<Void, Void, List<SessionManager.SessionWithState>> {

    private WeakReference<Context> contextRef;
    private TabsDatabase tabsDatabase;
    private WeakReference<AsyncQueryListener> listenerRef;

    public QueryTabsTask(Context context, TabsDatabase tabsDatabase,
    ...
    @Override
    protected List<SessionManager.SessionWithState>
    doInBackground(Void... voids) {
      ...
      if (context != null && tabsDatabase != null) {
        ...
        for (final TabEntity entity : tabEntityList) {
          Session session = new Session(entity.getId(), entity.getParentId(),
                                        entity.getUrl());
          // We seemed to have historically stored null into the DB. This patch
          // help guarantee Session.title will be non-null.
          final String title =
    ...
    private List<SessionManager.SessionWithState>
    restoreWebViewState(@NonNull Context context,
                        @NonNull List<Session> sessionList) {
      final List<SessionManager.SessionWithState> states = new ArrayList<>();
      File cacheDir =
          new File(context.getCacheDir(), TAB_WEB_VIEW_STATE_FOLDER_NAME);
      for (Session session : sessionList) {
        TabViewEngineSession es = new TabViewEngineSession();
        es.setWebViewState(
            FileUtils.readBundleFromStorage(cacheDir, session.getId()));
        ...
        String focusTabId =
            PreferenceManager.getDefaultSharedPreferences(context).getString(
                context.getResources().getString(
                    R.string.pref_key_focus_tab_id),
                "");
  ...
  private static class SaveTabsTask extends AsyncTask<Session, Void, Void> {
    ...
    public SaveTabsTask(Context context, TabsDatabase tabsDatabase,
                        AsyncSaveListener listener) {
      this.contextRef = new WeakReference<>(context);
      this.tabsDatabase = tabsDatabase;
      this.listenerRef = new WeakReference<>(listener);
    }
    ...
    protected Void doInBackground(Session... sessionList) {
      if (sessionList != null) {
        ...
        if (tabsDatabase != null) {
          ...
          tabsDatabase.tabDao().deleteAllTabsAndInsertTabsInTransaction(
              entities);
        }
      }
    ...
    private void saveWebViewState(@NonNull Context context,
                                  @NonNull Session[] sessionList) {
      final File cacheDir =
          new File(context.getCacheDir(), TAB_WEB_VIEW_STATE_FOLDER_NAME);
      ...
      if (cacheFiles != null) {
        List<File> outOfDateFileList =
            new ArrayList<>(Arrays.asList(cacheFiles));

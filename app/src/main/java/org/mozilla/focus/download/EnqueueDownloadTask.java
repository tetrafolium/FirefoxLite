 ...
 */
public class EnqueueDownloadTask
    extends AsyncTask<Void, Void, EnqueueDownloadTask.ErrorCode> {

  private static final String DOWNLOAD_MANAGER_PACKAGE_NAME =
  ...
  public EnqueueDownloadTask(@NonNull Activity activity,
                             @NonNull Download download,
                             @Nullable String refererUrl) {
  ...
  protected ErrorCode doInBackground(Void... params) {
    ...
    final String fileName =
        (download.getName() != null)
            ? download.getName()
            : URLUtil.guessFileName(download.getUrl(),
    ...
    final DownloadManager.Request request =
        new DownloadManager.Request(Uri.parse(download.getUrl()))
    ...
    // On Pixel, When downloading downloaded content which is still available,
    // DownloadManager returns the previous download id. For that case we remove
    // the old entry and re-insert a new one to move it to the top. (Note that
    // this is not the case for devices like Samsung, I have not verified yet if
    // this is a because of on those devices we move files to SDcard or if this
    // is true even if the file is not moved.)
    if (!DownloadInfoManager.getInstance().recordExists(downloadId)) {
      DownloadInfoManager.getInstance().insert(
          downloadInfo, new DownloadInfoManager.AsyncInsertListener() {
            @Override
      ...
      DownloadInfoManager.getInstance().queryByDownloadId(
          downloadId, new DownloadInfoManager.AsyncQueryListener() {
            @Override
            public void onQueryComplete(List downloadInfoList) {
              if (!downloadInfoList.isEmpty()) {
                ...
                DownloadInfoManager.getInstance().insert(
                    info, new DownloadInfoManager.AsyncInsertListener() {
                      @Override
                      public void onInsertComplete(long rowId) {
                        DownloadInfoManager.notifyRowUpdated(context, rowId);
                        RelocateService.broadcastRelocateFinished(context,
                                                                  rowId);
                      }
                    });
              }
  ...
  protected void onPostExecute(ErrorCode errorCode) {
    ...
    case FILE_NOT_SUPPORTED:
      Toast
          .makeText(activity, R.string.download_file_not_supported,
                    Toast.LENGTH_LONG)
          .show();
      break;
    case SUCCESS:
      if (!download.isStartFromContextMenu()) {
        Toast.makeText(activity, R.string.download_started, Toast.LENGTH_LONG)
            .show();
      }
      break;
    case GENERAL_ERROR:
  ...
  private boolean isDownloadManagerEnabled(final Context context) {
    ...
    try {
      state = context.getPackageManager().getApplicationEnabledSetting(
          DOWNLOAD_MANAGER_PACKAGE_NAME);
    } catch (IllegalArgumentException
                 e) { // throws when the named package does not exist
      e.printStackTrace();
    }

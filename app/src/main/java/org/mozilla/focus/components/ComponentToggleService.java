 ...
 * <p>
 * If the browser related packages list changed, it will clear default browser
 * config. Hence all this service doing is to enable then disable
 * ConfigActivity.
 */
public class ComponentToggleService extends Service {
  ...
  public static final IntentFilter SERVICE_STOP_INTENT_FILTER =
      new IntentFilter();
  public static final String SERVICE_STOP_ACTION =
      "_component_service_stopped_";

  static { SERVICE_STOP_INTENT_FILTER.addAction(SERVICE_STOP_ACTION); }
  ...
  public void onCreate() {
    super.onCreate();
    mPackageStatusReceiver =
        new PackageStatusReceiver(new PackageStatusReceiver.Listener() {
          @Override
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    final boolean hasDefaultBrowser =
        Browsers.hasDefaultBrowser(getApplicationContext());
    final boolean isDefaultBrowser =
        Browsers.isDefaultBrowser(getApplicationContext());
    final boolean configCleared = (!isDefaultBrowser && !hasDefaultBrowser);

    final PackageManager pkgMgr = getPackageManager();
    final ComponentName componentName =
        new ComponentName(getApplicationContext(), ConfigActivity.class);
    final boolean componentEnabled =
        (pkgMgr.getComponentEnabledSetting(componentName) ==
         PackageManager.COMPONENT_ENABLED_STATE_ENABLED);

    // if there is default-browser-setting and it is not me, let's enable
    // component to clear settings
    final boolean toStartJob =
        (!componentEnabled) && hasDefaultBrowser && !isDefaultBrowser;
  ...
  public static boolean isAlive(Context context) {
    ...
    for (ActivityManager.RunningServiceInfo service :
      ...
      if (ComponentToggleService.class.getName().equals(
  ...
  private void startToForeground() {
    ...
    final Notification notification =
        builder
            .setContentTitle(
                getString(R.string.setting_default_browser_notification_title))
            .setContentText(
                getString(R.string.setting_default_browser_notification_text))
            .setAutoCancel(false)
            .setOngoing(true)
  ...
  private void removeFromForeground() {
    ...
    // this notification will be removed by SettingsActivity if it is in
    // foreground
    final NotificationCompat.Builder builder =
        NotificationUtil.importantBuilder(getApplicationContext());
    ...
    final Notification notification =
        builder
            .setContentTitle(getString(
                R.string.setting_default_browser_notification_clickable_text))
            .setAutoCancel(true)
            .setContentIntent(buildIntent())
            .build();
  ...
  private PendingIntent buildIntent() {
    return PendingIntent.getActivity(
        getApplicationContext(), INTENT_REQ_CODE,
  ...
  private final static class PackageStatusReceiver extends BroadcastReceiver {
    ...
    PackageStatusReceiver(Listener listener) { mListener = listener; }

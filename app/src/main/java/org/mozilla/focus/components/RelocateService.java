...
public class RelocateService extends IntentService {
  ...
  private static final String ACTION_MOVE =
      "org.mozilla.focus.components.action.MOVE";
  private static final String CHANNEL_ID = "relocation_service";

  public RelocateService() { super(TAG); }
   ...
   * @see IntentService
   */
  public static void startActionMove(@NonNull Context context, long rowId,
                                     long downloadId, @NonNull File srcFile,
  ...
  private static void configForegroundChannel(Context context) {
    ...
    // NotificationChannel API is only available for Android O and above, so we
    // need to add the check here so IDE won't complain
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    ...
    if (intent != null) {
      ...
      if (ACTION_MOVE.equals(action)) {
        ...
        final long downloadId =
            intent.getLongExtra(Constants.EXTRA_DOWNLOAD_ID, -1);
        ...
        final File src =
            new File(intent.getStringExtra(Constants.EXTRA_FILE_PATH));
   ...
   * parameters.
   */
  private void handleActionMove(final long rowId, final long downloadId,
  ...
  /**
   * If removable storage exists, to move a file to it. Once moving completed,
   * also update database for latest file path.
   *
   * @param rowId      id of downloaded file in our database
   * @param downloadId downloadId of downloaded file, need this to update system
   *     database
   * @param srcFile    file to be moved
   * @param type       MIME type of the file, to decide sub directory
   */
  private void moveFile(final long rowId, final long downloadId,
                        final File srcFile, final String type) {
    final Settings settings = Settings.getInstance(getApplicationContext());
    File destFile = null;
    try {
      final File outputDir =
          StorageUtils.getTargetDirOnRemovableStorageForDownloads(this, type);
      if (outputDir != null) {
        ...
        if (!deleted) {
          throw new RuntimeException("Cannot delete original file: " +
        ...
        // we moved download file to removable-storage, now we should inform
        // user
        
        // avoid sending same message continuously
        if ((!settings.getRemovableStorageStateOnCreate()) && (settings.getShowedStorageMessage() !=
              Settings.STORAGE_MSG_TYPE_REMOVABLE_AVAILABLE)) {
          settings.setShowedStorageMessage(
              Settings.STORAGE_MSG_TYPE_REMOVABLE_AVAILABLE);
          final CharSequence msg =
      ...
      // avoid sending same message continuously
      if ((settings.getRemovableStorageStateOnCreate()) && (settings.getShowedStorageMessage() !=
            Settings.STORAGE_MSG_TYPE_REMOVABLE_UNAVAILABLE)) {
        settings.setShowedStorageMessage(
            Settings.STORAGE_MSG_TYPE_REMOVABLE_UNAVAILABLE);
        final CharSequence msg =
      ...
      try {
        if (((destFile != null) && destFile.exists() && destFile.canWrite() &&
          ...
          Log.w(TAG,
                "cannot delete copied file: " + destFile.getAbsolutePath());
        }
      } catch (Exception e2) {
        e2.printStackTrace();
      }
  ...
  private void broadcastNoPermission(final long downloadId,
    ...
    Log.d(
        TAG,
        "no permission for file relocating, send broadcast to grant permission");
  }
  ...
  public static void broadcastRelocateFinished(Context context, long rowId) {
    final Intent broadcastIntent =
        new Intent(Constants.ACTION_NOTIFY_RELOCATE_FINISH);
    broadcastIntent.addCategory(Constants.CATEGORY_FILE_OPERATION);

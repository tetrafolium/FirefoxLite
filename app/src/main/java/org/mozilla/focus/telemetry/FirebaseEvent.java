...
class FirebaseEvent {
  ...
  @VisibleForTesting static final int MAX_LENGTH_EVENT_NAME = 40;
  // method+object shouldn't longer than 20 characters so value part can have
  // enough space
  @VisibleForTesting static final int MAX_LENGTH_EVENT_NAME_PREFIX = 20;
  static final String EVENT_NAME_SEPARATOR = "__";

  // limitation to param
  // You can associate up to 25 unique Params with each Event type. Param names
  // can be up to 40 characters long, may only contain alphanumeric characters
  // and underscores ("_"), and must start with an alphabetic character. Param
  // values can be up to 100 characters long. The "firebase_", "google_" and
  // "ga_" prefixes are reserved and should not be used. see:
  ...
  FirebaseEvent(@NonNull String category, @NonNull String method,
                @Nullable String object, @Nullable String value) {
    ...
    final String eventNamePrefix =
        method + EVENT_NAME_SEPARATOR + object + EVENT_NAME_SEPARATOR;
    this.eventName = eventNamePrefix + value;

    final int prefixLength = eventNamePrefix.length();

    // TODO: check eventName should start with[a-zA-Z] , contains only
    // [a-zA-A0-9_], and shouldn't
    ...
    if (this.eventName.length() > MAX_LENGTH_EVENT_NAME) {
      ...
      if (prefixLength > MAX_LENGTH_EVENT_NAME_PREFIX) {
        LoggerWrapper.throwOrWarn(
            TAG, "Event[" + this.eventName + "]'s prefixLength too long  " +
                     prefixLength + " of " + MAX_LENGTH_EVENT_NAME_PREFIX);
      }

      LoggerWrapper.throwOrWarn(
          TAG, "Event[" + this.eventName +
                   "] exceeds Firebase event name limit " +
                   this.eventName.length() + " of " + MAX_LENGTH_EVENT_NAME);

      // fix the value if we just want to warn
      if (value != null) {
        int acceptValueLength =
            MAX_LENGTH_EVENT_NAME - eventNamePrefix.length();
        int valueLength = value.length();
        this.eventName =
            eventNamePrefix +
            value.substring(valueLength - acceptValueLength, valueLength);
      }
    }
  }

  @CheckResult
  public static FirebaseEvent create(@NonNull String category,
                                     @NonNull String method,
                                     @Nullable String object, String value) {
  ...
  public FirebaseEvent param(String name, String value) {
    ...
    if (this.eventParam.size() >= MAX_PARAM_SIZE) {
      LoggerWrapper.throwOrWarn(TAG, "Firebase event[" + eventName +
  ...
  // TODO: check param name should start with[a-zA-Z] , contains only
  // [a-zA-A0-9_], and shouldn't
  // TODO: start with ^(?!(firebase_|google_|ga_)).*
  private static String safeParamLength(@NonNull final String str,
                                        final int end) {
    // validate the length
    if (str.length() > end) {
      LoggerWrapper.throwOrWarn(
          TAG, "Exceeding limit of param content length:" + str.length() +
                   " of " + end);
    }
    // fix the value if we just want to warn
  ...
  /*** Queue the events and let Firebase Analytics to decide when to upload to
   * server
   *
  ...
  public void event(Context context) {
    ...
    if (TelemetryWrapper.isTelemetryEnabled(context)) {
      FirebaseHelper.getFirebase().event(context.getApplicationContext(),
                                         this.eventName, this.eventParam);
    }
  }
  ...
  static boolean isInitialized() { return prefKeyWhitelist.isEmpty(); }

...
public class ScreenshotDatabaseHelper {
  ...
  public interface Tables { String SCREENSHOT = "screenshot"; }
  ...
  private static final class OpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
      ...
      db.execSQL(CREATE_TABLE_IF_NOT_EXISTS + Tables.SCREENSHOT + " (" +
                 ...
                 Screenshot.TITLE + " TEXT," + Screenshot.URL +
                 " TEXT NOT NULL," + Screenshot.TIMESTAMP +
                 " INTEGER NOT NULL," + Screenshot.IMAGE_URI + " TEXT NOT NULL"
                 + ");");
      ...
      db.execSQL(CREATE_TRIGGER_IF_NOT_EXISTS + Tables.SCREENSHOT + "_inserted "
                 ...
                 " WHEN (SELECT count() FROM " + Tables.SCREENSHOT + ") > " +
                 SCREENSHOT_LIMIT + " BEGIN "
                 + "   DELETE FROM " + Tables.SCREENSHOT + "     WHERE " +
                 Screenshot._ID + " = "
                 + "(SELECT " + Screenshot._ID + " FROM " + Tables.SCREENSHOT +
                 " ORDER BY " + Screenshot.TIMESTAMP + " LIMIT 1);"
                 + " END");
    }
  ...
  public static synchronized ScreenshotDatabaseHelper
  getsInstacne(Context context) {
    if (sInstacne == null) {

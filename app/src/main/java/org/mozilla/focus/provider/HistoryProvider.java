...
public class HistoryProvider extends ContentProvider {
  ...
  static {
    sUriMatcher.addURI(HistoryContract.AUTHORITY, "browsing_history",
                       BROWSING_HISTORY);
  }
  ...
  public Cursor query(@NonNull Uri uri, String[] projection, String selection,
    ...
    SupportSQLiteQuery query =
        SupportSQLiteQueryBuilder.builder(Tables.BROWSING_HISTORY)
            .columns(projection)
            .selection(selection, selectionArgs)
            .orderBy(sortOrder)
            .limit(ProviderUtils.getLimitParam(uri.getQueryParameter("offset"),
  ...
  public int update(Uri uri, ContentValues values, String selection,
    ...
    final SupportSQLiteDatabase db = mDbHelper.getWritableDatabase();
    final int count =
        db.update(Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK, values,
  ...
  private long insertWithUrlUnique(SupportSQLiteDatabase db,
    ...
    try {
      SupportSQLiteQuery query =
          SupportSQLiteQueryBuilder.builder(Tables.BROWSING_HISTORY)
              .columns(null)
              .selection(BrowsingHistory.URL + " = ?",
                         new String[] {values.getAsString(BrowsingHistory.URL)})
              .groupBy(null)
              .having(null)
              .orderBy(null)
              .create();
      ...
      if (c != null) {
        if (c.moveToFirst()) {
          ...
          values.put(BrowsingHistory.VIEW_COUNT,
                     c.getLong((c.getColumnIndex(BrowsingHistory.VIEW_COUNT))) +
                         1);
          if (db.update(Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK,
                        values, BrowsingHistory._ID + " = ?",
                        new String[] {Long.toString(id)}) == 0) {
            id = -1;
          }
        } else {
          values.put(BrowsingHistory.VIEW_COUNT, 1);
          id = db.insert(Tables.BROWSING_HISTORY, OnConflictStrategy.ROLLBACK,
                         values);
        }
      }
      return id;
  ...
  private void notifyBrowsingHistoryChange() {
    getContext().getContentResolver().notifyChange(BrowsingHistory.CONTENT_URI,
                                                   null);
  }
}

...
public class TopSitesUtils {
  ...
  public static final String TOP_SITE_ASSET_PREFIX =
      "file:///android_asset/topsites/icon/";
    ...
    try {
      ...
      for (int i = 0; i < obj.length(); i++) {
        ((JSONObject)obj.get(i))
            .put("lastViewTimestamp", lastViewTimestampSystem);
      }
  ...
  public static String loadDefaultSitesFromAssets(Context context,
                                                  @RawRes int resId) {
    String json = "[]";
  ...
  public static List<Site> paresJsonToList(Context context,
                                           JSONArray jsonArray) {
      ...
      if (jsonArray != null) {
        for (int i = 0; i < jsonArray.length(); i++) {
          ...
          final String faviconUri =
              TOP_SITE_ASSET_PREFIX + json_site.getString(KEY_FAVICON);
          Site site =
              new Site(id, title, url, viewCount, lastViewed, faviconUri);

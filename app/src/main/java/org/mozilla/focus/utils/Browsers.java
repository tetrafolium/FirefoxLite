...
public class Browsers {
  public enum KnownBrowser {
    ...
    KnownBrowser(String packageName) { this.packageName = packageName; }
  }

  private Context appContext;
  private final Map<String, ActivityInfo> browsers;
  private final ActivityInfo defaultBrowser;
  // This will contain installed firefox branded browser ordered by priority
  // from Firefox, Firefox_Beta, Firefox Aurora and Firefox_Nightly. If multiple
  // firefox branded browser is installed then higher priority one will be
  // stored here
  ...
  public Browsers(Context context, @NonNull String url) {
    ...
    final Map<String, ActivityInfo> browsers =
        resolveBrowsers(packageManager, uri);

    // If there's a default browser set then modern Android systems won't return
    // other browsers anymore when using queryIntentActivities(). That's
    // annoying and our only option is to go through a list of known browsers
    // and see if anyone of them is installed and wants to handle our URL.
  ...
  private Map<String, ActivityInfo>
  resolveBrowsers(PackageManager packageManager, @NonNull Uri uri) {
  ...
  private void findKnownBrowsers(PackageManager packageManager,
                                 Map<String, ActivityInfo> browsers,
                                 @NonNull Uri uri) {
    for (final KnownBrowser browser : KnownBrowser.values()) {
      ...
      // resolveActivity() can be slow if the package isn't installed (e.g.
      // 200ms on an N6 with a bad WiFi connection). Hence we query if the
      // package is installed first, and only call resolveActivity for installed
      // packages. getPackageInfo() is fast regardless of a package being
      // installed
      try {
        // We don't need the result, we only need to detect when the package
        // doesn't exist
  ...
  private ActivityInfo findDefault(PackageManager packageManager,
    ...
    if (!browsers.containsKey(resolveInfo.activityInfo.packageName)) {
      // This default browser wasn't returned when we asked for *all* browsers.
      // It's likely that this is actually the resolver activity (aka intent
      // chooser). Let's ignore it.
      return null;
    }
  ...
  /**
   * Does this user have a default browser that is not Firefox (release) or
   * Focus (this build).
   */
  public boolean hasThirdPartyDefaultBrowser(Context context) {
    return defaultBrowser != null &&
        ...
        !(firefoxBrandedBrowser != null &&
          defaultBrowser.packageName.equals(
              firefoxBrandedBrowser.packageName)) &&
        !defaultBrowser.packageName.equals(context.getPackageName());
  }

  public @Nullable ActivityInfo getDefaultBrowser() { return defaultBrowser; }

  /**
   * Does this user have browsers installed that are not Focus, Firefox or the
   * default browser?
   */
  public boolean hasMultipleThirdPartyBrowsers(Context context) {
    ...
    for (ActivityInfo info : browsers.values()) {
      if (info != defaultBrowser &&
        ...
        // There's at least one browser that is not Focus or Firefox and also

...
public class DimenUtils {
  ...
  /**
   * @deprecated We're giving up this methodology. The only left use case is Tab
   *     Tray which should
   * be migrated in the future too. Please consider {@link
   * #iconTooBlurry(Resources, int)} as a replacement.
   */
  @Deprecated
  @FavIconType
  public static int getFavIconType(Resources res, Bitmap source) {
    if (source == null ||
        source.getWidth() <
            res.getDimensionPixelSize(R.dimen.favicon_initial_threshold_size)) {
      return TYPE_GENERATED;
    }

    if (source.getWidth() >
        res.getDimensionPixelSize(R.dimen.favicon_downscale_threshold_size)) {
   ...
   */
  @Deprecated
  public static Bitmap getRefinedBitmap(Resources res, Bitmap source,
                                        char initial) {
  ...
  public static boolean iconTooBlurry(Resources resources, int bitmapWidth) {
    return bitmapWidth <
        resources.getDimensionPixelSize(R.dimen.favicon_initial_threshold_size);
  }
  ...
  public static Bitmap getInitialBitmap(Resources resources, Bitmap source,
                                        char initial) {
    return FavIconUtils.getInitialBitmap(
        source, initial, getDefaultFaviconTextSize(resources),
        getDefaultFaviconBitmapSize(resources));
  }

  public static Bitmap getInitialBitmap(Resources resources, char initial,
                                        int backgroundColor) {
    return FavIconUtils.getInitialBitmap(
        initial, backgroundColor, getDefaultFaviconTextSize(resources),
        getDefaultFaviconBitmapSize(resources));
  }

  static Bitmap getRefinedShortcutIcon(Resources res, Bitmap source,
                                       char initial) {
    final int sizeThreshold =
        res.getDimensionPixelSize(R.dimen.shortcut_icon_size);

    if (source == null || source.getWidth() < sizeThreshold) {
      return DimenUtils.getInitialBitmap(res, source, initial);
    }
    if (source.getWidth() > sizeThreshold) {
      return Bitmap.createScaledBitmap(source, sizeThreshold, sizeThreshold,
                                       false);
    } else {
      return source;
    }

...
public class FilePickerUtil {
  ...
  /* Gets an intent that can open a particular mimetype. Will show a prompt with
   * a list of Activities that can handle the mietype. Asynchronously calls the
   * handler when one of the intents is selected. If the caller passes in null
   * for the handler, will still prompt for the activity, but will throw away
   * the result.
   */
  public static Intent getFilePickerIntent(final Context context,
  ...
  public static List<Intent> getIntentsForFilePicker(final Context context,
    ...
    // The base intent to use for the file picker. Even if this is an implicit
    // intent, Android will still show a list of Activities that match this
    // action/type.
    Intent baseIntent;

    // A HashMap of Activities the base intent will show in the chooser. This is
    // used to filter activities from other intents so that we don't show
    // duplicates.
    HashMap<String, Intent> baseIntents = new HashMap<String, Intent>();

    // A list of other activities to show in the picker (and the intents to
    // launch them).
    HashMap<String, Intent> intents = new HashMap<String, Intent>();

    String mimeType = (mimeTypes == null) ? "*/*" : mimeTypes[0];
    if (("audio/*".equals(mimeType)) || ("image/*".equals(mimeType)) ||
   ...
   *
   * @param context
   * @param targetIntent to find out how many activities could perform this
   *     intent

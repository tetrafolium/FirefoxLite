...
public class StorageUtils {
  ...
  /**
   * Test if we have a removable storage and throw Exception if no is available
   * and specify cause. Note that exception is thrown even if the user does not
   * want to save to removable storage.
   *
   * @param ctx Context
   * @return app-owned-directory on removable storage
   * @throws NoRemovableStorageException if there is no removable storage to
   *     use.
   */
  public static File getAppMediaDirOnRemovableStorage(@NonNull Context ctx)
    ...
    if (!Environment.MEDIA_MOUNTED.equals(
            Environment.getExternalStorageState(media))) {
      throw new NoRemovableStorageException(
          "No mounted-removable media to use");
    }
  ...
  /**
   * To get a directory to save downloaded file. Before invoke this method,
   * callee should check we already granted necessary permission.
   *
   * @param ctx  Context
   * @param type should be Download.TYPE_IMAGE, Download.TYPE_OTHER
   * @return a directory on removable storage to save files. return null if
   *     user's preference is off.
   * @throws NoRemovableStorageException if user's preference is on, but there
   *     is no removable storage to use.
   */
  public static File
  getTargetDirOnRemovableStorageForDownloads(@NonNull Context ctx, String type)
      throws NoRemovableStorageException {
  ...
  static File getFirstRemovableMedia(@NonNull Context ctx) {
    ...
    for (final File file : files) {
      try {
        ...
        // and throws an IllegalArgumentException if the target path has any
        // problem
        e.printStackTrace();
      }
    }

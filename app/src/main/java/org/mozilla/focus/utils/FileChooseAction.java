...
public class FileChooseAction {
  ...
  public void cancel() { this.callback.onReceiveValue(null); }
  ...
  public void startChooserActivity() {
    hostFragment.startActivityForResult(createChooserIntent(this.params),
  ...
  private Intent createChooserIntent(WebChromeClient.FileChooserParams params) {
    ...
    title = TextUtils.isEmpty(title)
                ? hostFragment.getString(R.string.file_picker_title)
                : title;

    return FilePickerUtil.getFilePickerIntent(hostFragment.getActivity(), title,
                                              mimeTypes);
  }
}

...
public final class BookmarkRepository {
  ...
  public static BookmarkRepository
  getInstance(final BookmarksDatabase database) {
    if (instance == null) {
  ...
  public String addBookmark(String title, String url) {
    final BookmarkModel bookmark =
        new BookmarkModel(UUID.randomUUID().toString(), title, url);
  ...
  public void updateBookmark(BookmarkModel bookmark) {
    ThreadUtils.postToBackgroundThread(
        () -> bookmarksDatabase.bookmarkDao().updateBookmark(bookmark));
  }
  ...
  public void deleteBookmarksByUrl(String url) {
    ThreadUtils.postToBackgroundThread(
        () -> bookmarksDatabase.bookmarkDao().deleteBookmarksByUrl(url));
  }
}

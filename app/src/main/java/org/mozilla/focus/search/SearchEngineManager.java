...
public final class SearchEngineManager extends BroadcastReceiver {
  ...
  public static SearchEngineManager getInstance() { return instance; }
  ...
  private synchronized void loadFromDisk(Context context) {
    ...
    try {
      ...
      final List<String> localeEngines =
          Arrays.asList(assetManager.list(localePath));
      final List<String> languageEngines =
          Arrays.asList(assetManager.list(languagePath));
      final List<String> defaultEngines =
          Arrays.asList(assetManager.list(defaultPath));
      ...
      for (int i = 0; i < engineNames.length(); i++) {
        ...
        if (localeEngines.contains(fileName)) {
          searchEngines.add(SearchEngineParser.load(
              assetManager, engineName, localePath + "/" + fileName));
        } else if (languageEngines.contains(fileName)) {
          searchEngines.add(SearchEngineParser.load(
              assetManager, engineName, languagePath + "/" + fileName));
        } else if (defaultEngines.contains(fileName)) {
          searchEngines.add(SearchEngineParser.load(
              assetManager, engineName, defaultPath + "/" + fileName));
        } else {
          Log.e(LOG_TAG,
                "Couldn't find configuration for engine: " + engineName);
        }
      }
    } catch (IOException e) {
  ...
  private JSONArray loadSearchEngineListForLocale(Context context)
      throws IOException {
    try {
      final Locale locale = Locale.getDefault();
      final JSONObject configuration =
          IOUtils.readAsset(context, "search/search_configuration.json");
      ...
      // No configuration for the current locale found. Let's use the default
      // configuration.
      return configuration.getJSONArray("default");
    } catch (JSONException e) {
      // Assertion error because this shouldn't happen: We check whether a key
      // exists before reading it. An error here would mean the JSON file is
      // corrupt.
  ...
  public synchronized SearchEngine getDefaultSearchEngine(Context context) {
    ...
    final String defaultSearch =
        Settings.getInstance(context).getDefaultSearchEngineName();
  ...
  // Our (searchEngines == null) check is deemed to be an unsynchronised access.
  // Similarly loadHasBeenTriggered also doesn't need synchronisation:
  @SuppressFBWarnings(
      value = "IS2_INCONSISTENT_SYNC",
      justification =
          "Variable is not being accessed, it is merely being tested for existence")
  public void
  awaitLoadingSearchEnginesLocked() {
    if (!loadHasBeenTriggered) {
      throw new IllegalStateException(
          "Attempting to retrieve search engines without a corresponding init()");
    }

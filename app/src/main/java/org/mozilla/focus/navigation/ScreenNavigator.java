...
/**
 * Provide a simple and clear interface to navigate between home/browser/url
 * fragments. This class only manages the relation between fragments, and the
 * detail of transaction was handled by TransactionHelper
 */
public class ScreenNavigator implements DefaultLifecycleObserver {
      ...
      new FragmentManager.FragmentLifecycleCallbacks() {
        ...
        public void onFragmentStarted(FragmentManager fm, Fragment f) {
          ...
          if (f instanceof UrlInputScreen) {
            ScreenNavigator.this.transactionHelper.onUrlInputScreenVisible(
                true);
          }
        }
  ...
  public static ScreenNavigator get(Context context) {
    ...
    if (BuildConfig.DEBUG) {
      throw new RuntimeException(
          "the given context should implement ScreenNavigator.Provider");
    } else {
   ...
   * 1. you just want to move the old existing browser fragment to the
   * foreground
   * 2. you've called Session#loadUrl() by yourself, and want to move that tab
   * to the foreground.
   */
  ...
  /**
   * Load target url on current/new tab and clear every thing above browser
   * fragment
   *
   * @param url            target url
   * @param withNewTab     whether to open and load target url in a new tab
   * @param isFromExternal if this url is started from external VIEW intent, if
   *     true, the app will finish when the user click back key
   */
  public void showBrowserScreen(String url, boolean withNewTab,
                                boolean isFromExternal) {
    logMethod(url, withNewTab);

    getBrowserScreen().loadUrl(url, withNewTab, isFromExternal,
   ...
   */
  // TODO: Make geo dialog a view in browser fragment, so we can remove this
  // method
  ...
  /**
   * Add a home fragment to back stack, so user can press back key to go back to
   * previous fragment. Use this when you want to start a new tab.
   */
  public void addHomeScreen(boolean animate) {
    logMethod();

    boolean found = this.transactionHelper.popScreensUntil(
        HOME_FRAGMENT_TAG, TransactionHelper.EntryData.TYPE_ATTACHED, false);
    log("found exist home: " + found);
    if (!found) {
      this.transactionHelper.showHomeScreen(
          animate, TransactionHelper.EntryData.TYPE_ATTACHED, false);
    }
    this.transactionHelper.executePendingTransaction();
  }

  /**
   * Clear every thing and show a home fragment. Use this when there's no tab
  ...
  public void popToHomeScreen(boolean animate) {
    ...
    boolean found = this.transactionHelper.popScreensUntil(
        HOME_FRAGMENT_TAG, TransactionHelper.EntryData.TYPE_ROOT, false);
    log("found exist home: " + found);
    if (!found) {
      this.transactionHelper.showHomeScreen(
          animate, TransactionHelper.EntryData.TYPE_ROOT, false);
    }
    this.transactionHelper.executePendingTransaction();
  }
  ...
  public LiveData<NavigationState> getNavigationState() {
    return Transformations.map(transactionHelper.getTopFragmentState(),
                               fragmentTag
                               -> new NavigationState(fragmentTag.isEmpty()
  ...
  public interface Provider { ScreenNavigator getScreenNavigator(); }
  ...
  public interface HostActivity extends LifecycleOwner {
    ...
    UrlInputScreen createUrlInputScreen(@Nullable String url,
   ...
   * Contract class for ScreenNavigator
   */
  public interface Screen { Fragment getFragment(); }
  ...
  public interface BrowserScreen extends Screen, BackKeyHandleable {
    void loadUrl(@NonNull final String url, boolean openNewTab,
  ...
  public static class NavigationState {
    ...
    NavigationState(String tag) { this.tag = tag; }

    public String getTag() { return this.tag; }

    public boolean isHome() { return this.tag.equals(HOME_FRAGMENT_TAG); }

    public boolean isBrowser() { return this.tag.equals(BROWSER_FRAGMENT_TAG); }

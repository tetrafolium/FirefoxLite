...
class TransactionHelper implements DefaultLifecycleObserver {
  ...
  void showUrlInput(@Nullable String url, String sourceFragment) {
    ...
    final Fragment existingFragment =
        fragmentManager.findFragmentByTag(URL_INPUT_FRAGMENT_TAG);
    if (existingFragment != null && existingFragment.isAdded() &&
        !existingFragment.isRemoving()) {
      // We are already showing an URL input fragment. This might have been a
      // double click on the fake URL bar. Just ignore it.
      return;
    }

    this.prepareUrlInput(url, sourceFragment)
        .addToBackStack(
            makeEntryTag(URL_INPUT_FRAGMENT_TAG, EntryData.TYPE_FLOATING))
        .commit();
  }
  ...
  boolean shouldFinish() {
    ...
    FragmentManager.BackStackEntry lastEntry =
        manager.getBackStackEntryAt(entryCount - 1);
  ...
  LiveData<String> getTopFragmentState() { return topFragmentState; }
  ...
  private FragmentTransaction prepareFirstRun() {
    ...
    if (fragmentManager.findFragmentByTag(FIRST_RUN_FRAGMENT_TAG) == null) {
      transaction
          .replace(R.id.container, screen.getFragment(), FIRST_RUN_FRAGMENT_TAG)
          .addToBackStack(
              makeEntryTag(FIRST_RUN_FRAGMENT_TAG, EntryData.TYPE_ROOT));
    }
  ...
  private FragmentTransaction prepareHomeScreen(boolean animated,
    ...
    int exitAnim =
        (type == EntryData.TYPE_ROOT) ? 0 : R.anim.tab_transition_fade_out;
    transaction.setCustomAnimations(enterAnim, 0, 0, exitAnim);

    transaction.add(R.id.container, homeScreen.getFragment(),
                    HOME_FRAGMENT_TAG);
  ...
  private FragmentTransaction prepareUrlInput(@Nullable String url,
    ...
    final FragmentManager fragmentManager =
        this.activity.getSupportFragmentManager();
    final ScreenNavigator.Screen urlScreen =
        this.activity.createUrlInputScreen(url, parentFragmentTag);
    FragmentTransaction transaction = fragmentManager.beginTransaction();
    transaction.add(R.id.container, urlScreen.getFragment(),
                    URL_INPUT_FRAGMENT_TAG);
    return transaction;
  }
  ...
  void onUrlInputScreenVisible(boolean visible) {
    ...
    final ScreenNavigator.Screen homeFragment =
        (ScreenNavigator.Screen)fragmentManager.findFragmentByTag(
            HOME_FRAGMENT_TAG);
  ...
  private static class BackStackListener
      implements FragmentManager.OnBackStackChangedListener {
    private Runnable stateRunnable;
    private TransactionHelper helper;

    BackStackListener(TransactionHelper helper) { this.helper = helper; }
    ...
    public void onBackStackChanged() {
      ...
      FragmentManager manager =
          this.helper.activity.getSupportFragmentManager();
    ...
    private boolean
    shouldKeepBrowserRunning(@NonNull TransactionHelper helper) {
    ...
    private void setBrowserState(boolean isForeground,
                                 @NonNull TransactionHelper helper) {
      this.stateRunnable = () -> setBrowserForegroundState(isForeground);

      FragmentAnimationAccessor actor =
          getTopAnimationAccessibleFragment(helper);
      Animation anim;
      if (actor == null ||
    ...
    private void setBrowserForegroundState(boolean isForeground) {
      ...
      FragmentManager manager =
          this.helper.activity.getSupportFragmentManager();
      BrowserScreen browserFragment =
          (BrowserScreen)manager.findFragmentById(R.id.browser);
      if (isForeground) {
    ...
    @Nullable
    FragmentAnimationAccessor
    getTopAnimationAccessibleFragment(@NonNull TransactionHelper helper) {
  ...
  static class EntryData {
    /**
     * argument passed to {@link FragmentTransaction#addToBackStack(String)},
     * pressing back when this type of fragment is in foreground will close the
     * app
     */
    ...
    /**
     * argument passed to {@link FragmentTransaction#addToBackStack(String)},
     * adding fragment of this type will make browser fragment go to background
     */
    static final int TYPE_ATTACHED = 1;

    /**
     * argument passed to {@link FragmentTransaction#addToBackStack(String)},
     * browsing fragment will still be in foreground after adding this type of
     * fragment.
     */

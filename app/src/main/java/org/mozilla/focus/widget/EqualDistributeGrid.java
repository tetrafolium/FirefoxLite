...
/**
 * A just-work custom ViewGroup for UI requirement. It *ASSUME* each children
 * has the same width and height, then to layout children in horizontal
 * orientation by inserting equal spacing between each children. If children
 * number is more than row-capacity(specified in XML), rest children will move
 * to next row, and align previous row.
 * <p>
 * ie: put 12 cells in this ViewGroup with row-capacity = 5, it will look like
 * this [C C C C C] [C C C C C] [C C      ]
 */
public class EqualDistributeGrid extends ViewGroup {
  ...
  public EqualDistributeGrid(Context context) { super(context); }

  public EqualDistributeGrid(Context context, AttributeSet attrs) {
    super(context, attrs);
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.EqualDistributeGrid);
    rowCapacity = a.getInt(R.styleable.EqualDistributeGrid_horizontalCapacity,
                           Integer.MAX_VALUE);
    gutter = a.getDimensionPixelSize(
        R.styleable.EqualDistributeGrid_verticalGutter, 0);
    a.recycle();
  }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int width = calculateWidth(widthMeasureSpec, heightMeasureSpec, cellCount,
                               rowLength, reference);
    int height = calculateHeight(widthMeasureSpec, heightMeasureSpec, cellCount,
                                 rowLength, reference);
    setMeasuredDimension(width, height);
  }

  private int calculateWidth(int widthMeasureSpec, int heightMeasureSpec,
                             int cellCount, int rowLength,
  ...
  private int calculateHeight(int widthMeasureSpec, int heightMeasureSpec,
    ...
    int height = (reference == null || cellCount == 0)
                     ? 0
                     : cellsOccupied + (lines - 1) * gutter + getPaddingTop() +
                           getPaddingBottom();
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    int spacing =
        (rowLength == 1)
            ? 0
            : ((r - l) - (cellWidth * rowLength) - paddingStart - paddingEnd) /
                  (rowLength - 1);
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      child.layout(locationX, locationY, locationX + cellWidth,
                   locationY + cellHeight);
      index++;
    }
  }
  ...
  public void setGutter(int gutter) { this.gutter = gutter; }
}

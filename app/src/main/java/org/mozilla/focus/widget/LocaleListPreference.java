...
public class LocaleListPreference extends ListPreference {
  ...
  static {
    // Only ICU 57 actually contains the Asturian name for Asturian, even
    // Android 7.1 is still shipping with ICU 56, so we need to override the
    // Asturian name (otherwise displayName will be the current locales version
    // of Asturian, see:
    // https://github.com/mozilla-mobile/focus-android/issues/634#issuecomment-303886118
    languageCodeToNameMap.put("ast", "Asturianu");
    // On an Android 8.0 device those languages are not known and we need to add
    // the names manually. Loading the resources at runtime works without
    // problems though.
    ...
    languageCodeToNameMap.put("ta", "தமிழ்");
    languageCodeToNameMap.put("kn", "ಕನ್ನಡ");
    languageCodeToNameMap.put("ml", "മലയാളം");
  }

  private volatile Locale entriesLocale;
  private CharacterValidator characterValidator;

  public LocaleListPreference(Context context) { this(context, null); }
  ...
  private static final class LocaleDescriptor
    ...
    public LocaleDescriptor(Locale locale, String tag) {
      ...
      final byte directionality =
          Character.getDirectionality(displayName.charAt(0));
    ...
    public String getDisplayName() { return this.nativeName; }
    ...
    public boolean isUsable(CharacterValidator validator) {
      ...
      return (this.tag.equals("or") || this.tag.equals("my") ||
          this.tag.equals("pa-IN") || this.tag.equals("gu-IN") ||
          this.tag.equals("bn-IN")) && (validator.characterIsMissingInFont(
                this.nativeName.substring(0, 1)));
  ...
  private LocaleDescriptor[] getUsableLocales() {
    final Collection<String> shippingLocales =
        LocaleManager.getPackagedLocaleTags(getContext());
    ...
    final LocaleDescriptor[] descriptors =
        locales.toArray(new LocaleDescriptor[usableCount]);
  ...
  private void buildList() {
    ...
    entries[0] =
        getContext().getString(R.string.preference_language_systemdefault);
    values[0] = "";

...
public class DefaultBrowserPreference extends Preference {
  ...
  public final static String EXTRA_RESOLVE_BROWSER =
      "_intent_to_resolve_browser_";
  ...
  public void onFragmentPause() { action.onFragmentPause(); }
  ...
  private void openAppDetailSettings(Context context) {
    ...
    //  fromParts might be faster than parse: ex.
  ...
  private void clearDefaultBrowser(Context context) {
    Intent intent = new Intent();
    intent.setComponent(
        new ComponentName(context, ComponentToggleService.class));
    context.startService(intent);
  }

  private void openSumoPage(Context context) {
    final Intent intent = InfoActivity.getIntentFor(
        context, SupportUtils.getSumoURLForTopic(context, "rocket-default"),
        getTitle().toString());
    context.startActivity(intent);
  }
  ...
  private void triggerWebOpen() {
    ...
    //  Put a mojo to force MainActivity finish it's self, we probably need an
    //  intent flag to handle the task problem (reorder/parent/top)
    ...
    public void onFragmentResume() {
      LocalBroadcastManager.getInstance(pref.getContext())
          .registerReceiver(this.receiver,
                            ComponentToggleService.SERVICE_STOP_INTENT_FILTER);
    }
  ...
  private static class ServiceReceiver extends BroadcastReceiver {
    ...
    ServiceReceiver(DefaultBrowserPreference pref) { this.pref = pref; }
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      // SettingsActivity is in foreground(because this BroadcastReceiver is
      // working), to remove notification which created by Service
      NotificationManagerCompat.from(context).cancel(
          ComponentToggleService.NOTIFICATION_ID);

      final boolean isDefaultBrowser = Browsers.isDefaultBrowser(context);
      final boolean hasDefaultBrowser = Browsers.hasDefaultBrowser(context);

      // The default-browser-config should be cleared, if the service finished
      // its job. if not been cleared, we regards it as 'fail'
      if (hasDefaultBrowser && !isDefaultBrowser) {
        TelemetryWrapper.onDefaultBrowserServiceFailed();
      }

      // if service finished its job, lets fire an intent to choose myself as
      // default browser

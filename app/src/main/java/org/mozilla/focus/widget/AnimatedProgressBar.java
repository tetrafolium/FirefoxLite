 ...
 * When changing progress of this bar, it does not change value directly.
 * Instead, it use
 * {@link Animator} to change value progressively. Moreover, change visibility
 * to View.GONE will cause closing animation.
 */
public class AnimatedProgressBar extends ProgressBar {
  ...
  /**
   * For closing animation. To indicate how many visible region should be
   * clipped.
   */
  ...
  /**
   * setProgress() might be invoked in constructor. Add to flag to avoid null
   * checking for animators.
   */
  ...
  private String endLoadingString() {
    return getContext().getString(
        R.string.accessibility_announcement_loading_finished);
  }

  /**
   * {@inheritDoc}
   * <p>
   * Instead of set progress directly, this method triggers an animator to
   * change progress.
   */
  @Override
  ...
  public void onDraw(Canvas canvas) {
    if (mClipRatio == 0) {
      ...
      canvas.save();
      if (mIsRtl) {
        canvas.clipRect(mRect.left, mRect.top, mRect.right - clipWidth,
                        mRect.bottom);
      } else {
        canvas.clipRect(mRect.left + clipWidth, mRect.top, mRect.right,
                        mRect.bottom);
      }
      super.onDraw(canvas);
      canvas.restore();
   ...
   * {@inheritDoc}
   * <p>
   * Instead of change visibility directly, this method also applies the closing
   * animation if progress reaches max value.
   */
  @Override
  ...
  public void onAttachedToWindow() {
    ...
    mIsRtl = (ViewCompat.getLayoutDirection(this) ==
  ...
  private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    final int duration =
        a.getInteger(R.styleable.AnimatedProgressBar_shiftDuration, 1000);
    final boolean wrap =
        a.getBoolean(R.styleable.AnimatedProgressBar_wrapShiftDrawable, false);
    @InterpolatorRes
    final int itplId =
        a.getResourceId(R.styleable.AnimatedProgressBar_shiftInterpolator,
                        INTEPOLATOR_NOT_EXIST);
    a.recycle();

    final Drawable progressDrawable =
        buildDrawable(getProgressDrawable(), wrap, duration, itplId);
    ...
    mClosingAnimator.addUpdateListener(
        new ValueAnimator.AnimatorUpdateListener() {
          @Override
  ...
  private Drawable buildDrawable(@NonNull Drawable original, boolean isWrap,
                                 int duration, @InterpolatorRes int itplId) {
    if (isWrap) {
      final Interpolator interpolator =
          isValidInterpolator(itplId)
  ...
  private static ValueAnimator
  createAnimator(int max, ValueAnimator.AnimatorUpdateListener listener) {

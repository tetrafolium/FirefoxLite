...
/**
 * A CoordinatorLayout implementation that resizes dynamically (by adding
 * padding to the bottom) based on whether a keyboard is visible or not. <p>
 ...
 * <p>
 * An optional viewToHideWhenActivated can be set: this is a View that will be
 * hidden when the keyboard is showing. That can be useful for things like FABs
 * that you don't need when someone is typing.
 */
public class ResizableKeyboardLayout extends CoordinatorLayout {

  private final int idOfViewToHide;

  @Nullable private View viewToHide;
  private int marginBottom;

  public ResizableKeyboardLayout(Context context) { this(context, null); }
  ...
  public ResizableKeyboardLayout(Context context, AttributeSet attrs,
    ...
    final TypedArray styleAttributeArray =
        getContext().getTheme().obtainStyledAttributes(
            attrs, R.styleable.ResizableKeyboardLayout, 0, 0);

    try {
      idOfViewToHide = styleAttributeArray.getResourceId(
          R.styleable.ResizableKeyboardLayout_viewToHideWhenActivated, -1);
    } finally {
    ...
    this.setOnApplyWindowInsetsListener((v, insets) -> {
      ...
      if (difference != 0) {
        ...
        if (getPaddingBottom() != difference) {
          ...
          // Zerda modification: We don't want extra margin in BrowserFragment
          // for main toolbar, so we truncate them.

...
public class DataSavingPathPreference extends ListPreference {
  ...
  public DataSavingPathPreference(Context context) { this(context, null); }
  ...
  protected void onAttachedToActivity() {
    ...
    buildList();
    // Put pingRemovableStorage() in background thread to avoid strict mode
    // violation: disk I/O on main thread.
  ...
  public CharSequence getSummary() {
    // design's spec, always show 'save to internal' if there is no removable
    // storage
    if (!hasRemovableStorage) {
      return getContext().getResources().getString(
          R.string.setting_dialog_internal_storage);
    }

    if (TextUtils.isEmpty(getEntry())) {
      final String[] entries = getContext().getResources().getStringArray(
          R.array.data_saving_path_entries);
      setValueIndex(0);
      return entries[0];
    }
  ...
  private void buildList() {
    final String[] entries = getContext().getResources().getStringArray(
        R.array.data_saving_path_entries);
    final String[] values = getContext().getResources().getStringArray(
        R.array.data_saving_path_values);

  ...
  protected void onDialogClosed(boolean positiveResult) {
    ...
    if (positiveResult) {
      ...
      for (String value : getValues()) {
        if (resources.getString(R.string.pref_value_clear_browsing_history)
                .equals(value)) {
          ...
          SharedPreferencePinSiteDelegate.Companion.resetPinSiteData(
              getContext());

        } else if (resources.getString(R.string.pref_value_clear_cookies)
                       .equals(value)) {
          ...
          if (PrivateMode.getInstance(getContext()).hasPrivateSession()) {
            // If there's a private mode process running, below intent will
            // reach PrivateModeActivity's onNewIntent, thus the activity won't
            // appear again. (assume that onNewIntent will always runs before
            // onStart() Fixme: we should rely on another Android component for
            // IPC to clear CookieManager Fixme: rather than rely on
            // PrivateModeActivity cause it will cause UI issue easily
            final Intent intent = PrivateSessionNotificationService.buildIntent(
                getContext().getApplicationContext(), true);
            getContext().startActivity(intent);
          }
        } else if (resources.getString(R.string.pref_value_clear_cache)
                       .equals(value)) {
          FileUtils.clearCache(getContext());
        } else if (resources.getString(R.string.pref_value_clear_form_history)
                       .equals(value)) {
      ...
      if (getValues().size() > 0) {
        Toast
            .makeText(getContext(), R.string.message_cleared_browsing_data,
                      Toast.LENGTH_SHORT)
            .show();
      }
    }

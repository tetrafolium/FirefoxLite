...
public class MockUIUtils {
  ...
  static void showSnackbarAndWait(@NotNull Activity activity, int strId,
                                  int actionStrId) {
    View view = activity.findViewById(R.id.container);
    if (view != null) {
      Snackbar.make(view, strId, Snackbar.LENGTH_LONG)
          .setAction(actionStrId, v -> {})
          .show();
  ...
  static void showToast(@NotNull Activity activity, int strId) {
    ...
    // Since we post a show toast runnable to main thread, we delay for a while
    // to make sure toast is displayed
    SystemClock.sleep(POPUP_DELAY);
  }

  static void showGeoPromptDialog(@NotNull Activity activity, String url) {
    activity.runOnUiThread(() -> {
      AlertDialog dialog = ((MainActivity)activity)
                               .getVisibleBrowserFragment()
                               .buildGeoPromptDialog();
      dialog.show();
    });
  }
}

...
public class SettingsScreenshot extends BaseScreenshot {
  ...
  public void screenshotSettings() {
    ...
    onView(allOf(withText(R.string.menu_settings),
                 withParent(withId(R.id.toolbar))))
        .check(matches(isDisplayed()));
    Screengrab.screenshot(ScreenshotNamingUtils.SETTINGS_LIST_1);

    // Click language
    onData(PreferenceMatchers.withKey(
    ...
    onData(PreferenceMatchers.withKey(resources.getString(
               R.string.pref_key_storage_clear_browsing_data)))
        .perform(click());
    onView(withText(R.string.setting_dialog_clear_data))
        .inRoot(isDialog())
        .check(matches(isDisplayed()));
    ...
    try {
      StorageUtils.getTargetDirOnRemovableStorageForDownloads(
          activityTestRule.getActivity(), "*/*");

      onData(PreferenceMatchers.withKey(resources.getString(
                 R.string.pref_key_storage_save_downloads_to)))
          .perform(click());
      onView(withText(R.string.preference_privacy_storage_save_downloads_to))
          .inRoot(isDialog())
          .check(matches(isDisplayed()));
      ...
      Log.e(TAG,
            "there's no removable storage for screenshot so we skip this.");
    }
    ...
    onData(PreferenceMatchers.withKey(
        ...
        .perform(click());
    onView(withText(R.string.rate_app_dialog_btn_feedback))
        .inRoot(isDialog())
        .check(matches(isDisplayed()));
    ...
    onData(PreferenceMatchers.withKey(
        ...
        .perform(click());
    onView(withText(R.string.share_app_dialog_btn_share))
        .inRoot(isDialog())
        .check(matches(isDisplayed()));
    ...
    onWebView()
        .withElement(findElement(
            Locator.XPATH, String.format("//a[contains(text(),'%s')]",
                                         resources.getString(
                                             R.string.about_link_your_rights))))
        .perform(webClick());
    SystemClock.sleep(MockUIUtils.SHORT_DELAY);

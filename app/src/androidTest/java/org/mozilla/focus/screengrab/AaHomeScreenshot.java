...
public class AaHomeScreenshot extends BaseScreenshot {
  ...
  public void setUp() {
    ...
    //        FalconScreenshotStrategy(activityTestRule.getActivity()));
  }
  ...
  public void screenshotHomeFragment() {
    ...
    onView(withText(R.string.bookmarks_empty_view_msg))
    ...
    onView(withText(R.string.browsing_history_empty_view_msg))
        .check(matches(isDisplayed()));
    Screengrab.screenshot(ScreenshotNamingUtils.HOME_MENU_NO_HISTORY);

    // Click screenshot
    onView(withId(R.id.screenshots)).perform(click());
    onView(withText(R.string.screenshot_grid_empty_text_title))
        .check(matches(isDisplayed()));
    ...
    AndroidTestUtils.toastContainsText(activityTestRule.getActivity(),
                                       R.string.message_disable_turbo_mode);
    ...
    SystemClock.sleep(MockUIUtils.POPUP_DELAY);
    AndroidTestUtils.toastContainsText(activityTestRule.getActivity(),
                                       R.string.message_enable_turbo_mode);
    ...
    SystemClock.sleep(MockUIUtils.POPUP_DELAY);
    AndroidTestUtils.toastContainsText(activityTestRule.getActivity(),
                                       R.string.message_enable_block_image);
    ...
    SystemClock.sleep(MockUIUtils.POPUP_DELAY);
    AndroidTestUtils.toastContainsText(activityTestRule.getActivity(),
                                       R.string.message_disable_block_image);
    ...
    activityTestRule.getActivity().runOnUiThread(
        ()
            -> DialogUtils.showRateAppNotification(
                activityTestRule.getActivity()));
    device.wait(
        Until.hasObject(By.text(
            resources.getString(R.string.rate_app_dialog_text_title,
                                resources.getString(R.string.app_name)) +
            "\uD83D\uDE00")),
        MockUIUtils.LONG_DELAY);
    Screengrab.screenshot(ScreenshotNamingUtils.HOME_NOTI_RATE_APP);

    activityTestRule.getActivity().runOnUiThread(
        ()
            -> DialogUtils.showDefaultSettingNotification(
                activityTestRule.getActivity()));
    device.wait(Until.hasObject(By.text(
                    resources.getString(R.string.preference_default_browser) +
                    "?\uD83D\uDE0A")),
                MockUIUtils.LONG_DELAY);
    SystemClock.sleep(MockUIUtils.POPUP_DELAY);
    ...
    activityTestRule.getActivity().runOnUiThread(
        ()
            -> DialogUtils.showPrivacyPolicyUpdateNotification(
                activityTestRule.getActivity()));
    device.wait(Until.hasObject(By.text(resources.getString(

...
public final class AndroidTestUtils {
  ...
  public static Buffer readTestAsset(String filename) throws IOException {
    try (final InputStream stream =
             InstrumentationRegistry.getContext().getAssets().open(filename)) {
  ...
  public static void removeNewAddedTab() {
    ...
    onView(withId(R.id.tab_tray))
        .perform(RecyclerViewActions.actionOnItemAtPosition(
            0, clickChildViewWithId(R.id.close_button)));
  }
  ...
  public static ViewAction clickXY(final int x, final int y, final Tap tap) {
    return new GeneralClickAction(tap, new CoordinatesProvider() {
      @Override
  ...
  public static void showMenu(ActivityTestRule<MainActivity> activityTestRule) {
    if (activityTestRule != null) {
      ...
      mainActivity.runOnUiThread(
          ()
              -> Inject.obtainChromeViewModel(mainActivity)
                     .getShowMenu()
                     .call());
    }
  }
  ...
  public static void tapSettingButton() {
    onView(allOf(withId(R.id.menu_preferences), isDisplayed()))
        .perform(click());
  }
  ...
  public static void setRateAppPromotionIsReadyToShow() {
    ...
    final SharedPreferences preferences =
        PreferenceManager.getDefaultSharedPreferences(context);
    preferences.edit()
        .putBoolean(context.getResources().getString(
                        R.string.pref_key_did_show_rate_app_dialog),
                    false)
        .apply();

    // Force app created count to hit threshold to show app promotion dialog
    final Settings.EventHistory history =
        Settings.getInstance(context).getEventHistory();
    history.setCount(Settings.Event.AppCreate,
                     (int)AppConfigWrapper.getRateDialogLaunchTimeThreshold());
  }

  public static void toastContainsText(@NotNull final Activity activity,
                                       final int strId) {

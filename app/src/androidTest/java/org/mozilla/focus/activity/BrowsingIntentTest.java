...
public class BrowsingIntentTest {
  ...
  private static final String TARGET_URL_SITE_1 =
      "https://developer.mozilla.org/en-US/";
  private static final String TARGET_URL_SITE_2 =
  ...
  public void receivedBrowsingIntent_tabIsCreated() {
    ...
    onView(withId(R.id.url_edit))
        .perform(replaceText(TARGET_URL_SITE_2), pressImeActionButton());
    onView(withId(R.id.display_url))
        .check(matches(isDisplayed()))
        .check(matches(withText(TARGET_URL_SITE_2)));

    // Currently BrowserFragment need tabSession completes it's job to know if
    // we can go back. Without an idling resource there is pressBack() will
    // leave the app and make the test crash
    // TODO: Add idling resource, or extend SessionIdlingResource to not only
    // wait for onTabFinished,
    // TODO: but also wait for TabSession to complete. See canGoBack() in
    // BrowserFragment Click back
    //        Espresso.pressBack();
    //
  ...
  public void appHasOneTabAndReceiveBrowsingIntent_tabIncreasedAndBrowse() {
    ...
    SessionManager sessionManager =
        TabsSessionProvider.getOrThrow(activityTestRule.getActivity());

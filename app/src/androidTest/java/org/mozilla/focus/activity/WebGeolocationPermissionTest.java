...
// "For applications targeting Android N and later releases (API level > M) the
// geolocation api is only supported on secure origins such as https."
...
public class WebGeolocationPermissionTest {
  ...
  public void webPopupGeoPermissionRequest_clickAllowAndUpdateGeoData() {
    ...
    // Find the element in HTML with id "get_button" and click it. "get_button"
    // will try to access user's current location
    onWebView()
        .withElement(findElement(Locator.ID, HTML_ELEMENT_ID_GET_BUTTON))
        .perform(webClick());

    // Check the Geolocation permission dialog is popup and click allow button
    onView(withText(R.string.geolocation_dialog_allow))
        .check(matches(isDisplayed()))
        .perform(click());

    // Once the permission is granted, web page will start updating Geolocation
    onWebView()
        .withElement(findElement(Locator.ID, HTML_ELEMENT_ID_RESULT))
        .check(webMatches(getText(), containsString(RESULT_CHECK_TEXT)));

...
public class ChangeLanguageTest {
  ...
  // This is the lang name of Android system. We hard code it here so it will
  // only fits emulator For physical devices this may vary.
  ...
  public void changeDisplayLang() {
    // To do : we hard code the language may apply
    // getPackagedLocaleTags(getContext()) later Start MainActivity
    ...
    onData(withKey(resources.getString(R.string.pref_key_locale)))
        .perform(click());
  ...
  private UiScrollable scrollToTapLang(MainActivity activity, String lang) {
    ...
    try {
      ...
      throw new AssertionError(
          String.format("Could not find %s in system language dialog", lang),
          e);
    }

    // Check lang displayed
    onData(allOf(is(instanceOf(Preference.class)),
                 withKey(activity.getResources().getString(
                     R.string.pref_key_locale))))
        .onChildView(withText(lang))
        .check(matches(isDisplayed()));
    return langList;
  }
}

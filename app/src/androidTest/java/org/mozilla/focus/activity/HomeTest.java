...
public class HomeTest {
  ...
  // Defer the startup of the activity cause we want to avoid First Run / Share
  // App / Rate App dialogs
  @Rule
  ...
  public void clickTopSite_loadTopSite() {
    ...
    try {
      ...
      final JSONArray jsonDefault =
          new JSONArray(Inject.getDefaultTopSites(context));
      final List<Site> defaultSites =
          TopSitesUtils.paresJsonToList(context, jsonDefault);

      // Check the title of the sample top site is correct
      onView(withId(R.id.main_list))
          .check(matches(atPosition(
              0, hasDescendant(withText(defaultSites.get(0).getTitle())))));

      // Click and load the sample top site
      // Some intermittent issues happens when performing a single click event,
      // we add a rollback action in case of a long click action is triggered
      ...
      onView(withId(R.id.display_url))
          .check(matches(
              allOf(withText(defaultSites.get(0).getUrl()), isDisplayed())));
  ...
  public void checkBasicHomeComponents_allAreVisible() {
    ...
    onView(bottomBarRobot.homeBottomBarItemView(R.id.bottom_bar_menu))
        .check(matches(isDisplayed()));

    // Check if tab tray button is visible
    onView(bottomBarRobot.homeBottomBarItemView(R.id.bottom_bar_tab_counter))
        .check(matches(isDisplayed()));
  }
  ...
  public void dismissMenu() {
    ...
    final DisplayMetrics displayMetrics =
        activityRule.getActivity().getResources().getDisplayMetrics();

...
public class SaveRestoreTabsTest {
  ...
  private static final TabEntity TAB =
      new TabEntity("TEST_ID", "ID_HOME", "Yahoo TW", "https://tw.yahoo.com");
  private static final TabEntity TAB_2 = new TabEntity(
      "TEST_ID_2", TAB.getId(), "Google", "https://www.google.com");
   ...
   * 6. open tab tray to check url, icon, website title, website subtitle, close
   * button displayed
   */
  @Test
  public void restoreEmptyTab_addNewTabThenRelaunch() {
    ...
    // Some intermittent issues happens when performing a single click event, we
    // add a rollback action in case of a long click action is triggered
    // unexpectedly here. i.e. pressBack() can dismiss the popup menu.

    onView(withId(R.id.main_list))
        .perform(
            RecyclerViewActions.actionOnItemAtPosition(0, click(pressBack())));
  ...
  public void tabTray_closeAllTab() throws InterruptedException {
    ...
    assertThat(RecyclerViewTestUtils.getCountFromRecyclerView(R.id.tab_tray),
               is(2));

    // Tap Close All -> Tap Ok
    onView(allOf(withId(R.id.close_all_tabs_btn), isDisplayed()))
        .perform(click());
  ...
  private void checkHomeTabCounterText(String text) {
    onView(allOf(withId(R.id.counter_text),
                 isDescendantOfA(new BottomBarRobot().homeBottomBarItemView(

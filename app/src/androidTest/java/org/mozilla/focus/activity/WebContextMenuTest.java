...
public class WebContextMenuTest {
  ...
  private static final String HTML_FILE_FULL_SCREEN_IMAGE =
      "fullscreen_image_test.html";
  ...
  @Rule
  public final GrantPermissionRule writePermissionRule =
      GrantPermissionRule.grant(Manifest.permission.WRITE_EXTERNAL_STORAGE);
  @Rule
  public final GrantPermissionRule readPermissionRule =
      GrantPermissionRule.grant(Manifest.permission.READ_EXTERNAL_STORAGE);

  @Rule
  public IntentsTestRule<MainActivity> intentsTestRule = new IntentsTestRule<
      MainActivity>(MainActivity.class, true, false) {
    @Override
    protected void beforeActivityLaunched() {
      ...
      try {
        webServer.enqueue(
            new MockResponse()
                .setBody(
                    AndroidTestUtils.readTestAsset(HTML_FILE_FULL_SCREEN_IMAGE))
                .addHeader(
                    "Set-Cookie",
                    "sphere=battery; Expires=Wed, 21 Oct 2035 07:28:00 GMT;"));
        webServer.enqueue(new MockResponse().setBody(
            AndroidTestUtils.readTestAsset(IMAGE_FILE_NAME_DOWNLOADED)));
        webServer.enqueue(new MockResponse().setBody(
            AndroidTestUtils.readTestAsset(IMAGE_FILE_NAME_DOWNLOADED)));
  ...
  public void openLinkInNewTab() {
    ...
    onView(allOf(withId(R.id.snackbar_action),
                 withText(R.string.new_background_tab_switch)))
        .check(matches(isDisplayed()));
    ...
    onView(allOf(withId(R.id.counter_text),
                 isDescendantOfA(new BottomBarRobot().browserBottomBarItemView(
                     R.id.bottom_bar_tab_counter))))
  ...
  public void shareLink_sendShareIntent() {
    ...
    // Click "Share link"
    onView(allOf(withText(R.string.contextmenu_link_share), isDisplayed()))
        .perform(click());
    ...
    intended(allOf(hasAction(Intent.ACTION_CHOOSER),
                   hasExtra(is(Intent.EXTRA_INTENT),
                            allOf(hasAction(Intent.ACTION_SEND),
                                  hasExtra(Intent.EXTRA_TEXT, getLinkUrl())))));
  }
  ...
  public void copyLinkAddress() {
    ...
    onView(allOf(withText(R.string.contextmenu_link_copy), isDisplayed()))
        .perform(click());

    // Get clip data form clip board
    final ClipboardManager clipboard =
        (ClipboardManager)intentsTestRule.getActivity().getSystemService(
  ...
  public void shareImage_sendShareIntent() {
    ...
    // Click "Share image"
    onView(allOf(withText(R.string.contextmenu_image_share), isDisplayed()))
        .perform(click());
  ...
  public void copyImageAddress() {
    ...
    onView(allOf(withText(R.string.contextmenu_image_copy), isDisplayed()))
        .perform(click());
  ...
  public void saveImageThenDelete_imageSaveAndDeleteSuccessfully() {
    ...
    downloadCompleteIdlingResource =
        new DownloadCompleteIdlingResource(intentsTestRule.getActivity());
    downloadCompleteIdlingResource.registerDownloadCompleteObserver();

    // Click save image button
    onView(allOf(withText(R.string.contextmenu_image_save), isDisplayed()))
        .perform(click());
    ...
    // Open menu
    // Since right now snackbar will overlap with menu bar and we don't want to
    // wait until snackbar is dismissed, we cannot call
    // onView(withId(R.id.btn_menu) here so call showMenu in MainActivity
    // instead.
    ...
    onView(withId(R.id.recyclerview))
        .check(
            matches(atPosition(0, hasDescendant(withText(containsString(
                                      IMAGE_FILE_NAME_DOWNLOADED_PREFIX))))));
  }

  private void loadTestWebsiteAndOpenContextMenu() {
    sessionLoadedIdlingResource =
        new SessionLoadedIdlingResource(intentsTestRule.getActivity());
    ...
    onView(withId(R.id.url_edit))
        .perform(replaceText(webServer.url(TEST_PATH).toString()),
                 pressImeActionButton());

    // Waiting for the page is loaded
    IdlingRegistry.getInstance().register(sessionLoadedIdlingResource);

    // Since test web page has a fullscreen image, we simulate the long click
    // event by clicking at the center of image Long click on image with popup
    // context menu
    final DisplayMetrics displayMetrics =
        intentsTestRule.getActivity().getResources().getDisplayMetrics();
    final int displayWidth = displayMetrics.widthPixels;
    final int displayHeight = displayMetrics.heightPixels;
    onView(withId(R.id.main_content))
        .check(matches(isDisplayed()))
        .perform(AndroidTestUtils.clickXY(displayWidth / 2, displayHeight / 2,
                                          Tap.LONG));
    IdlingRegistry.getInstance().unregister(sessionLoadedIdlingResource);
  }

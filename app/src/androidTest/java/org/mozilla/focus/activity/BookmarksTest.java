...
public class BookmarksTest {
  ...
  private static final String MOCK_BOOKMARK_CONTENT_LONG =
  ...
  public void addAndRemoveBookmarks_bookmarkIsAddedAndRemoved() {
    ...
    onView(allOf(withId(com.google.android.material.R.id.snackbar_text),
                 withText(R.string.bookmark_saved)))
    ...
    // wait until snackbar is dismissed
  ...
  public void editBookmarkWithChangingContent_bookmarkIsUpdated() {
    ...
    onView(withId(R.id.bookmark_name))
        .perform(replaceText(MOCK_BOOKMARK_CONTENT));

    // Type some text in location field
    onView(withId(R.id.bookmark_location))
        .perform(replaceText(MOCK_BOOKMARK_CONTENT));

    // Click the save button
    onView(withText(R.string.bookmark_edit_save)).perform(click());

    // We don't check "Bookmark updated" toast here since the toast is sent by
    // EditBookmarkActivity which finish itself immediately after show the
    // toast. It might cause some inconsistent result once we check the
    // visibility of toast. Check toast work on local and ci/push but failed on
    // ci/pr. Check if the target item bookmark is updated
    onView(withId(R.id.recyclerview))
        .check(matches(atPosition(0, hasDescendant(withText(containsString(
                                         MOCK_BOOKMARK_CONTENT))))));
  }
  ...
  public void editBookmarkWithoutChangingContent_saveButtonIsDisabled() {
    ...
    onView(withText(R.string.bookmark_edit_save))
  ...
  public void editBookmarkWithVariousWords_bookmarkIsUpdated() {
    ...
    // Type some text in name field
    onView(withId(R.id.bookmark_name))
        .perform(replaceText(MOCK_BOOKMARK_CONTENT_LONG));
  ...
  private void tapBookmarkItemActionMenu() {
    ...
    onView(withId(R.id.recyclerview))
        .check(matches(
            atPosition(0, hasDescendant(withText(containsString(targetUrl))))));

    // Open target bookmark item's action menu
    onView(withId(R.id.recyclerview))
        .perform(RecyclerViewActions.actionOnItemAtPosition(
            0, clickChildViewWithId(R.id.history_item_btn_more)));
  }
}
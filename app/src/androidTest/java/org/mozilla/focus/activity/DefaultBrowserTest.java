...
// However, Browser select dialog behavior is different between API 24,25 (7.X)
// and API 26 (8.0) so this test caters to API 24, 25
@RunWith(AndroidJUnit4.class)
@SdkSuppress(minSdkVersion = 24, maxSdkVersion = 25)
public class DefaultBrowserTest {

  // This is the title of Android system setting. We hard code it here so it
  // will only fits emulator For physical devices this may vary.
  private final static String SETTING_DEFAULT_BROWSER = "Browser app";

  private UiDevice uiDevice;

  @Rule
  public final ActivityTestRule<SettingsActivity> settingsActivity =
  ...
  /**
   * Test case no: TC0090
   * Test case name: Set Firefox Lite as default browser when Chrome is default
   * browser Steps:
  ...
  public void changeDefaultBrowser_whenChromeIsDefault() {
    ...
    final String prefName =
        activity.getString(R.string.pref_key_default_browser);
    ...
    // Click on "Default Browser" setting, this will brings up the Android
    // system setting
    clickDefaultBrowserSetting(prefName);

    // Open default browser setting
    final UiObject allAppsButton =
        uiDevice.findObject(new UiSelector().text(SETTING_DEFAULT_BROWSER));
    ...
    try {
      ...
      throw new AssertionError("Could find the chrome app in default browser",
                               e);
    }
    // Since it is not on root activity so we use ui-automator instead of
    // Espresso Press back
    UiDevice mDevice =
    ...
    onData(allOf(is(instanceOf(Preference.class)), withKey(prefName)))
    ...
    settingsActivity.launchActivity(
        (new Intent().setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK)));

    // Check if the "Default Browser" pref is correctly displayed (switch
    // checked)
    onData(allOf(is(instanceOf(Preference.class)), withKey(prefName)))
        .onChildView(withClassName(is(Switch.class.getName())))
  ...
  private void chooseLiteAsDefaultBrowser(MainActivity activity) {
    ...
    final UiSelector rocketView =
        new UiSelector().text(activity.getString(R.string.app_name));
    try {
      ...
      throw new AssertionError(
          "Could find the Firefox Lite app in default browser", e);
    }
  }

...
public class ScreenNavigatorTest {
  ...
  /**
   * Test whether showBrowserScreen() can load and bring browser screen to
   * foreground properly
   */
  @Test
   ...
   * 5. Repeat step 3~4
   * 6. Press back
   * 7. There should be only 1 home screen, and pressing back will lead to
   * previous browsing page
   */
  @Test
   ...
   * 7. There should be only 1 home screen, and pressing back will finish the
   * app
   */
  ...
  private void assertNewPageLoaded(ScreenNavigatorWrapper navigator,
                                   String url) {
  ...
  private static class ScreenNavigatorWrapper {
    ...
    ScreenNavigatorWrapper(MainActivity activity) { this.activity = activity; }

    void showBrowserScreen(final String url) {
      activity.runOnUiThread(
          ()
              -> activity.getScreenNavigator().showBrowserScreen(url, true,
                                                                 false));
    }

    void addHomeScreen() {
      activity.runOnUiThread(
          () -> activity.getScreenNavigator().addHomeScreen(false));
    }

    void popToHomeScreen() {
      activity.runOnUiThread(
          () -> activity.getScreenNavigator().popToHomeScreen(false));
    }

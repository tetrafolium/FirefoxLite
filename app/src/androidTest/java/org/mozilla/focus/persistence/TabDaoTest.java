...
public class TabDaoTest {
  ...
  public void updateAndGetTab() {
    ...
    TabEntity updatedTab = new TabEntity(TAB.getId(), TAB.getParentId(),
                                         "new title", TAB.getUrl());
  ...
  public void migrationFrom1To2_containsCorrectData() throws IOException {
    ...
    // Re-open the database with version 2 and provide MIGRATION_1_2 as the
    // migration process.
    testHelper.runMigrationsAndValidate(TEST_DB_NAME, 2, true,
                                        TabsDatabase.MIGRATION_1_2);

    // MigrationTestHelper automatically verifies the schema changes, but not
    // the data validity Validate that the data was migrated properly.

...
/* package-private */ class Trie {
  ...
  public static class WhiteListTrie extends Trie {
    ...
    public void putWhiteList(final FocusString string, final Trie whitelist) {
      ...
      if (node.whitelist != null) {
        throw new IllegalStateException("Whitelist already set for node " +
                                        string);
      }
  ...
  public Trie findNode(final FocusString string) {
    if (terminator) {
      // Match achieved - and we're at a domain boundary. This is important,
      // because we don't want to return on partial domain matches. (E.g. if the
      // trie node is bar.com, and the search string is foo-bar.com, we
      // shouldn't match. But foo.bar.com should match.)

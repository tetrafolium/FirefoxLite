...
/* package-private */ class EntityList {
  ...
  public EntityList() { rootNode = WhiteListTrie.createRootNode(); }
  ...
  public boolean isWhiteListed(final Uri site, final Uri resource) {
    ...
    if (UrlUtils.isPermittedResourceProtocol(resource.getScheme()) &&
      ...
      final FocusString revSitehost =
          FocusString.create(site.getHost()).reverse();
      final FocusString revResourcehost =
          FocusString.create(resource.getHost()).reverse();
      ...
      // things like "nielsenwebid://nuid/999" and/or sets an iFrame URL to
      // that:
      return false;
    }
  }

  private boolean isWhiteListed(final FocusString site,
                                final FocusString resource,
                                final Trie revHostTrie) {
    final WhiteListTrie next =
        (WhiteListTrie)revHostTrie.children.get(site.charAt(0));
    ...
    if (next.whitelist != null && next.whitelist.findNode(resource) != null) {
      return true;
    }

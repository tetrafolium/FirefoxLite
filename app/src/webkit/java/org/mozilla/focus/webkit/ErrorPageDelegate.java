...
public class ErrorPageDelegate {
  ...
  ErrorPageDelegate(final WebView webView) {
    factory = new ErrorViewFactory<HtmlErrorViewHolder>() {
      ...
      @Override
      public void onBindErrorView(HtmlErrorViewHolder viewHolder, int errorCode,
      ...
      @Override
      public void onBindSslErrorView(HtmlErrorViewHolder viewHolder,
                                     SslErrorHandler handler, SslError error) {
        WebBackForwardList list = webView.copyBackForwardList();
        WebHistoryItem currentItem = list.getCurrentItem();
        if (currentItem != null) {
          webView.removeView(viewHolder.rootView);
          ErrorPage.loadErrorPage(webView, error.getUrl(),
                                  error.getPrimaryError());
        }
      }

      @Override
      public void onBindHttpErrorView(HtmlErrorViewHolder viewHolder,
                                      WebResourceRequest request,
                                      WebResourceResponse errorResponse) {
        ErrorPage.loadErrorPage(viewHolder.webView, request.getUrl().toString(),
      ...
      // Web view here is used only for local error page, should have no
      // security problem. thus suppressing the lint
  ...
  public void onPageStarted() { dismissErrorView(); }

  public void onReceivedError(WebView webView, int errorCode,
                              String description, String failingUrl) {
    createErrorView();
    factory.onBindErrorView(factory.getErrorViewHolder(), errorCode,
  ...
  public void onReceivedHttpError(WebView webView, WebResourceRequest request,
    ...
    // createErrorView();
    // factory.onBindHttpErrorView(factory.getErrorViewHolder(), request,
    // errorResponse);
  }

  private void createErrorView() { factory.createErrorView(); }

  private void dismissErrorView() { factory.destroyErrorView(); }
  ...
  public static abstract class ErrorViewFactory<T extends ErrorViewHolder> {
    ...
    public abstract void onBindErrorView(T viewHolder, int errorCode,
                                         String description, String failingUrl);
    public abstract void
    onBindSslErrorView(T viewHolder, SslErrorHandler handler, SslError error);
    public abstract void onBindHttpErrorView(T viewHolder,
                                             WebResourceRequest request,
                                             WebResourceResponse errorResponse);
    ...
    private T getErrorViewHolder() { return errorViewHolder; }
  }

  public static class ErrorViewHolder {
    public View rootView;
    ErrorViewHolder(View rootView) { this.rootView = rootView; }
  }

  public static class HtmlErrorViewHolder extends ErrorViewHolder {
    WebView webView;

    HtmlErrorViewHolder(View rootView) { super(rootView); }
  }
}

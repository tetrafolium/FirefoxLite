 ...
 * <p>
 * We regularly need to reverse strings. The standard way of doing this in Java
 * would be to copy the string to reverse (e.g. using StringBuffer.reverse()).
 * This seems wasteful when we only read our Strings character by character, in
...
public abstract class FocusString {
  ...
  private FocusString(final String string, final int offsetStart,
                      final int offsetEnd) {
    this.string = string;
    ...
    if (offsetStart > offsetEnd || offsetStart < 0 || offsetEnd < 0) {
      throw new StringIndexOutOfBoundsException(
          "Cannot create negative-length String");
    }
  }
  ...
  private static class ForwardString extends FocusString {
    public ForwardString(final String string, final int offsetStart,
                         final int offsetEnd) {
  ...
  private static class ReverseString extends FocusString {
    public ReverseString(final String string, final int offsetStart,
                         final int offsetEnd) {
      super(string, offsetStart, offsetEnd);
    }

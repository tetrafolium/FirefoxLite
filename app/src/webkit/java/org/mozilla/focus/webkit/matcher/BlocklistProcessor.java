...
public class BlocklistProcessor {
  ...
  public enum ListType { BASE_LIST, OVERRIDE_LIST }

  public static Map<String, Trie>
  loadCategoryMap(final JsonReader reader, final Map<String, Trie> categoryMap,
                  final ListType listType) throws IOException {
    reader.beginObject();
  ...
  private static class ListCallback implements UrlListCallback {
    ...
    /**
     * @param desiredOwners A set containing all the site owners that should be
     *     stored in the list.
     *                      Corresponds to the group owners listed in
     * blocklist.json (e.g. "Facebook", "Twitter", etc.)
     */
  ...
  private static class TrieCallback implements UrlListCallback {
    ...
    TrieCallback(final Trie trie) { this.trie = trie; }
  ...
  private static void extractCategories(final JsonReader reader,
    ...
    while (reader.hasNext()) {
      ...
      if (IGNORED_CATEGORIES.contains(categoryName)) {
        ...
        ListCallback callback =
            new ListCallback(socialOverrides, DISCONNECT_MOVED);
        ...
        if (listType == ListType.BASE_LIST) {
          if (categoryMap.containsKey(categoryName)) {
            throw new IllegalStateException(
                "Cannot insert already loaded category: " + categoryName);
          }
          ...
          if (categoryTrie == null) {
            throw new IllegalStateException(
                "Cannot add override items to nonexistent category");
          }
        }
    ...
    if (socialTrie == null && listType == ListType.BASE_LIST) {
      throw new IllegalStateException(
          "Expected social list to exist. Can't copy FB/Twitter into non-existing list");
    }
  ...
  private static void extractCategory(final JsonReader reader,
                                      final UrlListCallback callback)
      throws IOException {
    reader.beginArray();
  ...
  private static void extractSite(final JsonReader reader,
                                  final UrlListCallback callback)
      throws IOException {
    reader.beginObject();

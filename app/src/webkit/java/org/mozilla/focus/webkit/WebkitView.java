...
public class WebkitView extends NestedWebView implements TabView {
  ...
  public WebkitView(Context context, AttributeSet attrs) {
    ...
    webViewClient =
        new FocusWebViewClient(getContext().getApplicationContext()) {
          @Override
    ...
    webViewClient.setErrorPageDelegate(
        (errorPageDelegate = new ErrorPageDelegate(this)));
  ...
  public void restoreViewState(Bundle savedInstanceState) {
    ...
    // Pages are only added to the back/forward list when loading finishes. If a
    // new page is loading when the Activity is paused/killed, then that page
    // won't be in the list, and needs to be restored separately to the history
    // list. We detect this by checking whether the last fully loaded page
    // (getCurrentItem()) matches the last page that the WebView was actively
    // loading (which was retrieved during saveViewState(): WebView.getUrl()
    // always returns the currently loading or loaded page). If the app is
    // paused/killed before the initial page finished loading, then the entire
    // list will be null - so we need to additionally check whether the list
    // even exists.

    final String desiredURL = savedInstanceState.getString(KEY_CURRENTURL);

    // If WebView was connecting to a non-exist host (ie. 1.1.1.1:42), getUrl()
    // returns null in saveViewState. In any cases we can not get desiredURL, no
    // need to load it.
    ...
    if (backForwardList != null &&
        (currentItem = backForwardList.getCurrentItem()) != null) {
      ...
      if (desiredURL.equals(latestHistoryUrl)) {
        ...
        // Two possible cases:
        // Case 1: last url ends up become an error page, the url saved in
        // back-forward list is an error-url. Case 2: last url is not yet being
        // saved by the WebView (WebView didn't finished loading)

        // noinspection StatementWithEmptyBody, for clearer logic and comment
        if (UrlUtils.isInternalErrorURL(latestHistoryUrl) &&
            desiredURL.equals(getUrl())) {
          // Case 1: What we get from WebHistoryItem:
          // WebHistoryItem#getOriginalUrl(): error-url
          // WebHistoryItem#getUrl(): error-url
          //
          // However, after WebView restores this error-url, WebView#getUrl()
          // will instead return a virtual url given when we called
          // loadDataWithBaseUrl(..., virtualUrl)
          //
          // What we get from WebView after error-url is restored:
          // WebView#getOriginalUrl(): error-url
          // WebView#getUrl(): virtual-url

          // The condition here means "WebView and us agree the last page is
          // <virtual-url>, which however, ends up become an error page.
  ...
  public void saveViewState(Bundle outState) {
    ...
    // See restoreWebViewState() for an explanation of why we need to save this
    // in _addition_ to WebView's state
  ...
  public void bindOnNewWindowCreation(@NonNull Message msg) {
    if (!(msg.obj instanceof WebView.WebViewTransport)) {
      throw new IllegalArgumentException(
          "Message payload is not a WebViewTransport instance");
    }
  ...
  public void performExitFullScreen() {
    evaluateJavascript(
        "(function() { return document.webkitExitFullscreen(); })();", null);
  }
  ...
  public @SiteIdentity.SecurityState int getSecurityState() {
    // FIXME: Having certificate doesn't mean the connection is secure, see
    // #1562
    return getCertificate() == null ? SiteIdentity.INSECURE
                                    : SiteIdentity.SECURE;
  }
  ...
  public void cleanup() {
    ...
    // We don't care about the viewClient - we just want to make sure cookies
    // are gone
  ...
  public static void deleteContentFromKnownLocations(final Context context) {
    ThreadUtils.postToBackgroundThread(new Runnable() {
      @Override
      public void run() {
        // We call all methods on WebView to delete data. But some traces still
        // remain on disk. This will wipe the whole webview directory.
        FileUtils.deleteWebViewDirectory(context);

        // WebView stores some files in the cache directory. We do not use it
        // ourselves so let's truncate it.
  ...
  public void insertBrowsingHistory() {
    ...
    evaluateJavascript(
        "(function() { return document.getElementById('mozillaErrorPage'); })();",
        errorPage -> {
          if (!"null".equals(errorPage)) {
            return;
          }

          Site site = BrowsingHistoryManager.prepareSiteForFirstInsert(
              url, getTitle(), System.currentTimeMillis());
      ...
      public void onDownloadStart(String url, String userAgent,
        ...
        if (downloadCallback != null) {
          ...
          final Download download =
              new Download(url, name, userAgent, contentDisposition, mimetype,
                           contentLength, false);

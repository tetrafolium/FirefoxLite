...
/**
 * Generates a new implementation of {@link
 * com.bumptech.glide.request.RequestOptions} containing static versions of
 * methods included in the base class and static and instance versions of all
 * methods annotated with {@link GlideOption} in classes annotated with
 * {@link GlideExtension}.
 *
 * <p>The generated class looks something like this:
 * <pre>
 * <code>
 * public final class GlideOptions extends
 * com.bumptech.glide.request.RequestOptions {
 *
 *   public static com.google.android.apps.photos.glide.GlideOptions
 * signatureOf( com.bumptech.glide.load.Key arg0) { return new
 * com.google.android.apps.photos.glide.GlideOptions()
 *         .apply(com.bumptech.glide.request.RequestOptions.signatureOf(arg0));
 *   }
 *
 *   ... // The rest of the static versions of methods from RequestOptions go
 * here.
 *
...
final class RequestOptionsGenerator {
  ...
  RequestOptionsGenerator(ProcessingEnvironment processingEnvironment,
  ...
  TypeSpec generate(String generatedCodePackageName,
    ...
    glideOptionsName = ClassName.get(generatedCodePackageName,
    ...
    List<MethodAndStaticVar> staticOverrides =
        generateStaticMethodOverridesForRequestOptions();
    List<MethodSpec> instanceOverrides =
        generateInstanceMethodOverridesForRequestOptions();

    List<MethodAndStaticVar> allMethodsAndStaticVars = new ArrayList<>();
    for (MethodAndStaticVar item : staticOverrides) {
      if (extensionMethodSignatures.contains(
              new MethodSignature(item.method))) {
        continue;
      }
  ...
  private List<MethodAndStaticVar>
  generateMethodsForExtensions(Set<String> glideExtensionClassNames) {
    List<ExecutableElement> requestOptionExtensionMethods =
        processorUtil.findAnnotatedElementsInClasses(glideExtensionClassNames,
                                                     GlideOption.class);

    List<MethodAndStaticVar> result =
        new ArrayList<>(requestOptionExtensionMethods.size());
    for (ExecutableElement requestOptionsExtensionMethod :
         requestOptionExtensionMethods) {
      result.addAll(generateMethodsForRequestOptionsExtension(
          requestOptionsExtensionMethod));
    }
  ...
  private List<MethodSpec> generateInstanceMethodOverridesForRequestOptions() {
    return Lists.transform(processorUtil.findInstanceMethodsReturning(
                               requestOptionsType, requestOptionsType),
  ...
  private MethodSpec
  generateRequestOptionOverride(ExecutableElement methodToOverride) {
    MethodSpec.Builder result =
        MethodSpec.overriding(methodToOverride)
            ...
            .addCode(
                CodeBlock.builder()
                    .add("return ($T) super.$N(", glideOptionsName,
                         methodToOverride.getSimpleName())
                    .add(
                        FluentIterable.from(methodToOverride.getParameters())
    ...
    if (methodToOverride.getSimpleName().toString().equals("transforms")) {
      result.addAnnotation(AnnotationSpec.builder(SafeVarargs.class).build());
    }
  ...
  private List<MethodAndStaticVar>
  generateMethodsForRequestOptionsExtension(ExecutableElement element) {
    boolean isOverridingRequestOptionsMethod =
        isMethodInRequestOptions(element);
    ...
    } else if (!isOverridingRequestOptionsMethod &&
    ...
    // Generates the String and list of arguments to pass in when calling this
    // method or super. IE centerCrop(context) creates methodLiterals="%L" and
    // methodArgs=[centerCrop, context].
    ...
    builder.beginControlFlow("if (isAutoCloneEnabled())")
        .addStatement("return clone().$N(" + methodLiterals + ")",
    ...
    // Adds: <AnnotatedClass>.<thisMethodName>(RequestOptions<?>, <arg1>,
    // <arg2>, <argN>);
    ...
    builder.addAnnotation(
        AnnotationSpec.builder(CHECK_RESULT_CLASS_NAME).build());
  ...
  generateStaticMethodOverridesForRequestOptions() {
    List<ExecutableElement> staticMethodsThatReturnRequestOptions =
        processorUtil.findStaticMethodsReturning(requestOptionsType,
                                                 requestOptionsType);
    ...
    for (ExecutableElement element : staticMethodsThatReturnRequestOptions) {
      ...
      staticMethods.add(
          generateStaticMethodEquivalentForRequestOptionsStaticMethod(element));
    }
    return staticMethods;
  }

  /**
   * This method is a bit of a hack, but it lets us tie the static version of a
   * method with the instance version. In turn that lets us call the instance
   * versions on the generated subclass, instead of just delegating to the
   * RequestOptions static methods. Using the instance methods on the generated
   * subclass allows our static methods to properly call code that overrides an
   * existing method in RequestOptions.
   *
   * <p>The string names here just map between the static methods in
   * {@link com.bumptech.glide.request.RequestOptions} and the instance methods
   * they call.
   */
  private static String
  getInstanceMethodNameFromStaticMethodName(String staticMethodName) {
    ...
    if ("bitmapTransform".equals(staticMethodName)) {
      ...
      throw new IllegalArgumentException("Unrecognized static method name: " +
                                         staticMethodName);
    }
    return equivalentInstanceMethodName;
  }

  private MethodAndStaticVar
  generateStaticMethodEquivalentForRequestOptionsStaticMethod(
    ...
    if (!parameters.isEmpty()) {
      ...
      createNewOptionAndCall = createNewOptionAndCall.substring(
          0, createNewOptionAndCall.length() - 2);
    }
    createNewOptionAndCall += ")";
    ...
    if (memoize) {
      ...
      //   GlideOptions.<methodName> = new
      //   GlideOptions().<methodName>().autoClone()
      // }
      ...
      requiredStaticField =
          FieldSpec.builder(glideOptionsName, staticVariableName)
              ...
              .build();
      methodSpecBuilder
          .beginControlFlow("if ($T.$N == null)", glideOptionsName,
                            staticVariableName)
          .addStatement("$T.$N =\n" + createNewOptionAndCall + ".$N",
                        glideOptionsName, staticVariableName, glideOptionsName,
                        equivalentInstanceMethodName, "autoClone()")
          .endControlFlow()
          .addStatement("return $T.$N", glideOptionsName, staticVariableName);
    } else {
      // Generates code that looks like:
      // return new GlideOptions().<methodName>()
      methodSpecBuilder.addStatement("return " + createNewOptionAndCall,
                                     glideOptionsName,
                                     equivalentInstanceMethodName);
    }

    List<? extends TypeParameterElement> typeParameters =
        staticMethod.getTypeParameters();
    for (TypeParameterElement typeParameterElement : typeParameters) {
      methodSpecBuilder.addTypeVariable(TypeVariableName.get(
  ...
  memoizeStaticMethodFromArguments(ExecutableElement staticMethod) {
    return staticMethod.getParameters().isEmpty() ||
        (staticMethod.getParameters().size() == 1 &&
         staticMethod.getParameters().get(0).getSimpleName().toString().equals(
  ...
  private MethodAndStaticVar generateStaticMethodEquivalentForExtensionMethod(
    ...
    // Always remove the first parameter because it's always RequestOptions in
    // extensions. The actual method we want to generate will pass the
    // RequestOptions in to the extension method, but should not itself require
    // a RequestOptions object to be passed in.
    if (parameters.isEmpty()) {
      throw new IllegalArgumentException("Expected non-empty parameters for: " +
                                         instanceMethod);
    }
    ...
    if (memoize) {
      ...
      methodSpecBuilder
          ...
          .addStatement("$T.$N =\n" + createNewOptionAndCall + ".$N",
                        glideOptionsName, staticVariableName, glideOptionsName,
                        instanceMethodName, "autoClone()")
          .endControlFlow()
          .addStatement("return $T.$N", glideOptionsName, staticVariableName);
    } else {
      // return new GlideOptions().<methodName>()
      methodSpecBuilder.addStatement("return " + createNewOptionAndCall,
                                     glideOptionsName, instanceMethodName);
    }

    List<? extends TypeParameterElement> typeParameters =
        instanceMethod.getTypeParameters();
    for (TypeParameterElement typeParameterElement : typeParameters) {
      methodSpecBuilder.addTypeVariable(TypeVariableName.get(
          typeParameterElement.getSimpleName().toString()));
    }
  ...
  private boolean isAndroidContext(VariableElement variableElement) {
    Element element = processingEnvironment.getTypeUtils().asElement(
  ...
  private boolean isMethodInRequestOptions(ExecutableElement toFind) {
    // toFind is a method in a GlideExtension whose first argument is a
    // BaseRequestOptions<?> type. Since we're comparing against methods in
    // BaseRequestOptions itself, we need to drop that first type.
    List<String> toFindParameterNames =
        getComparableParameterNames(toFind, true /*skipFirst*/);
  ...
  private static ParameterSpec getParameterSpec(VariableElement variable) {
    return ParameterSpec
        .builder(TypeName.get(variable.asType()),
                 variable.getSimpleName().toString())
        .build();
  }

  private static List<String>
  getComparableParameterNames(ExecutableElement element, boolean skipFirst) {
  ...
  private static boolean
  memoizeStaticMethodFromAnnotation(ExecutableElement element) {
    GlideOption glideOption = element.getAnnotation(GlideOption.class);
  ...
  private static final class MethodAndStaticVar {
    ...
    MethodAndStaticVar() { this(null /*method*/); }
    ...
    MethodAndStaticVar(@Nullable MethodSpec method,
                       @Nullable FieldSpec staticField) {
      this.method = method;
  ...
  private static final class MethodSignature {
    ...
    MethodSignature(MethodSpec spec) {
      ...
      parameterTypes = Lists.transform(
          spec.parameters, new Function<ParameterSpec, TypeName>() {
            @Nullable
            @Override

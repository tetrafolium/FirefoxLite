...
// Links in Javadoc will work due to build setup, even though there is no direct
// dependency here.
/**
 * Generates classes based on Glide's annotations that configure Glide, add
 * support for additional resource types, and/or extend Glide's API.
 *
 * <p>This processor discovers all {@link AppGlideModule} and
 * {@link LibraryGlideModule} implementations that are
 * annotated with {@link com.bumptech.glide.annotation.GlideModule}. Any
 * implementations missing the annotation will be ignored.
 *
 * <p>This processor also discovers all {@link
 ...
 *      specific package that will later be used by the processor to discover
 * all
 ...
 *   <ul>
 *     <li>A {@link com.bumptech.glide.request.RequestOptions} implementation
 * that contains static versions of all builder methods in the base class and
 * both static and instance versions of methods in all {@link
 * com.bumptech.glide.annotation.GlideExtension}s. <li>If one or more methods in
 * one or more
 *     {@link com.bumptech.glide.annotation.GlideExtension} annotated classes
 * are annotated with
 *     {@link GlideType}:
 *     <ul>
 *       <li>A {@link com.bumptech.glide.RequestManager} implementation
 * containing a generated method for each method annotated with
 *       {@link GlideType}.
 *       <li>A {@link
 * com.bumptech.glide.manager.RequestManagerRetriever.RequestManagerFactory}
 *       implementation that produces the generated {@link
 * com.bumptech.glide.RequestManager}s. <li>A {@link com.bumptech.glide.Glide}
 * look-alike that implements all static methods in the {@link
 * com.bumptech.glide.Glide} singleton and returns the generated
 *       {@link com.bumptech.glide.RequestManager} implementation when
 * appropriate.
 *     </ul>
 *   </ul>
 * </ul>
...
public final class GlideAnnotationProcessor extends AbstractProcessor {
  ...
  public synchronized void init(ProcessingEnvironment processingEnvironment) {
    ...
    libraryModuleProcessor =
        new LibraryModuleProcessor(processorUtil, indexerGenerator);
    appModuleProcessor =
        new AppModuleProcessor(processingEnvironment, processorUtil);
    extensionProcessor =
        new ExtensionProcessor(processorUtil, indexerGenerator);
  }
   ...
   * <ol>
   *   <li>Find all AppGlideModules and save them to an instance variable (throw
   * if > 1). <li>Find all LibraryGlideModules <li>For each LibraryGlideModule,
   * write an Indexer with an Annotation with the class name. <li>If we wrote
   * any Indexers, return and wait for the next round. <li>If we didn't write
   * any Indexers and there is a AppGlideModule, write the
   *   GeneratedAppGlideModule. Once the GeneratedAppGlideModule is written, we
   * expect to be finished. Any further generation of related classes will
   * result in errors.
   * </ol>
   */
  @Override
  public boolean process(Set<? extends TypeElement> set, RoundEnvironment env) {
    ...
    boolean newExtensionWritten =
        extensionProcessor.processExtensions(set, env);

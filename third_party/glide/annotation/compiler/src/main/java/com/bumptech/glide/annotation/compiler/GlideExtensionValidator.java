...
/**
 * Validates that classes annotated with {@link
 * com.bumptech.glide.annotation.GlideExtension} contains methods with the
 * expected format.
 *
 * <p>Validation is performed so that errors can be found when a library is
 * compiled. Without validation, an error written in to a library wouldn't be
 * found until Glide tried to generate code for an Application.
 */
final class GlideExtensionValidator {
  ...
  static void validateExtension(TypeElement typeElement) {
    if (!typeElement.getModifiers().contains(Modifier.PUBLIC)) {
      throw new IllegalArgumentException(
          "RequestOptionsExtensions must be public");
    }
    for (Element element : typeElement.getEnclosedElements()) {
      if (element.getKind() == ElementKind.CONSTRUCTOR) {
        if (!element.getModifiers().contains(Modifier.PRIVATE)) {
          throw new IllegalArgumentException(
              "RequestOptionsExtensions must be public, with private"
              +
  ...
  private static void
  validateExtendsRequestOptions(ExecutableElement executableElement) {
    ...
    if (!expected.toString().equals(
      ...
      throw new IllegalArgumentException(
          "@GlideOption methods must take a"
          + " RequestOptions object as their first parameter, but given: " +
          expected);
    }
  }

  private static void
  validateExtendsRequestManager(ExecutableElement executableElement) {
    ...
    if (!expected.toString().startsWith("com.bumptech.glide.RequestBuilder")) {
      throw new IllegalArgumentException(
          "@GlideType methods must take a"
          + " RequestBuilder object as their first parameter, but given: " +
          expected);
    }
  }

  private static void validateStaticVoid(ExecutableElement executableElement,
                                         Class<?> clazz) {

...
/**
 * Generates a new implementation of a AppGlideModule that calls all included
 * LibraryGlideModules and the original AppGlideModule.
 *
 * <p>The generated class will always call the AppGlideModule last to give it
 * priority over choices made or classes registered in LibraryGlideModules.
 *
 * <p>Android logging is included to allow developers to see exactly which
 * modules are included at runtime.
 *
 * <p>The generated class looks something like this:
 * <pre>
 * <code>
 *  final class GeneratedAppGlideModuleImpl extends
 * com.bumptech.glide.GeneratedAppGlideModule { private final
 * com.bumptech.glide.samples.giphy.GiphyGlideModule appGlideModule;
 *
 *    GeneratedAppGlideModule() {
 *      appGlideModule = new
 * com.bumptech.glide.samples.giphy.GiphyGlideModule(); if
 * (android.util.Log.isLoggable("Glide", android.util.Log.DEBUG)) {
 *        android.util.Log.d("Glide", "Discovered AppGlideModule from
 * annotation:"
 *            + " com.bumptech.glide.samples.giphy.GiphyGlideModule");
 *        android.util.Log.d("Glide", "Discovered LibraryGlideModule from
 * annotation:"
 *            +
...
final class AppModuleGenerator {
  ...
  TypeSpec generate(TypeElement appGlideModule,
                    Set<String> libraryGlideModuleClassNames) {
    ...
    MethodSpec constructor = generateConstructor(appGlideModuleClassName,
                                                 libraryGlideModuleClassNames,
                                                 excludedGlideModuleClassNames);

    MethodSpec registerComponents = generateRegisterComponents(
        libraryGlideModuleClassNames, excludedGlideModuleClassNames);
    ...
    MethodSpec applyOptions =
        MethodSpec.methodBuilder("applyOptions")
            ...
            .addParameter(ClassName.get("android.content", "Context"),
                          "context")
            .addParameter(ClassName.get("com.bumptech.glide", "GlideBuilder"),
                          "builder")
    ...
    Builder builder =
        TypeSpec.classBuilder(GENERATED_APP_MODULE_IMPL_SIMPLE_NAME)
            ...
            .addField(appGlideModuleClassName, "appGlideModule",
    ...
    ClassName generatedRequestManagerFactoryClassName =
        ClassName.get(RequestManagerFactoryGenerator
                          .GENERATED_REQUEST_MANAGER_FACTORY_PACKAGE_NAME,
                      RequestManagerFactoryGenerator
                          .GENERATED_REQUEST_MANAGER_FACTORY_SIMPLE_NAME);

    builder.addMethod(MethodSpec.methodBuilder("getRequestManagerFactory")
                          .addAnnotation(Override.class)
                          .returns(generatedRequestManagerFactoryClassName)
                          .addStatement("return new $T()",
                                        generatedRequestManagerFactoryClassName)
                          .build());
  ...
  // remove this method.
  private MethodSpec
  generateGetExcludedModuleClasses(Set<String> excludedClassNames) {
    ...
    if (excludedClassNames.isEmpty()) {
      ...
      builder.addStatement("$T excludedClasses = new $T()",
      ...
      for (String excludedClassName : excludedClassNames) {
        ...
        // Using a Literal ($L) instead of a type ($T) to get a fully qualified
        // import that allows us to suppress deprecation warnings. Aimed at
        // deprecated GlideModules.
        builder.addStatement("excludedClasses.add($L.class)",
                             excludedClassName);
      }
  ...
  private MethodSpec
  generateRegisterComponents(Set<String> libraryGlideModuleClassNames,
                             Set<String> excludedGlideModuleClassNames) {
    MethodSpec.Builder registerComponents =
        MethodSpec.methodBuilder("registerComponents")
            ...
            .addParameter(ClassName.get("com.bumptech.glide", "Registry"),
                          "registry");
    ...
    for (String glideModule : libraryGlideModuleClassNames) {
      ...
      registerComponents.addStatement(
          "new $T().registerComponents(context, glide, registry)",
          moduleClassName);
    }
  ...
  private MethodSpec
  generateConstructor(ClassName appGlideModule,
                      Set<String> libraryGlideModuleClassNames,
                      Set<String> excludedGlideModuleClassNames) {
    MethodSpec.Builder constructorBuilder = MethodSpec.constructorBuilder();
    constructorBuilder.addStatement("appGlideModule = new $T()",
                                    appGlideModule);

    ClassName androidLogName = ClassName.get("android.util", "Log");

    // Add some log lines to indicate to developers which modules where
    // discovered.
    constructorBuilder.beginControlFlow("if ($T.isLoggable($S, $T.DEBUG))",
                                        androidLogName, GLIDE_LOG_TAG,
                                        androidLogName);
    constructorBuilder.addStatement(
        "$T.d($S, $S)", androidLogName, GLIDE_LOG_TAG,
        "Discovered AppGlideModule from annotation: " + appGlideModule);
    // Excluded GlideModule classes from the manifest are logged in Glide's
    // singleton.
    for (String glideModule : libraryGlideModuleClassNames) {
      ClassName moduleClassName = ClassName.bestGuess(glideModule);
      if (excludedGlideModuleClassNames.contains(glideModule)) {
  ...
  private Set<String>
  getExcludedGlideModuleClassNames(TypeElement appGlideModule) {

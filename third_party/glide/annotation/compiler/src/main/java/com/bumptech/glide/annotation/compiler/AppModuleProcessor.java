...
final class AppModuleProcessor {
  ...
  AppModuleProcessor(ProcessingEnvironment processingEnv,
    ...
    requestManagerGenerator =
        new RequestManagerGenerator(processingEnv, processorUtil);
    ...
    requestBuilderGenerator =
        new RequestBuilderGenerator(processingEnv, processorUtil);
  }

  void processModules(Set<? extends TypeElement> set, RoundEnvironment env) {
    for (TypeElement element :
         processorUtil.getElementsFor(GlideModule.class, env)) {
  ...
  boolean maybeWriteAppModule() {
    // appGlideModules is added to in order to catch errors where multiple
    // AppGlideModules may be present for a single application or library.
    // Because we only add to appGlideModules, we use
    // isGeneratedAppGlideModuleWritten to make sure the GeneratedAppGlideModule
    ...
    // If this package is null, it means there are no classes with this package
    // name. One way this could happen is if we process an annotation and reach
    // this point without writing something to the package. We do not error
    // check here because that shouldn't happen with the current implementation.
    PackageElement glideGenPackage =
        processingEnv.getElementUtils().getPackageElement(
            COMPILER_PACKAGE_NAME);
    FoundIndexedClassNames indexedClassNames =
        getIndexedClassNames(glideGenPackage);

    // Write all generated code to the package containing the AppGlideModule.
    // Doing so fixes classpath collisions if more than one Application
    // containing a AppGlideModule is included in a project.
    String generatedCodePackageName =
        appModule.getEnclosingElement().toString();

    TypeSpec generatedRequestOptions = requestOptionsGenerator.generate(
        generatedCodePackageName, indexedClassNames.extensions);
    writeRequestOptions(generatedCodePackageName, generatedRequestOptions);

    TypeSpec generatedRequestBuilder = requestBuilderGenerator.generate(
        generatedCodePackageName, generatedRequestOptions);
    ...
    TypeSpec requestManager = requestManagerGenerator.generate(
        generatedCodePackageName, generatedRequestOptions,
        generatedRequestBuilder, indexedClassNames.extensions);
    writeRequestManager(generatedCodePackageName, requestManager);

    TypeSpec requestManagerFactory = requestManagerFactoryGenerator.generate(
        generatedCodePackageName, requestManager);
    writeRequestManagerFactory(requestManagerFactory);

    TypeSpec glide = glideGenerator.generate(
        generatedCodePackageName, getGlideName(appModule), requestManager);
    ...
    processorUtil.infoLog("Wrote GeneratedAppGlideModule with: " +
  ...
  private FoundIndexedClassNames
  getIndexedClassNames(PackageElement glideGenPackage) {
    Set<String> glideModules = new HashSet<>();
    Set<String> extensions = new HashSet<>();
    List<? extends Element> glideGeneratedElements =
        glideGenPackage.getEnclosedElements();
    for (Element indexer : glideGeneratedElements) {
      Index annotation = indexer.getAnnotation(Index.class);
      // If the annotation is null, it means we've come across another class in
      // the same package that we can safely ignore.
      if (annotation != null) {
  ...
  private void writeRequestManager(String packageName,
  ...
  private void writeRequestManagerFactory(TypeSpec requestManagerFactory) {
    processorUtil.writeClass(
        AppModuleGenerator.GENERATED_ROOT_MODULE_PACKAGE_NAME,
        requestManagerFactory);
  }

  // The app module we generate subclasses a package private class. We don't
  // care about classpath collisions in IDEs since this class isn't an API
  // class.
  private void writeAppModule(TypeSpec appModule) {
    processorUtil.writeClass(
        AppModuleGenerator.GENERATED_ROOT_MODULE_PACKAGE_NAME, appModule);
  }

  private void writeRequestOptions(String packageName,
                                   TypeSpec requestOptions) {
    processorUtil.writeClass(packageName, requestOptions);
  }

  private void writeRequestBuilder(String packageName,
  ...
  private static final class FoundIndexedClassNames {
    ...
    private FoundIndexedClassNames(Set<String> glideModules,

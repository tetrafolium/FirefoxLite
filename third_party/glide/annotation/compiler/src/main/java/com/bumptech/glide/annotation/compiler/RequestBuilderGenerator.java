...
/**
 * Generates a {@link com.bumptech.glide.RequestBuilder} subclass containing all
 * methods from the base class, all methods from {@link
 * com.bumptech.glide.request.RequestOptions} and all non-override {@link
 * GlideOption} annotated methods in {@link GlideExtension} annotated classes.
 *
 * <p>Generated code looks like this:
 * <pre>
 * <code>
 * public final class GlideRequest<TranscodeType> extends
 * RequestBuilder<TranscodeType> { GlideRequest(Class<TranscodeType>
 * transcodeClass, RequestBuilder<?> other) { super(transcodeClass, other);
 *   }
 *
 ...
 * getMutableOptions()).dontAnimate(); } else { this.requestOptions = new
 * GlideOptions().apply(this.requestOptions).dontAnimate();
 *     }
 *     return this;
 *   }
 *
...
final class RequestBuilderGenerator {
  ...
  private static final ImmutableSet<String>
      EXCLUDED_METHODS_FROM_BASE_REQUEST_OPTIONS =
  ...
  RequestBuilderGenerator(ProcessingEnvironment processingEnv,
                          ProcessorUtil processorUtil) {
    this.processingEnv = processingEnv;
    this.processorUtil = processorUtil;

    requestBuilderType = processingEnv.getElementUtils().getTypeElement(
  ...
  TypeSpec generate(String generatedCodePackageName,
    ...
    generatedRequestBuilderClassName = ClassName.get(
        generatedCodePackageName, GENERATED_REQUEST_BUILDER_SIMPLE_NAME);
    generatedRequestBuilderOfTranscodeType = ParameterizedTypeName.get(
        generatedRequestBuilderClassName, transcodeTypeName);
    ...
    if (generatedOptions != null) {
      ...
      requestOptionsClassName =
          ClassName.get(RequestOptionsGenerator.REQUEST_OPTIONS_PACKAGE_NAME,
                        RequestBuilderGenerator.REQUEST_OPTIONS_SIMPLE_NAME);
    }

    ParameterizedTypeName requestBuilderOfTranscodeType =
        ParameterizedTypeName.get(ClassName.get(REQUEST_BUILDER_PACKAGE_NAME,
    ...
    return TypeSpec.classBuilder(GENERATED_REQUEST_BUILDER_SIMPLE_NAME)
        ...
        .addMethods(
            generateGeneratedRequestOptionsEquivalents(generatedOptions))
        .addMethods(generateRequestBuilderOverrides())
        .build();
  }

  /**
   * Generates overrides of all methods in {@link
   * com.bumptech.glide.RequestBuilder} that return
   * {@link com.bumptech.glide.RequestBuilder} so that they return our generated
   * subclass instead.
   */
  private List<MethodSpec> generateRequestBuilderOverrides() {
    ...
    return Lists.transform(processorUtil.findInstanceMethodsReturning(
                               requestBuilderType, rawRequestBuilderType),
  ...
  /**
   * Generates an override of a particular method in {@link
   * com.bumptech.glide.RequestBuilder} that returns {@link
   * com.bumptech.glide.RequestBuilder} so that it returns our generated
   * subclass instead.
   */
  private MethodSpec
  generateRequestBuilderOverride(ExecutableElement methodToOverride) {
    // We've already verified that this method returns a RequestBuilder and
    // RequestBuilders have exactly one type argument, so this is safe unless
    // those assumptions change.
    TypeMirror typeArgument = ((DeclaredType)methodToOverride.getReturnType())
                                  .getTypeArguments()
                                  .get(0);

    ParameterizedTypeName generatedRequestBuilderOfType =
        ParameterizedTypeName.get(generatedRequestBuilderClassName,
  ...
  /**
   * Generates methods with equivalent names and arguments to methods annotated
   * with
   * {@link GlideOption} in
   * {@link com.bumptech.glide.annotation.GlideExtension}s that return our
   * generated
  ...
  /**
   * Returns {@code true} if the given {@link MethodSpec} is a useful method to
   * have in our
   * {@link com.bumptech.glide.RequestBuilder} subclass.
   *
   * <p>Only newly generated methods will be included in the generated
   * {@link com.bumptech.glide.request.BaseRequestBuilder} subclass, so we only
   * have to filter out methods that override other methods to avoid duplicates.
   */
  private boolean
  isUsefulGeneratedRequestOption(MethodSpec requestOptionMethod) {
    return !EXCLUDED_METHODS_FROM_BASE_REQUEST_OPTIONS.contains(
        ...
        requestOptionMethod.returnType.toString().equals(
            requestOptionsClassName.toString());
  }

  /**
   * Generates a particular method with  an equivalent name and arguments to the
   * given method from the generated {@link
   * com.bumptech.glide.request.BaseRequestBuilder} subclass.
   */
  private MethodSpec
  generateGeneratedRequestOptionEquivalent(MethodSpec requestOptionMethod) {
    ...
    MethodSpec.Builder result =
        MethodSpec.methodBuilder(requestOptionMethod.name)
                    ...
                    .filter(new Predicate<AnnotationSpec>() {
                      ...
                      public boolean apply(AnnotationSpec input) {
                        return !input.type.equals(TypeName.get(Override.class))
                            ...
                            // GlideRequest is non-final to allow for mocking.
                            &&
                            !input.type.equals(TypeName.get(SafeVarargs.class))
                            // @CheckResult isn't applicable for RequestBuilder
                            // because there is no autoClone() in
                            // RequestBuilder.
            ...
            .beginControlFlow("if (getMutableOptions() instanceof $T)",
                              requestOptionsClassName)
            ...
            .addCode(CodeBlock.of(
                "this.requestOptions = new $T().apply(this.requestOptions)",
                requestOptionsClassName))
            .addCode(callRequestOptionsMethod)
            .endControlFlow()
  ...
  private List<MethodSpec> generateConstructors() {
    ParameterizedTypeName classOfTranscodeType = ParameterizedTypeName.get(
        ClassName.get(Class.class), transcodeTypeName);

    TypeName wildcardOfObject = WildcardTypeName.subtypeOf(Object.class);
    ParameterizedTypeName requestBuilderOfWildcardOfObject =
        ParameterizedTypeName.get(ClassName.get(requestBuilderType),
                                  wildcardOfObject);
  ...
  /**
   * Overrides the protected downloadOnly method in {@link
   * com.bumptech.glide.RequestBuilder} to return our generated subclass
   * instead.
   */

...
/**
 * Generates a Glide look-alike that acts as the entry point to the generated
 * API (GlideApp.with(...)).
 *
 * <p>>Generated {@link com.bumptech.glide.Glide} look-alikes look like this
 * (note that the name is configurable in {@link
 * com.bumptech.glide.annotation.GlideModule}): <pre> <code> public final class
 * GlideApp { private GiphyGlide() {
 *   }
 *
 ...
 *   }
 *
 *   public static GeneratedRequestManager with(androidx.fragment.app.Fragment
 * fragment) { return (GeneratedRequestManager) Glide.with(fragment);
 *   }
 * </code>
 * </pre>
 */
final class GlideGenerator {
  ...
  GlideGenerator(ProcessingEnvironment processingEnv,
  ...
  TypeSpec generate(String generatedCodePackageName, String glideName,
    ...
    return TypeSpec.classBuilder(glideName)
        ...
        .addMethod(MethodSpec.constructorBuilder()
                       ...
                       .build())
        .addMethods(generateOverridesForGlideMethods(generatedCodePackageName,
                                                     generatedRequestManager))
        .build();
  }

  private List<MethodSpec>
  generateOverridesForGlideMethods(final String generatedCodePackageName,
  ...
  private MethodSpec
  overrideGlideStaticMethod(ExecutableElement methodToOverride) {
    List<? extends VariableElement> parameters =
        methodToOverride.getParameters();

    TypeElement element = (TypeElement)processingEnv.getTypeUtils().asElement(
    ...
    MethodSpec.Builder builder =
        MethodSpec.methodBuilder(methodToOverride.getSimpleName().toString())
            ...
            .addJavadoc(
                processorUtil.generateSeeMethodJavadoc(methodToOverride))
            .addParameters(Lists.transform(
                parameters, new Function<VariableElement, ParameterSpec>() {
                  @Override
    ...
    TypeElement visibleForTestingType =
        processingEnv.getElementUtils().getTypeElement(
            VISIBLE_FOR_TESTING_QUALIFIED_NAME);
    for (AnnotationMirror mirror : methodToOverride.getAnnotationMirrors()) {
      builder.addAnnotation(AnnotationSpec.get(mirror));

      // Suppress a lint warning if we're overriding a VisibleForTesting method.
      // See #1977.
      if (mirror.getAnnotationType().asElement().equals(
              visibleForTestingType)) {
        builder.addAnnotation(
            AnnotationSpec
                .builder(ClassName.get(SUPPRESS_LINT_PACKAGE_NAME,
  ...
  private MethodSpec
  overrideGlideWithMethod(String packageName, TypeSpec generatedRequestManager,
    ...
    return MethodSpec.methodBuilder(methodToOverride.getSimpleName().toString())
        ...
        .addStatement("return ($T) $T.$N($L)", generatedRequestManagerClassName,
                      glideType, methodToOverride.getSimpleName().toString(),

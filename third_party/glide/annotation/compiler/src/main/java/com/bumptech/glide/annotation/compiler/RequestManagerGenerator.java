...
/**
 * Generates an implementation of {@link com.bumptech.glide.RequestManager} that
 * contains generated methods from {@link GlideExtension}s and {@link
 * GlideType}.
 *
 * <p>Generated {@link com.bumptech.glide.RequestManager} implementations look
 * like this: <pre> <code> public final class GeneratedRequestManager extends
 * RequestManager { GeneratedRequestManager(Glide glide, Lifecycle lifecycle,
 * RequestManagerTreeNode treeNode) { super(glide, lifecycle, treeNode);
 *   }
 *
...
final class RequestManagerGenerator {
  ...
  RequestManagerGenerator(ProcessingEnvironment processingEnv,
    ...
    requestManagerType =
        elementUtils.getTypeElement(REQUEST_MANAGER_QUALIFIED_NAME);
    ...
    requestBuilderType = elementUtils.getTypeElement(
  ...
  TypeSpec generate(String generatedCodePackageName,
                    @Nullable TypeSpec requestOptions, TypeSpec requestBuilder,
  ...
  private List<MethodSpec> generateRequestManagerMethodOverrides() {
    // Without the erasure, this is a RequestBuilder<Y>. A RequestBuilder<X> is
    // not assignable to a RequestBuilder<Y>. After type erasure this is a
    // RequestBuilder. A RequestBuilder<X> is assignable to the raw
    // RequestBuilder.
  ...
  /**
   * Generates overrides of existing RequestManager methods so that they return
   * our generated RequestBuilder subtype.
   */
  private MethodSpec
  generateRequestManagerMethodOverride(ExecutableElement methodToOverride) {
  ...
  private List<MethodSpec>
  generateAdditionalRequestManagerMethods(Set<String> glideExtensions) {
    ...
    return Lists.transform(requestManagerExtensionMethods,
                           new Function<ExecutableElement, MethodSpec>() {
                             @Override
                             public MethodSpec apply(ExecutableElement input) {
                               return generateAdditionalRequestManagerMethod(
                                   input);
                             }
                           });
  }
  ...
  private MethodSpec
  generateAdditionalRequestManagerMethod(ExecutableElement extensionMethod) {
    String returnType = processorUtil
                            .findClassValuesFromAnnotationOnClassAsNames(
                                extensionMethod, GlideType.class)
                            .iterator()
                            .next();
    ClassName returnTypeClassName = ClassName.bestGuess(returnType);
    ParameterizedTypeName parameterizedTypeName = ParameterizedTypeName.get(
        generatedRequestBuilderClassName, returnTypeClassName);
    ...
    return MethodSpec.methodBuilder(extensionMethod.getSimpleName().toString())
        ...
        .addStatement("$T requestBuilder = this.as($T.class)",
                      parameterizedTypeName, returnTypeClassName)
  ...
  /**
   * The {@link com.bumptech.glide.request.RequestOptions} subclass should
   * always be our generated subclass type to avoid inadvertent errors where a
   * different subclass is applied that accidentally wipes out some logic in
   * overidden methods in our generated subclass.
   */
  @Nullable
  private MethodSpec generateOverrideSetRequestOptions(
      String generatedCodePackageName,
      @Nullable TypeSpec generatedRequestOptions) {
    if (generatedRequestOptions == null) {
      return null;
    }

    Elements elementUtils = processingEnv.getElementUtils();
    TypeElement requestOptionsType = elementUtils.getTypeElement(
    ...
    // This class may have just been generated and therefore may not be found if
    // we try to obtain it via Elements, so use just the String version instead.
    ...
    return MethodSpec.methodBuilder(methodName)
        ...
        .addParameter(
            ParameterSpec
                .builder(ClassName.get(requestOptionsType), parameterName)
                .addAnnotation(ClassName.get(androidNonNullType))
                .build())
        .beginControlFlow("if ($N instanceof $L)", parameterName,

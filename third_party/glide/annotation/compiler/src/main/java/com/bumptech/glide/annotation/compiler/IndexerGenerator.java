...
/**
 * Generates an empty class with an annotation containing the class names of one
 * or more LibraryGlideModules and/or one or more GlideExtensions.
 *
 * <p>We use a separate class so that LibraryGlideModules and GlideExtensions
 * written in libraries can be bundled into an AAR and later retrieved by the
 * annotation processor when it processes the AppGlideModule in an application.
 *
 * <p>The output file generated by this class with a LibraryGlideModule looks
 * like this: <pre> <code>
 ...
 *  )
 *  public class
 * Indexer_GlideModule_com_bumptech_glide_integration_okhttp3_OkHttpLibraryGlideModule
 *  {
 *  }
 * </code>
 * </pre>
 *
 * <p>The output file generated by this class with a GlideExtension looks like
 * this: <pre> <code>
 *  {@literal @com.bumptech.glide.annotation.compiler.Index(}
 *      extensions = "com.bumptech.glide.integration.gif.GifOptions"
 *  )
 *  public class
 * Indexer_GlideExtension_com_bumptech_glide_integration_gif_GifOptions {
 *  }
 * </code>
 * </pre>
 * </p>
...
final class IndexerGenerator {
  ...
  private static TypeSpec generate(List<TypeElement> libraryModules,
    ...
    for (TypeElement childModule : libraryModules) {
      annotationBuilder.addMember(value, "$S",
                                  ClassName.get(childModule).toString());
    }
  ...
  getAnnotationValue(Class<? extends Annotation> annotation) {
    if (annotation == GlideModule.class) {
      return "modules";
    } else if (annotation == GlideExtension.class) {
      return "extensions";
    } else {
      throw new IllegalArgumentException("Unrecognized annotation: " +
                                         annotation);
    }
  }
}

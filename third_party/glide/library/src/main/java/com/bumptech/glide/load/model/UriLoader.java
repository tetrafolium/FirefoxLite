...
/**
 * A ModelLoader for {@link android.net.Uri}s that handles local {@link
 * android.net.Uri}s directly and routes remote {@link android.net.Uri}s to a
 * wrapped
 * {@link com.bumptech.glide.load.model.ModelLoader} that handles
 * {@link com.bumptech.glide.load.model.GlideUrl}s.
 *
 * @param <Data> The type of data that will be retrieved for {@link
 *     android.net.Uri}s.
 */
public class UriLoader<Data> implements ModelLoader<Uri, Data> {
  private static final Set<String> SCHEMES =
      Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
          ContentResolver.SCHEME_FILE, ContentResolver.SCHEME_ANDROID_RESOURCE,
  ...
  /**
   * Factory for obtaining a {@link DataFetcher} for a data type for a
   * particular {@link Uri}.
   *
   * @param <Data> The type of data the returned {@link DataFetcher} will
   *     obtain.
   */
  ...
  public static class FileDescriptorFactory
    ...
    @Override
    public ModelLoader<Uri, ParcelFileDescriptor>
    build(MultiModelLoaderFactory multiFactory) {
      return new UriLoader<>(this);
    }

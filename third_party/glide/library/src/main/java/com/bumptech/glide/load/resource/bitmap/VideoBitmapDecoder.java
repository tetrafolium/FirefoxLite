...
/**
 * An {@link com.bumptech.glide.load.ResourceDecoder} that can decode a
 * thumbnail frame
 * {@link android.graphics.Bitmap} from a {@link
 * android.os.ParcelFileDescriptor} containing a video.
 *
...
public class VideoBitmapDecoder
  ...
  /**
   * A constant indicating we should use whatever frame we consider best,
   * frequently not the first frame.
   */
  public static final long DEFAULT_FRAME = -1;

  /**
   * A long indicating the time position (in microseconds) of the target frame
   * which will be retrieved. {@link
   * android.media.MediaMetadataRetriever#getFrameAtTime(long)} is used to
   * extract the video frame.
   *
   * <p>When retrieving the frame at the given time position, there is no
   * guarantee that the data source has a frame located at the position. When
   * this happens, a frame nearby will be returned. If the long is negative,
   * time position and option will ignored, and any frame that the
  ...
  public static final Option<Long> TARGET_FRAME = Option.disk(
      ...
      DEFAULT_FRAME, new Option.CacheKeyUpdater<Long>() {
        private final ByteBuffer buffer =
            ByteBuffer.allocate(Long.SIZE / Byte.SIZE);
        @Override
        public void update(byte[] keyBytes, Long value,
   ...
   *
   * <p>This option will be ignored if {@link #TARGET_FRAME} is not set or is
   * set to
   * {@link #DEFAULT_FRAME}.
   *
  ...
  public static final Option<Integer> FRAME_OPTION = Option.disk(
      ...
      null /*defaultValue*/, new Option.CacheKeyUpdater<Integer>() {
        private final ByteBuffer buffer =
            ByteBuffer.allocate(Integer.SIZE / Byte.SIZE);
        @Override
        public void update(byte[] keyBytes, Integer value,
                           MessageDigest messageDigest) {
          if (value == null) {
            return;
          }
  ...
  public boolean handles(ParcelFileDescriptor data, Options options) {
    // Calling setDataSource is expensive so avoid doing so unless we're
    // actually called. For non-videos this isn't any cheaper, but for videos it
    // safes the redundant call and 50-100ms.
    return true;
  }

  @Override
  public Resource<Bitmap> decode(ParcelFileDescriptor resource, int outWidth,
                                 int outHeight, Options options)
      throws IOException {
    ...
    try {
      ...
      // MediaMetadataRetriever APIs throw generic runtime exceptions when given
      // invalid data.

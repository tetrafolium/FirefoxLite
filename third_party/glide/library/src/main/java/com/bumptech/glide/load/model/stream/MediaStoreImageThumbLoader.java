...
/**
 * Loads {@link InputStream}s from media store image {@link Uri}s that point to
...
public class MediaStoreImageThumbLoader
  ...
  public LoadData<InputStream> buildLoadData(Uri model, int width, int height,
    ...
    if (MediaStoreUtil.isThumbnailSize(width, height)) {
      return new LoadData<>(new ObjectKey(model),
                            ThumbFetcher.buildImageFetcher(context, model));
    } else {
      return null;
    }
  }
  ...
  /**
   * Factory that loads {@link InputStream}s from media store image {@link
   * Uri}s.
   */

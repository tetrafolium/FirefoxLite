...
public final class GlideException extends Exception {
  ...
  public Throwable fillInStackTrace() {
    // Avoid an expensive allocation by doing nothing here. Causes should
    // contain all relevant stack traces.
    return this;
  }
   ...
   *
   * <p>Causes may or may not be {@link GlideException GlideExceptions}. Causes
   * may also not be root causes, and in turn my have been caused by other
   * failures.</p>
   *
   * @see #getRootCauses()
   */
  public List<Exception> getCauses() { return causes; }

  /**
   * Returns the list of root causes that are the leaf nodes of all children of
   * this exception.
   *
   * <p>Use this method to do things like look for http exceptions that indicate
   * the load may have failed due to an error that can be retried. Keep in mind
   * that because Glide may attempt to load a given model using multiple
   * different pathways, there may be multiple related or unrelated reasons for
   * a load to fail.
   */
   ...
   *
   * <p>Each root cause is logged separately to avoid throttling. {@link
   * #printStackTrace()} will provide a more succinct overview of why the
   * exception occurred, although it does not include complete stack traces.
   */
  ...
  public String getMessage() {
    return super.getMessage() + (dataClass != null ? ", " + dataClass : "") +
        (dataSource != null ? ", " + dataSource : "") +
        (key != null ? ", " + key : "");
  }

  // Appendable throws, PrintWriter, PrintStream, and IndentedAppendable do not,
  // so this should never happen.
  @SuppressWarnings("PMD.PreserveStackTrace")
  private static void appendExceptionMessage(Exception e,
                                             Appendable appendable) {
    try {
      appendable.append(e.getClass().toString())
          .append(": ")
          .append(e.getMessage())
          .append('\n');
  ...
  // so this should never happen.
  @SuppressWarnings("PMD.PreserveStackTrace")
  private static void appendCauses(List<Exception> causes,
                                   Appendable appendable) {
    try {
  ...
  private static void appendCausesWrapped(List<Exception> causes,
                                          Appendable appendable)
      throws IOException {
  ...
  private static final class IndentedAppendable implements Appendable {
    ...
    IndentedAppendable(Appendable appendable) { this.appendable = appendable; }
    ...
    @Override
    public Appendable append(@Nullable CharSequence charSequence, int start,
      ...
      printedNewLine =
          charSequence.length() > 0 && charSequence.charAt(end - 1) == '\n';

...
public class HttpUrlFetcher implements DataFetcher<InputStream> {
  ...
  // Visible for testing.
  HttpUrlFetcher(GlideUrl glideUrl, int timeout,
  ...
  public void loadData(Priority priority,
    ...
    try {
      result = loadDataWithRedirects(glideUrl.toURL(), 0 /*redirects*/,
    ...
    if (Log.isLoggable(TAG, Log.VERBOSE)) {
      Log.v(TAG, "Finished http url fetcher fetch in " +
                     LogTime.getElapsedMillis(startTime) + " ms and loaded " +
                     result);
    }
  ...
  private InputStream loadDataWithRedirects(URL url, int redirects, URL lastUrl,
    ...
    // Stop the urlConnection instance of HttpUrlConnection from following
    // redirects so that redirects will be handled by recursive calls to this
    // method, loadDataWithRedirects.
    urlConnection.setInstanceFollowRedirects(false);

    // Connect explicitly to avoid errors in decoders if connection fails.
    urlConnection.connect();
    // Set the stream so that it's closed in cleanup to avoid resource leaks.
    // See #2352.
    ...
    if (statusCode / 100 == 2) {
      ...
      // Closing the stream specifically is required to avoid leaking
      // ResponseBodys in addition to disconnecting the url connection below.
      // See #2352.
      cleanup();
  ...
  @Override
  public void cancel() {
    // TODO: we should consider disconnecting the url connection here, but we
    // can't do so directly because cancel is often called on the main thread.
    isCancelled = true;
  }

...
/**
 * A collection of static methods for creating new {@link
 * com.bumptech.glide.RequestManager}s or retrieving existing ones from
...
public class RequestManagerRetriever implements Handler.Callback {
  ...
  // Hacks based on the implementation of FragmentManagerImpl in the non-support
  // libraries that allow us to iterate over and retrieve all active Fragments
  // in a FragmentManager.
  ...
  private final ArrayMap<View, Fragment> tempViewToSupportFragment =
      new ArrayMap<>();
  private final ArrayMap<View, android.app.Fragment> tempViewToFragment =
      new ArrayMap<>();
    ...
    if (applicationManager == null) {
      synchronized (this) {
        if (applicationManager == null) {
          ...
          // the fragment or activity. However, in this case since the manager
          // attached to the application will not receive lifecycle events, we
          // must force the manager to start resumed using ApplicationLifecycle.
  ...
  public RequestManager get(View view) {
    ...
    // Support Fragments.
    // Although the user might have non-support Fragments attached to
    // FragmentActivity, searching for non-support Fragments is so expensive pre
    // O and that should be rare enough that we prefer to just fall back to the
    // Activity directly.
  ...
  private static void findAllSupportFragmentsWithViews(
    ...
    for (Fragment fragment : topLevelFragments) {
      // getFragment()s in the support FragmentManager may contain null values,
      // see #1991.
      ...
      findAllSupportFragmentsWithViews(
          fragment.getChildFragmentManager().getFragments(), result);
    }
  }
  ...
  private android.app.Fragment findFragment(View target, Activity activity) {
    ...
    findAllFragmentsWithViews(activity.getFragmentManager(),
                              tempViewToFragment);
  ...
  // TODO: Consider using an accessor class in the support library package to
  // more directly retrieve non-support Fragments.
  @TargetApi(Build.VERSION_CODES.O)
  private void
  findAllFragmentsWithViews(android.app.FragmentManager fragmentManager,
  ...
  private void
  findAllFragmentsWithViewsPreO(android.app.FragmentManager fragmentManager,
                                ArrayMap<View, android.app.Fragment> result) {
    int index = 0;
    while (true) {
  ...
  getRequestManagerFragment(final android.app.FragmentManager fm,
  ...
  getSupportRequestManagerFragment(final FragmentManager fm,
                                   Fragment parentHint) {
    ...
    if (current == null) {
      ...
      if (current == null) {
        ...
        fm.beginTransaction()
            .add(current, FRAGMENT_TAG)
            .commitAllowingStateLoss();
        handler.obtainMessage(ID_REMOVE_SUPPORT_FRAGMENT_MANAGER, fm)
            .sendToTarget();
      }
    }
    return current;
  ...
  public interface RequestManagerFactory {
    RequestManager build(Glide glide, Lifecycle lifecycle,

...
/**
 * Maintains an ordered put of {@link ModelLoader}s and the model and data types
 * they handle in order from highest priority to lowest.
 */
public class ModelLoaderRegistry {
  ...
  public synchronized<Model, Data> void
  ...
  public synchronized<Model, Data> void remove(Class<Model> modelClass,
  ...
  private <Model, Data> void
  tearDown(List<ModelLoaderFactory<Model, Data>> factories) {
  ...
  public synchronized<A> List<ModelLoader<A, ?>> getModelLoaders(A model) {
    List<ModelLoader<A, ?>> modelLoaders =
  ...
  private <A> List<ModelLoader<A, ?>>
  getModelLoadersForClass(Class<A> modelClass) {
    List<ModelLoader<A, ?>> loaders = cache.get(modelClass);
    if (loaders == null) {
      loaders = Collections.unmodifiableList(
  ...
  private static class ModelLoaderCache {
    ...
    public void clear() { cachedModelLoaders.clear(); }

    public <Model> void put(Class<Model> modelClass,
                            List<ModelLoader<Model, ?>> loaders) {
      Entry<?> previous =
          cachedModelLoaders.put(modelClass, new Entry<>(loaders));
      if (previous != null) {
        throw new IllegalStateException("Already cached loaders for model: " +
                                        modelClass);
      }
    }

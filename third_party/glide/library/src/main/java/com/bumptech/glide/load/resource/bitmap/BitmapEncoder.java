...
/**
 * An {@link com.bumptech.glide.load.ResourceEncoder} that writes {@link
 * android.graphics.Bitmap}s to {@link java.io.OutputStream}s.
 *
 ...
 *
 * @see android.graphics.Bitmap#compress(android.graphics.Bitmap.CompressFormat,
 *     int,
 * java.io.OutputStream)
 */
public class BitmapEncoder implements ResourceEncoder<Bitmap> {
  /**
   * An integer option between 0 and 100 that is used as the compression
   * quality.
   *
   * <p> Defaults to 90. </p>
   */
  ...
  /**
   * An {@link android.graphics.Bitmap.CompressFormat} option used as the format
   * to encode the {@link android.graphics.Bitmap}.
   *
   * <p> Defaults to {@link android.graphics.Bitmap.CompressFormat#JPEG} for
   * images without alpha and {@link android.graphics.Bitmap.CompressFormat#PNG}
   * for images with alpha. </p>
   */
  ...
  public boolean encode(Resource<Bitmap> resource, File file, Options options) {
    ...
    TraceCompat.beginSection("encode: [" + bitmap.getWidth() + "x" +
                             bitmap.getHeight() + "] " + format);
    try {
      ...
      if (Log.isLoggable(TAG, Log.VERBOSE)) {
        Log.v(TAG, "Compressed with type: " + format + " of size " +
                       Util.getBitmapByteSize(bitmap) + " in " +

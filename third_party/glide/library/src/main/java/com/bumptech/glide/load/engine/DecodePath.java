 ...
 *
 * @param <DataType>     The type of data ResourceType that will be decoded
 *     from.
 * @param <ResourceType> The type of intermediate resource that will be decoded.
 * @param <Transcode>    The final type of resource that will be transcoded from
 *     ResourceType and
...
public class DecodePath<DataType, ResourceType, Transcode> {
  ...
  private final List<? extends ResourceDecoder<DataType, ResourceType>>
      decoders;
  ...
  private Resource<ResourceType> decodeResource(DataRewinder<DataType> rewinder,
    ...
    try {
      return decodeResourceWithList(rewinder, width, height, options,
                                    exceptions);
    } finally {
  ...
  decodeResourceWithList(DataRewinder<DataType> rewinder, int width, int height,
                         Options options, List<Exception> exceptions)
      throws GlideException {
    ...
    for (int i = 0, size = decoders.size(); i < size; i++) {
      ...
      try {
        ...
        // Some decoders throw unexpectedly. If they do, we shouldn't fail the
        // entire load path, but instead log and continue. See #2406 for an
        // example.
  ...
  public String toString() {
    return "DecodePath{"
        + " dataClass=" + dataClass + ", decoders=" + decoders +

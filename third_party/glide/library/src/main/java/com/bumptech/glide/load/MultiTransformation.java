...
/**
 * A transformation that applies one or more transformations in iteration order
 * to a resource.
 *
 * @param <T> The type of {@link com.bumptech.glide.load.engine.Resource} that
 *     will be transformed.
 */
public class MultiTransformation<T> implements Transformation<T> {
  ...
  public MultiTransformation(
      Collection<? extends Transformation<T>> transformationList) {
  ...
  public Resource<T> transform(Context context, Resource<T> resource,
    ...
    for (Transformation<T> transformation : transformations) {
      Resource<T> transformed =
          transformation.transform(context, previous, outWidth, outHeight);
      if (previous != null && !previous.equals(resource) &&
          !previous.equals(transformed)) {
        previous.recycle();
      }

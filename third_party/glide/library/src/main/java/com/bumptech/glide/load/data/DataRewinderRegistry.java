...
/**
 * Stores a mapping of data class to {@link
 * com.bumptech.glide.load.data.DataRewinder.Factory} and allows registration of
 * new types and factories.
 */
public class DataRewinderRegistry {
  ...
  public synchronized<T> DataRewinder<T> build(T data) {
    ...
    DataRewinder.Factory<T> result =
        (DataRewinder.Factory<T>)rewinders.get(data.getClass());
    if (result == null) {
      for (DataRewinder.Factory<?> registeredFactory : rewinders.values()) {
        if (registeredFactory.getDataClass().isAssignableFrom(
                data.getClass())) {
  ...
  private static class DefaultRewinder implements DataRewinder<Object> {
    ...
    public DefaultRewinder(Object data) { this.data = data; }

...
/**
 * An {@link java.io.InputStream} that catches {@link java.io.IOException}s
 * during read and skip calls and stores them so they can later be handled or
 * thrown. This class is a workaround for a framework issue where exceptions
 * during reads while decoding bitmaps in {@link android.graphics.BitmapFactory}
...
public class ExceptionCatchingInputStream extends InputStream {
  ...
  public static ExceptionCatchingInputStream obtain(InputStream toWrap) {
    ...
    synchronized (QUEUE) { result = QUEUE.poll(); }
    if (result == null) {
  ...
  void setInputStream(InputStream toWrap) { wrapped = toWrap; }
  ...
  public void release() {
    exception = null;
    wrapped = null;
    synchronized (QUEUE) { QUEUE.offer(this); }
  }
}

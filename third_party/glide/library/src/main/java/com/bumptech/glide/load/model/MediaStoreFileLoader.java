...
public final class MediaStoreFileLoader implements ModelLoader<Uri, File> {
  ...
  MediaStoreFileLoader(Context context) { this.context = context; }

  @Override
  public LoadData<File> buildLoadData(Uri uri, int width, int height,
                                      Options options) {
    return new LoadData<>(new ObjectKey(uri),
                          new FilePathFetcher(context, uri));
  }
  ...
  private static class FilePathFetcher implements DataFetcher<File> {
    ...
    public void loadData(Priority priority,
                         DataCallback<? super File> callback) {
      Cursor cursor = context.getContentResolver().query(
          uri, PROJECTION, null /*selection*/, null /*selectionArgs*/,
          null /*sortOrder*/);
      ...
      if (TextUtils.isEmpty(filePath)) {
        callback.onLoadFailed(
            new FileNotFoundException("Failed to find file path for: " + uri));
      } else {

...
public final class GlideExecutor extends ThreadPoolExecutor {
  ...
  /**
   * The default thread name prefix for executors used to load/decode/transform
   * data not found in cache.
   */
  public static final String DEFAULT_SOURCE_EXECUTOR_NAME = "source";
  /**
   * The default thread name prefix for executors used to load/decode/transform
   * data found in Glide's cache.
   */
  public static final String DEFAULT_DISK_CACHE_EXECUTOR_NAME = "disk-cache";
  /**
   * The default thread count for executors used to load/decode/transform data
   * found in Glide's cache.
   */
  ...
  // Don't use more than four threads when automatically determining thread
  // count..
  private static final int MAXIMUM_AUTOMATIC_THREAD_COUNT = 4;
  private final boolean executeSynchronously;

  /**
   * The default thread name prefix for executors from unlimited thread pool
   * used to load/decode/transform data not found in cache.
   */
  private static final String SOURCE_UNLIMITED_EXECUTOR_NAME =
      "source-unlimited";
  /**
   * The default keep alive time for threads in source unlimited executor pool
   * in milliseconds.
   */
   ...
   * {@link #calculateBestThreadCount()}, the {@link
   * #DEFAULT_DISK_CACHE_EXECUTOR_NAME} thread name prefix, and the
   * {@link
   ...
   * Returns a new fixed thread pool with the default thread count returned from
   * {@link #calculateBestThreadCount()}, the {@link
   * #DEFAULT_DISK_CACHE_EXECUTOR_NAME} thread name prefix, and a custom
   * {@link
   ...
   * com.bumptech.glide.load.engine.executor.GlideExecutor.UncaughtThrowableStrategy}
   * to use to handle uncaught exceptions.
   */
  public static GlideExecutor
  newDiskCacheExecutor(UncaughtThrowableStrategy uncaughtThrowableStrategy) {
    return newDiskCacheExecutor(DEFAULT_DISK_CACHE_EXECUTOR_THREADS,
                                DEFAULT_DISK_CACHE_EXECUTOR_NAME,
  ...
  /**
   * Returns a new fixed thread pool with the given thread count, thread name
   * prefix, and {@link
   * com.bumptech.glide.load.engine.executor.GlideExecutor.UncaughtThrowableStrategy}.
   *
   * <p>Disk cache executors do not allow network operations on their threads.
   ...
   * Returns a new fixed thread pool with the default thread count returned from
   * {@link #calculateBestThreadCount()}, the {@link
   * #DEFAULT_SOURCE_EXECUTOR_NAME} thread name prefix, and the
   * {@link
   * com.bumptech.glide.load.engine.executor.GlideExecutor.UncaughtThrowableStrategy#DEFAULT}
   * uncaught throwable strategy.
   *
  ...
  public static GlideExecutor newSourceExecutor() {
    return newSourceExecutor(calculateBestThreadCount(),
   ...
   * Returns a new fixed thread pool with the default thread count returned from
   * {@link #calculateBestThreadCount()}, the {@link
   * #DEFAULT_SOURCE_EXECUTOR_NAME} thread name prefix, and a custom
   * {@link
   * com.bumptech.glide.load.engine.executor.GlideExecutor.UncaughtThrowableStrategy}
   * uncaught throwable strategy.
   *
   ...
   */
  public static GlideExecutor
  newSourceExecutor(UncaughtThrowableStrategy uncaughtThrowableStrategy) {
    return newDiskCacheExecutor(DEFAULT_DISK_CACHE_EXECUTOR_THREADS,
                                DEFAULT_DISK_CACHE_EXECUTOR_NAME,
                                uncaughtThrowableStrategy);
  }
  ...
  /**
   * Returns a new unlimited thread pool with zero core thread count to make
   * sure no threads are created by default, {@link
   * #SOURCE_UNLIMITED_EXECUTOR_KEEP_ALIVE_TIME_MS} keep alive time, the {@link
   * #SOURCE_UNLIMITED_EXECUTOR_NAME} thread name prefix, the
   * {@link
   * com.bumptech.glide.load.engine.executor.GlideExecutor.UncaughtThrowableStrategy#DEFAULT}
   * uncaught throwable strategy, and the {@link SynchronousQueue} since using
   * default unbounded blocking queue, for example, {@link
   * PriorityBlockingQueue} effectively won't create more than
  ...
  public static GlideExecutor newUnlimitedSourceExecutor() {
    return new GlideExecutor(
        0 /* corePoolSize */, Integer.MAX_VALUE /* maximumPoolSize */,
        SOURCE_UNLIMITED_EXECUTOR_KEEP_ALIVE_TIME_MS,
        SOURCE_UNLIMITED_EXECUTOR_NAME, UncaughtThrowableStrategy.DEFAULT,
  ...
  GlideExecutor(int poolSize, String name,
    ...
    this(poolSize /* corePoolSize */, poolSize /* maximumPoolSize */,
         0 /* keepAliveTimeInMs */, name, uncaughtThrowableStrategy,
         preventNetworkOperations, executeSynchronously);
  }

  GlideExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTimeInMs,
                String name,
                UncaughtThrowableStrategy uncaughtThrowableStrategy,
                boolean preventNetworkOperations,
                boolean executeSynchronously) {
    this(corePoolSize, maximumPoolSize, keepAliveTimeInMs, name,
         uncaughtThrowableStrategy, preventNetworkOperations,
         executeSynchronously, new PriorityBlockingQueue<Runnable>());
  }

  GlideExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTimeInMs,
                String name,
                UncaughtThrowableStrategy uncaughtThrowableStrategy,
                boolean preventNetworkOperations, boolean executeSynchronously,
                BlockingQueue<Runnable> queue) {
    super(corePoolSize, maximumPoolSize, keepAliveTimeInMs,
          TimeUnit.MILLISECONDS, queue,
          new DefaultThreadFactory(name, uncaughtThrowableStrategy,
   ...
   *
   * <p>{@link Runtime#availableProcessors()} returns the number of awake cores,
   * which may not be the number of available cores depending on the device's
   * current state. See http://goo.gl/8H670N.
   */
  public static int calculateBestThreadCount() {
    ...
    return Math.min(MAXIMUM_AUTOMATIC_THREAD_COUNT,
                    Math.max(availableProcessors, cpuCount));
  }

  /**
   * A strategy for handling unexpected and uncaught {@link Throwable}s thrown
   * by futures run on the pool.
   */
  ...
  /**
   * A {@link java.util.concurrent.ThreadFactory} that builds threads slightly
   * above priority {@link android.os.Process#THREAD_PRIORITY_BACKGROUND}.
   */

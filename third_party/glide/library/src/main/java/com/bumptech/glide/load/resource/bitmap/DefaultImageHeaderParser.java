...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
  ...
  private static final int[] BYTES_PER_FORMAT = {0, 1, 1, 2, 4, 8, 1,
                                                 1, 2, 4, 8, 4, 8};
  // WebP-related
  // "RIFF"
  ...
  private ImageType getType(Reader reader) throws IOException {
    ...
    final int firstFourBytes =
        firstTwoBytes << 16 & 0xFFFF0000 | reader.getUInt16() & 0xFFFF;
    // PNG.
    if (firstFourBytes == PNG_HEADER) {
      // See:
      // http://stackoverflow.com/questions/2057923/how-to-check-a-png-for-grayscale-alpha
      // -color-type
      reader.skip(25 - 4);
    ...
    // WebP (reads up to 21 bytes). See
    // https://developers.google.com/speed/webp/docs/riff_container for details.
    ...
    if ((fourthFourBytes & VP8_HEADER_TYPE_MASK) == VP8_HEADER_TYPE_EXTENDED) {
      ...
      reader.skip(4);
      return (reader.getByte() & WEBP_EXTENDED_ALPHA_FLAG) != 0
          ? ImageType.WEBP_A
          : ImageType.WEBP;
    }
    if ((fourthFourBytes & VP8_HEADER_TYPE_MASK) == VP8_HEADER_TYPE_LOSSLESS) {
      // See
      // chromium.googlesource.com/webm/libwebp/+/master/doc/webp-lossless-bitstream-spec.txt
      // for more info.
      reader.skip(4);
      return (reader.getByte() & WEBP_LOSSLESS_ALPHA_FLAG) != 0
          ? ImageType.WEBP_A
          : ImageType.WEBP;
    }
  ...
  /**
   * Parse the orientation from the image header. If it doesn't handle this
   * image type (or this is not an image) it will return a default value rather
   * than throwing an exception.
   *
   * @return The exif orientation if present or -1 if the header couldn't be
   *     parsed or doesn't
   * contain an orientation
   * @throws IOException
   */
  private int getOrientation(Reader reader, ArrayPool byteArrayPool)
      throws IOException {
  ...
  private int parseExifSegment(Reader reader, byte[] tempArray,
                               int exifSegmentLength) throws IOException {
    ...
    boolean hasJpegExifPreamble =
        hasJpegExifPreamble(tempArray, exifSegmentLength);
    if (hasJpegExifPreamble) {
      return parseExifSegment(
          new RandomAccessReader(tempArray, exifSegmentLength));
    } else {
      if (Log.isLoggable(TAG, Log.DEBUG)) {
  ...
  private boolean hasJpegExifPreamble(byte[] exifData, int exifSegmentLength) {
    boolean result =
        exifData != null &&
        exifSegmentLength > JPEG_EXIF_SEGMENT_PREAMBLE_BYTES.length;
    if (result) {
  ...
  /**
   * Moves reader to the start of the exif segment and returns the length of the
   * exif segment or
  ...
  private static int parseExifSegment(RandomAccessReader segmentData) {
    ...
    int firstIfdOffset =
        segmentData.getInt32(headerOffsetSize + 4) + headerOffsetSize;
    ...
    for (int i = 0; i < tagCount; i++) {
      ...
      if (byteCount < 0 || tagValueOffset + byteCount > segmentData.length()) {
        if (Log.isLoggable(TAG, Log.DEBUG)) {
          Log.d(TAG,
                "Illegal number of bytes for TI tag data tagType=" + tagType);
        }
        continue;
      }
  ...
  private static final class RandomAccessReader {
    ...
    void order(ByteOrder byteOrder) { this.data.order(byteOrder); }

    int length() { return data.remaining(); }
  ...
  private static final class StreamReader implements Reader {
    ...
    public long skip(long total) throws IOException {
      ...
      while (toSkip > 0) {
        ...
        if (skipped > 0) {
          ...
          // Skip has no specific contract as to what happens when you reach the
          // end of the stream. To differentiate between temporarily not having
          // more data and having finished the stream, we read a single byte
          // when we fail to skip any amount of data.
    ...
    public int read(byte[] buffer, int byteCount) throws IOException {
      ...
      int read;
      while (toRead > 0 &&
             ((read = is.read(buffer, byteCount - toRead, toRead)) != -1)) {
        toRead -= read;
      }

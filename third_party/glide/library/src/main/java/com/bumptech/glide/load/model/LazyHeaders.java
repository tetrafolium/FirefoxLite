 ...
 * allowing headers to be constructed lazily.
 *
 * <p> Ideally headers are constructed once and then re-used for multiple loads,
 * rather then being constructed individually for each load. </p>
 *
...
public final class LazyHeaders implements Headers {
  ...
  private Map<String, String> generateHeaders() {
    ...
    for (Map.Entry<String, List<LazyHeaderFactory>> entry :
         headers.entrySet()) {
  ...
  /**
   * Adds an {@link LazyHeaderFactory} that will be used to construct a value
   * for the given key* lazily on a background thread.
   *
   * <p> This class is not thread safe. </p>
   *
   * <p> This class may include default values for User-Agent and
   * Accept-Encoding headers. These will be replaced by calls to either {@link
   * #setHeader(String, LazyHeaderFactory)} or
   * {@link #addHeader(String, String)}, even though {@link #addHeader(String,
   * LazyHeaderFactory)} would usually append an additional value. </p>
   */
  ...
  public static final class Builder {
    ...
    // Set Accept-Encoding header to do our best to avoid gzip since it's both
    // inefficient for images and also makes it more difficult for us to detect
    // and prevent partial content rendering. See #440.
    static {
     ...
     *
     * <p> Use {@link #addHeader(String, LazyHeaderFactory)} if obtaining the
     value requires I/O
     ...
     * for the given key lazily on a background thread.
     *
     * <p> Headers may have multiple values whose order is defined by the order
     * in which this method is called. </p>
     *
     * <p> This class does not prevent you from adding the same value to a given
     * key multiple times </p>
     */
    ...
    /**
     * Replaces all existing {@link LazyHeaderFactory LazyHeaderFactorys} for
     * the given key with the given {@link LazyHeaderFactory}.
     *
     * <p> If the given value is {@code null}, the header at the given key will
     * be removed. </p>
     *
     * <p> Use {@link #setHeader(String, LazyHeaderFactory)} if obtaining the
     * value requires I/O (i.e. an OAuth token). </p>
     */
    public Builder setHeader(String key, String value) {
      return setHeader(key,
                       value == null ? null : new StringHeaderFactory(value));
    }
    ...
    /**
     * Ensures that the default header will pass OkHttp3's checks for header
     * values.
     *
     * <p>See #2331.
     */
  ...
  static final class StringHeaderFactory implements LazyHeaderFactory {
    ...
    StringHeaderFactory(String value) { this.value = value; }

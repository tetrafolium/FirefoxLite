 ...
 * {@link com.bumptech.glide.load.engine.LoadPath}s capable of decoding with the
 * requested types.
 */
public class LoadPathCache {
  ...
  public boolean contains(Class<?> dataClass, Class<?> resourceClass,
                          Class<?> transcodeClass) {
    MultiClassKey key = getKey(dataClass, resourceClass, transcodeClass);
    boolean result;
    synchronized (cache) { result = cache.containsKey(key); }
    keyRef.set(key);
    return result;
  }
  ...
  get(Class<Data> dataClass, Class<TResource> resourceClass,
  ...
  public void put(Class<?> dataClass, Class<?> resourceClass,
                  Class<?> transcodeClass, LoadPath<?, ?, ?> loadPath) {
    synchronized (cache) {
      cache.put(new MultiClassKey(dataClass, resourceClass, transcodeClass),
                loadPath);
    }
  }

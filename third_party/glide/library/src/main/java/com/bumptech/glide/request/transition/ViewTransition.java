...
/**
 * A {@link Transition} that can apply a {@link android.view.animation.Animation
 * Animation} to a
...
public class ViewTransition<R> implements Transition<R> {
  ...
  /**
   * Constructs a new ViewAnimation that will start the given {@link
   ...
   * {@link android.view.View#startAnimation(android.view.animation.Animation)}
   * and then returns
   * {@code false} because the animation does not actually put the current
   * resource on the view.
   *
  ...
  public boolean transition(R current, ViewAdapter adapter) {
    ...
    if (view != null) {
      view.clearAnimation();
      Animation animation =
          viewTransitionAnimationFactory.build(view.getContext());

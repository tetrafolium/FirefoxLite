 ...
 * com.bumptech.glide.load.model.GlideUrl} (http/https URLS) into {@link
 * java.io.InputStream} data.
 */
public class HttpGlideUrlLoader implements ModelLoader<GlideUrl, InputStream> {
  /**
   * An integer option that is used to determine the maximum connect and read
   * timeout durations (in milliseconds) for network connections.
   *
  ...
  public HttpGlideUrlLoader() { this(null); }
  ...
  @Override
  public LoadData<InputStream> buildLoadData(GlideUrl model, int width,
                                             int height, Options options) {
    // GlideUrls memoize parsed URLs so caching them saves a few object
    // instantiations and time spent parsing urls.
    GlideUrl url = model;
    if (modelCache != null) {

...
public final class ByteBufferUtil {
  ...
  public static void toStream(ByteBuffer byteBuffer, OutputStream os)
    ...
    if (safeArray != null) {
      os.write(safeArray.data, safeArray.offset,
               safeArray.offset + safeArray.limit);
    } else {
  ...
  public static byte[] toBytes(ByteBuffer byteBuffer) {
    ...
    if (safeArray != null && safeArray.offset == 0 &&
        safeArray.limit == safeArray.data.length) {
  ...
  public static ByteBuffer fromStream(InputStream stream) throws IOException {
    ...
    // Some resource decoders require a direct byte buffer. Prefer
    // allocateDirect() over wrap()
  ...
  private static SafeArray getSafeArray(ByteBuffer byteBuffer) {
    if (!byteBuffer.isReadOnly() && byteBuffer.hasArray()) {
      return new SafeArray(byteBuffer.array(), byteBuffer.arrayOffset(),
                           byteBuffer.limit());
    }
    return null;
  }
  ...
  private static class ByteBufferStream extends InputStream {
    ...
    public synchronized void reset() throws IOException {
      ...
      // reset() was not implemented correctly in 4.0.4, so we track the mark
      // position ourselves.

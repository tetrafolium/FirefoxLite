...
class SizeStrategy implements LruPoolStrategy {
  ...
  public Bitmap get(int width, int height, Bitmap.Config config) {
    ...
    if (possibleSize != null && possibleSize != size &&
        possibleSize <= size * MAX_SIZE_MULTIPLE) {
      keyPool.offer(key);
      key = keyPool.get(possibleSize);
    }

    // Do a get even if we know we don't have a bitmap so that the key moves to
    // the front in the lru pool

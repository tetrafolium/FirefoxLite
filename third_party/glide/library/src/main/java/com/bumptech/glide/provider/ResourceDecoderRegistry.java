...
/**
 * Contains an ordered list of {@link ResourceDecoder}s capable of decoding
 * arbitrary data types into arbitrary resource types from highest priority
 * decoders to lowest priority decoders.
 */
...
public class ResourceDecoderRegistry {
  ...
  public synchronized void setBucketPriorityList(List<String> buckets) {
    ...
    for (String previousBucket : previousBuckets) {
      if (!buckets.contains(previousBucket)) {
        // Keep any buckets from the previous list that aren't included here,
        // but but them at the end.
  ...
  public synchronized<T, R> List<ResourceDecoder<T, R>>
  getDecoders(Class<T> dataClass, Class<R> resourceClass) {
  ...
  @SuppressWarnings("unchecked")
  public synchronized<T, R> List<Class<R>>
  getResourceClasses(Class<T> dataClass, Class<R> resourceClass) {
  ...
  public synchronized<T, R> void append(String bucket,
                                        ResourceDecoder<T, R> decoder,
                                        Class<T> dataClass,
                                        Class<R> resourceClass) {
    getOrAddEntryList(bucket).add(
        new Entry<>(dataClass, resourceClass, decoder));
  }

  public synchronized<T, R> void prepend(String bucket,
                                         ResourceDecoder<T, R> decoder,
                                         Class<T> dataClass,
                                         Class<R> resourceClass) {
    getOrAddEntryList(bucket).add(
        0, new Entry<>(dataClass, resourceClass, decoder));
  }

...
public class RecyclableBufferedInputStream extends FilterInputStream {
  ...
  /**
   * The currently marked position. -1 indicates no mark has been put or the
   ...
   * blocking for more input. This method returns the number of bytes available
   * in the buffer plus those available in the source stream, but see {@link
   * InputStream#available} for important caveats.
   *
  ...
  /**
   * Reduces the mark limit to match the current buffer length to prevent the
   * buffer from continuing to increase in size.
   *
   * <p>Subsequent calls to {@link #mark(int)} will be obeyed and may cause the
   * buffer size to increase.
   */
  public synchronized void fixMarkLimit() { marklimit = buf.length; }
  ...
  private int fillbuf(InputStream localIn, byte[] localBuf) throws IOException {
    ...
    // Added count == localBuf.length so that we do not immediately double the
    // buffer size before reading any data when marklimit > localBuf.length.
    // Instead, we will double the buffer size only after reading the initial
    // localBuf worth of data without finding what we're looking for in the
    // stream. This allows us to put a relatively small initial buffer size and
    // a large marklimit for safety without causing an allocation each time read
    // is called.
   ...
   * {@link #reset()} will reposition the stream back to the marked position if
   * {@code readlimit} has not been surpassed. The underlying buffer may be
  ...
  /**
   * Indicates whether {@code BufferedInputStream} supports the {@link
   * #mark(int)} and {@link #reset()} methods.
   *
   ...
   * mark has not been put and the requested number of bytes is larger than the
   * receiver's buffer size, this implementation bypasses the buffer and simply
   * places the results directly into {@code buffer}.
   *
   * @param buffer the byte array in which to store the bytes read.
   * @return the number of bytes actually read or -1 if end of stream.
   * @throws IndexOutOfBoundsException if {@code offset < 0} or {@code byteCount
   *     < 0}, or if {@code
   *                                   offset + byteCount} is greater than the
   * size of {@code buffer}.
   ...
   *
   * @throws IOException if this stream is closed, no mark has been put or the
   *     mark is no longer
   *                     valid because more than {@code readlimit} bytes have
  ...
  public synchronized void reset() throws IOException {
    ...
    if (-1 == markpos) {
      throw new InvalidMarkException("Mark has been invalidated, pos: " + pos +
                                     " markLimit: " + marklimit);
    }
    pos = markpos;
  }
  ...
  /**
   * Skips {@code byteCount} bytes in this stream. Subsequent calls to {@link
   * #read} will not return these bytes unless {@link #reset} is used.
   *
   * @param byteCount the number of bytes to skip. {@link #skip} does nothing
   *     and returns 0 if
  ...
  public synchronized long skip(long byteCount) throws IOException {
    // Use local refs since buf and in may be invalidated by an unsynchronized
    // close()
    byte[] localBuf = buf;
    InputStream localIn = in;
    if (localBuf == null) {
  ...
  /**
   * An exception thrown when a mark can no longer be obeyed because the
   * underlying buffer size is smaller than the amount of data read after the
   * mark position.
   */

...
/**
 * A {@link TransitionFactory} for complex types that have a {@link
 * android.graphics.Bitmap} inside. The transitioning bitmap is wrapped in a
 * {@link android.graphics.drawable.BitmapDrawable}. Most commonly used with
 * {@link DrawableCrossFadeFactory}.
 *
 * @param <R> The type of the composite object that contains the {@link
 *     android.graphics.Bitmap} to
 *            be transitioned.
 */
public abstract class BitmapContainerTransitionFactory<R>
  ...
  public BitmapContainerTransitionFactory(
      TransitionFactory<Drawable> realFactory) {
  ...
  public Transition<R> build(DataSource dataSource, boolean isFirstResource) {
    Transition<Drawable> transition =
        realFactory.build(dataSource, isFirstResource);
   ...
   *
   * @param current composite object containing a Bitmap and some other
   *     information
  ...
  private class BitmapGlideAnimation implements Transition<R> {
    ...
    public boolean transition(R current, ViewAdapter adapter) {
      ...
      Drawable currentBitmap =
          new BitmapDrawable(resources, getBitmap(current));

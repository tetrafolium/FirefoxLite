...
/**
 * A coordinator that coordinates two individual {@link Request}s that load a
 * small thumbnail version of an image and the full size version of the image at
 * the same time.
 */
public class ThumbnailRequestCoordinator
  ...
  /**
   * Returns true if the request is either the request loading the full size
   * image or if the request loading the full size image has not yet completed.
   *
  ...
  public boolean canSetImage(Request request) {
    return parentCanSetImage() &&
        (request.equals(full) || !full.isResourceSet());
  }
  ...
  /**
   * Returns true if the request is the request loading the full size image and
   * if neither the full nor the thumbnail image have completed successfully.
   *
   * @param request {@inheritDoc}.
   */
  @Override
  public boolean canNotifyStatusChanged(Request request) {
    return parentCanNotifyStatusChanged() && request.equals(full) &&
        !isAnyResourceSet();
  }
  ...
  public void onRequestSuccess(Request request) {
    ...
    // Clearing the thumb is not necessarily safe if the thumb is being
    // displayed in the Target, as a layer in a cross fade for example. The only
    // way we know the thumb is not being displayed and is therefore safe to
    // clear is if the thumb request has not yet completed.
    if (!thumb.isComplete()) {
      thumb.clear();
    }
  }
  ...
  public boolean isEquivalentTo(Request o) {
    if (o instanceof ThumbnailRequestCoordinator) {
      ...
      return (full == null ? that.full == null
                           : full.isEquivalentTo(that.full)) &&
          (thumb == null ? that.thumb == null
                         : thumb.isEquivalentTo(that.thumb));
    }
    return false;
  }
}

...
/**
 * A fixed size Array Pool that evicts arrays using an LRU strategy to keep the
 * pool under the maximum byte size.
 */
public final class LruArrayPool implements ArrayPool {
  ...
  /**
   * The maximum number of times larger an int array may be to be than a
   * requested size to eligible to be returned from the pool.
   */
  private static final int MAX_OVER_SIZE_MULTIPLE = 8;
  /**
   * Used to calculate the maximum % of the total pool size a single byte array
   * may consume.
   */
   ...
   */
  public LruArrayPool(int maxSize) { this.maxSize = maxSize; }
  ...
  public <T> T get(int size, Class<T> arrayClass) {
    ...
    synchronized (this) {
      ...
      if (result != null) {
        currentSize -= arrayAdapter.getArrayLength(result) *
                       arrayAdapter.getElementSizeInBytes();
  ...
  private boolean mayFillRequest(int requestedSize, Integer actualSize) {
    return actualSize != null &&
        (isNoMoreThanHalfFull() ||
         actualSize <= (MAX_OVER_SIZE_MULTIPLE * requestedSize));
  }
  ...
  private void evict() { evictToSize(maxSize); }

  private void evictToSize(int size) {
    while (currentSize > size) {
      Object evicted = groupedMap.removeLast();
      Preconditions.checkNotNull(evicted);
      ArrayAdapterInterface<Object> arrayAdapter =
          getAdapterFromObject(evicted);
      currentSize -= arrayAdapter.getArrayLength(evicted) *
                     arrayAdapter.getElementSizeInBytes();
      decrementArrayOfSize(arrayAdapter.getArrayLength(evicted),
                           evicted.getClass());
      if (Log.isLoggable(arrayAdapter.getTag(), Log.VERBOSE)) {
        Log.v(arrayAdapter.getTag(),
  ...
  private NavigableMap<Integer, Integer>
  getSizesForAdapter(Class<?> arrayClass) {
  ...
  @SuppressWarnings("unchecked")
  private <T> ArrayAdapterInterface<T>
  getAdapterFromType(Class<T> arrayPoolClass) {
  ...
  int getCurrentSize() {
    ...
    for (Class<?> type : sortedSizes.keySet()) {
      for (Integer size : sortedSizes.get(type).keySet()) {
        ...
        currentSize += size * sortedSizes.get(type).get(size) *
  ...
  private static final class Key implements Poolable {
    ...
    Key(KeyPool pool) { this.pool = pool; }

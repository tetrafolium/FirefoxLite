...
/**
 * A wrapper drawable to square the wrapped drawable so that it expands to fill
 * a square with exactly the given side length. The goal of this drawable is to
 * ensure that square thumbnail drawables always match the size of the view they
 * will be displayed in to avoid a costly requestLayout call. This class should
 * not be used with views or drawables that are not square.
 */
public class FixedSizeDrawable extends Drawable {
  ...
  FixedSizeDrawable(State state, Drawable wrapped) {
    ...
    wrapped.setBounds(0, 0, wrapped.getIntrinsicWidth(),
                      wrapped.getIntrinsicHeight());

    matrix = new Matrix();
    wrappedRect = new RectF(0, 0, wrapped.getIntrinsicWidth(),
                            wrapped.getIntrinsicHeight());
    bounds = new RectF();
  }
  ...
  static class State extends ConstantState {
    ...
    State(State other) { this(other.wrapped, other.width, other.height); }

...
/**
 * Provides implementations of {@link Pool} never return {@code null}, log when
 * new instances are created, and that can use the {@link
 * com.bumptech.glide.util.pool.FactoryPools.Poolable} interface to ensure
 * objects aren't used while inside the pool.
 */
public final class FactoryPools {
   ...
   * {@link Pool#acquire()} and that contains objects of the type created by the
   * given
   * {@link Factory} with the given maximum size.
   *
   * <p>If the pool is empty when {@link Pool#acquire()} is called, the given
   * {@link Factory} will be used to create a new instance.
   *
   * @param <T> The type of object the pool will contains.
   */
  public static <T extends Poolable> Pool<T> simple(int size,
                                                    Factory<T> factory) {
   ...
   * @param <T> The type of object the pool will contains.
   */
  public static <T extends Poolable> Pool<T> threadSafe(int size,
                                                        Factory<T> factory) {
    return build(new SynchronizedPool<T>(size), factory);
  }

  /**
   * Returns a new {@link Pool} that never returns {@code null} and that
   * contains {@link List Lists} of a specific generic type with a standard
   * maximum size of 20.
   *
   * <p>If the pool is empty when {@link Pool#acquire()} is called, a new {@link
  ...
  /**
   * Returns a new thread safe {@link Pool} that never returns {@code null} and
   * that contains
  ...
  public static <T> Pool<List<T>> threadSafeList(int size) {
    return build(new SynchronizedPool<List<T>>(size),
                 new Factory<List<T>>() {
                   @Override
  ...
  private static <T extends Poolable> Pool<T> build(Pool<T> pool,
                                                    Factory<T> factory) {
   ...
   */
  public interface Resetter<T> { void reset(T object); }

  /**
   * Allows additional verification to catch errors caused by using objects
   * while they are in an object pool.
   */
  public interface Poolable { StateVerifier getVerifier(); }

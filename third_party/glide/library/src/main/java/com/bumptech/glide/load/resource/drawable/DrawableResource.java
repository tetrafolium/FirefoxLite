 ...
 * {@link android.graphics.drawable.Drawable.ConstantState#newDrawable() new
 * drawable} based on it's {@link
 * android.graphics.drawable.Drawable.ConstantState state}.
 *
 * <b>Suggested usages only include {@code T}s where the new drawable is of the
 * same or descendant class.</b>
 *
...
public abstract class DrawableResource<T extends Drawable>
  ...
  public final T get() {
    ...
    // If we ever return the original drawable, it's temporary state may be
    // changed and subsequent copies may end up with that temporary state. See
    // #276.

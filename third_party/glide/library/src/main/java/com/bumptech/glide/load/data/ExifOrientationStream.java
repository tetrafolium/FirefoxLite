...
/**
 * Adds an exif segment with an orientation attribute to a wrapped {@link
 * InputStream} containing image data.
 *
 * <p>This class assumes that the wrapped stream contains an image format that
 * can contain exif information and performs no verification. </p>
 */
public final class ExifOrientationStream extends FilterInputStream {
  ...
  public ExifOrientationStream(InputStream in, int orientation) {
    ...
    if (orientation < -1 || orientation > 8) {
      throw new IllegalArgumentException("Cannot add invalid orientation: " +
                                         orientation);
    }
  ...
  public int read(byte[] buffer, int byteOffset, int byteCount)
    ...
    if (position > ORIENTATION_POSITION) {
      ...
      System.arraycopy(EXIF_SEGMENT, position - SEGMENT_START_POSITION, buffer,
                       byteOffset, read);
    }
    if (read > 0) {
      position += read;

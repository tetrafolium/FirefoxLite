...
public interface ArrayPool {
  /**
   * A standard size to use to increase hit rates when the required size isn't
   ...
   *
   * <p>Arrays may be ignored, for example if the array is larger than the
   * maximum size of the pool.
   */
  <T> void put(T array, Class<T> arrayClass);

  /**
   * Returns a non-null array of the given type with a length >= to the given
   * size.
   *
   * <p>If an array of the given size isn't in the pool, a new one will be
   * allocated.
   *
   ...
   *
   * @param level A trim specified in {@link

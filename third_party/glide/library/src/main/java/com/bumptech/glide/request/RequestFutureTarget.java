...
/**
 * A {@link java.util.concurrent.Future} implementation for Glide that can be
 * used to load resources in a blocking manner on background threads.
 *
 * <p> Note - Unlike most targets, RequestFutureTargets can be used once and
 * only once. Attempting to reuse a RequestFutureTarget will probably result in
 * undesirable behavior or exceptions. Instead of reusing objects of this class,
...
public class RequestFutureTarget<R> implements FutureTarget<R>, Runnable {
  ...
  RequestFutureTarget(Handler mainHandler, int width, int height,
                      boolean assertBackgroundThread, Waiter waiter) {
  ...
  private void clearOnMainThread() { mainHandler.post(this); }
  ...
  static class Waiter {
    ...
    public void waitForTimeout(Object toWaitOn, long timeoutMillis)
        throws InterruptedException {
      toWaitOn.wait(timeoutMillis);
    }

    public void notifyAll(Object toNotify) { toNotify.notifyAll(); }
  }
}

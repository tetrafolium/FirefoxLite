...
/**
 * Maintains a cache of Model + Resource class to a set of registered resource
 * classes that are subclasses of the resource class that can be decoded from
 * the model class.
 */
public class ModelToResourceClassCache {
  private final AtomicReference<MultiClassKey> resourceClassKeyRef =
      new AtomicReference<>();
  private final ArrayMap<MultiClassKey, List<Class<?>>>
      registeredResourceClassCache = new ArrayMap<>();
  ...
  public void put(Class<?> modelClass, Class<?> resourceClass,
                  List<Class<?>> resourceClasses) {
    synchronized (registeredResourceClassCache) {
      registeredResourceClassCache.put(
          new MultiClassKey(modelClass, resourceClass), resourceClasses);
    }
  }

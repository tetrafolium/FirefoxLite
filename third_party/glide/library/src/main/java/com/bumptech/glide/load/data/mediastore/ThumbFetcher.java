...
/**
 * A {@link DataFetcher} implementation for {@link InputStream}s that loads data
 * from thumbnail files obtained from the {@link MediaStore}.
 */
...
public class ThumbFetcher implements DataFetcher<InputStream> {
  ...
  // TODO(nnaze): These build methods do not need the full Glide or Context
  // objects.

  public static ThumbFetcher buildImageFetcher(Context context, Uri uri) {
    return build(context, uri,
                 new ImageThumbnailQuery(context.getContentResolver()));
  }

  public static ThumbFetcher buildVideoFetcher(Context context, Uri uri) {
    return build(context, uri,
                 new VideoThumbnailQuery(context.getContentResolver()));
  }

  private static ThumbFetcher build(Context context, Uri uri,
    ...
    ThumbnailStreamOpener opener = new ThumbnailStreamOpener(
        Glide.get(context).getRegistry().getImageHeaderParsers(), query,
        byteArrayPool, context.getContentResolver());
  ...
  static class VideoThumbnailQuery implements ThumbnailQuery {
    ...
    public Cursor query(Uri uri) {
      ...
      return contentResolver.query(
          MediaStore.Video.Thumbnails.EXTERNAL_CONTENT_URI, PATH_PROJECTION,
          PATH_SELECTION, new String[] {videoId}, null /*sortOrder*/);
    }
  }
  ...
  static class ImageThumbnailQuery implements ThumbnailQuery {
    ...
    public Cursor query(Uri uri) {
      ...
      return contentResolver.query(
          MediaStore.Images.Thumbnails.EXTERNAL_CONTENT_URI, PATH_PROJECTION,
          PATH_SELECTION, new String[] {imageId}, null /*sortOrder*/);
    }
  }
}

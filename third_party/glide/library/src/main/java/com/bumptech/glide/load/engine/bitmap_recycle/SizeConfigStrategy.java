 ...
 *
 * <p> Using both the config and the byte size allows us to safely re-use a
 * greater variety of
 * {@link android.graphics.Bitmap Bitmaps}, which increases the hit rate of the
 * pool and therefore the performance of applications. This class works around
 * #301 by only allowing re-use of
 * {@link android.graphics.Bitmap Bitmaps} with a matching number of bytes per
 * pixel. </p>
 */
...
public class SizeConfigStrategy implements LruPoolStrategy {
  ...
  // We probably could allow ARGB_4444 and RGB_565 to decode into each other,
  // but ARGB_4444 is deprecated and we'd rather be safe.
  ...
  public void put(Bitmap bitmap) {
    ...
    NavigableMap<Integer, Integer> sizes =
        getSizesForConfig(bitmap.getConfig());
  ...
  public Bitmap get(int width, int height, Bitmap.Config config) {
    ...
    if (result != null) {
      ...
      result.reconfigure(width, height,
                         result.getConfig() != null ? result.getConfig()
                                                    : Bitmap.Config.ARGB_8888);
    }
    return result;
  }
  ...
  private Key findBestKey(int size, Bitmap.Config config) {
    ...
    for (Bitmap.Config possibleConfig : getInConfigs(config)) {
      NavigableMap<Integer, Integer> sizesForPossibleConfig =
          getSizesForConfig(possibleConfig);
      Integer possibleSize = sizesForPossibleConfig.ceilingKey(size);
      if (possibleSize != null && possibleSize <= size * MAX_SIZE_MULTIPLE) {
        if (possibleSize != size ||
            (possibleConfig == null ? config != null
  ...
  getSizesForConfig(Bitmap.Config config) {
  ...
  public String toString() {
    StringBuilder sb = new StringBuilder()
                           ...
                           .append(groupedMap)
                           .append(", sortedSizes=(");
    for (Map.Entry<Bitmap.Config, NavigableMap<Integer, Integer>> entry :
         sortedSizes.entrySet()) {
  ...
  static final class Key implements Poolable {
    ...
    public Key(KeyPool pool) { this.pool = pool; }

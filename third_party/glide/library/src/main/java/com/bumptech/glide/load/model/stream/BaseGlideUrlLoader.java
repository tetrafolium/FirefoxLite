...
/**
 * A base class for loading data over http/https. Can be subclassed for use with
 * any model that can be translated in to {@link java.io.InputStream} data.
 *
...
public abstract class BaseGlideUrlLoader<Model>
  ...
  public LoadData<InputStream> buildLoadData(Model model, int width, int height,
    ...
    if (result == null) {
      ...
      result =
          new GlideUrl(stringURL, getHeaders(model, width, height, options));
    ...
    // TODO: this is expensive and slow to calculate every time, we should
    // either cache these, or try to come up with a way to avoid finding them
    // when not necessary.
    List<String> alternateUrls =
        getAlternateUrls(model, width, height, options);
    ...
    if (alternateUrls.isEmpty()) {
      ...
      return new LoadData<>(concreteLoaderData.sourceKey,
  ...
  /**
   * Returns a valid url http:// or https:// for the given model and dimensions
   * as a string.
   *
   * @param model  The model.
   * @param width  The width in pixels of the view/target the image will be
   *     loaded into.
   * @param height The height in pixels of the view/target the image will be
   *     loaded into.
   */
  ...
  /**
   * Returns a list of alternate urls for the given model, width, and height
   * from which equivalent data can be obtained (usually the same image with the
   * same aspect ratio, but in a larger size) as the primary url.
   *
   * <p> Implementing this method allows Glide to fulfill requests for bucketed
   * images in smaller bucket sizes using already cached data for larger bucket
   * sizes. </p>
   *
   * @param width  The width in pixels of the view/target the image will be
   *     loaded into.
  ...
  /**
   * Returns the headers for the given model and dimensions as a map of strings
   * to sets of strings, or null if no headers should be added.
   *
   * @param model The model.
   * @param width The width in pixels of the view/target the image will be
   *     loaded into.

...
/**
 * A class that manages a load by adding and removing callbacks for for the load
 * and notifying callbacks when the load completes.
 */
class EngineJob<R> implements DecodeJob.Callback<R>, Poolable {
  ...
  // Used when we realize we're cancelled on a background thread in reschedule
  // and can recycle immediately rather than waiting for a result or an error.
  ...
  // A put of callbacks that are removed while we're notifying other callbacks
  // of a change in status.
  private List<ResourceCallback> ignoredCallbacks;
  private EngineResource<?> engineResource;
  private DecodeJob<R> decodeJob;

  // Checked primarily on the main thread, but also on other threads in
  // reschedule.
  ...
  EngineJob(GlideExecutor diskCacheExecutor, GlideExecutor sourceExecutor,
    ...
    this(diskCacheExecutor, sourceExecutor, sourceUnlimitedExecutor, listener,
         pool, DEFAULT_FACTORY);
  }
  ...
  private GlideExecutor getActiveSourceExecutor() {
    return useUnlimitedSourceGeneratorPool ? sourceUnlimitedExecutor
                                           : sourceExecutor;
  }

  // We cannot remove callbacks while notifying our list of callbacks directly
  // because doing so would cause a ConcurrentModificationException. However, we
  // need to obey the cancellation request such that if notifying a callback
  // early in the callbacks list cancels a callback later in the request list,
  // the cancellation for the later request is still obeyed. Using a put of
  // ignored callbacks allows us to avoid the exception while still meeting the
  // requirement.
  ...
  void cancel() {
    ...
    isCancelled = true;
    decodeJob.cancel();
    // TODO: Consider trying to remove jobs that have never been run before from
    // executor queues. Removing jobs that have run before can break things. See
    // #1996.
  ...
  void handleResultOnMainThread() {
    ...
    // Hold on to resource for duration of request so we don't recycle it in the
    // middle of notifying if it synchronously released by one of the callbacks.
